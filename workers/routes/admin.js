// 2026-07-24 rev.1 — admin.js (AI 프로필 생성: "프로필 생성" 탭용 confirmed/uncertain 판정 추가, 생년만으로 동명이인 확인, 분량 200~1000자 유동, 생년월일 언급 금지, 수상내역 자유형식, 마무리 문장 다양화)
/* ══════════════════════════════════════════════════════════════
   관리자 전용 API 라우트
   GET    /admin/title-map
   GET    /admin/rankings
  ...
  if (path === "/admin/persons/ai-draft" && request.method === "POST") {
    ...
    try {
      ...
      const systemPrompt =
        ...
        "'인정받았다' 정도로만 짧게 언급하고 상 이름을 나열하지 마라. 마지막 문장은 이 인물의 " +
        "현재 활동을 정리하는 문장으로 마무리하되, '꾸준한 사랑을 받고 있다'류의 상투적인 표현을 " +
        "반복하지 말고, 그 사람의 실제 이력(활동 빈도, 최근 행보, 연차 등)에 근거해서 매번 다른 " +
        "표현을 써라. " +
        // [2026-07-24 수정] 수상내역 형식 강제("연도 시상식명 부문(작품명)") 제거 → 찾은 그대로 자유형식.
        "수상내역(awards)은 검색으로 확인되는 것을 최대한 빠짐없이 모아라. 한두 개만 찾고 멈추지 " +
        "말고 여러 차례 검색해서 이 인물이 받은 상을 가능한 한 많이 찾아라. 형식을 억지로 통일하지 " +
