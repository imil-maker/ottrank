// 2026-07-27 rev.12 — admin.js (MBTI 프롬프트 2건 수정 — ① systemPrompt의 "배우" 고정 표현을 jobLabel(배우/감독)로 교체, ② 검색어에 "최신" 추가해서 오래된 MBTI 정보보다 최근 밝힌 정보 우선 검색)
/* ══════════════════════════════════════════════════════════════
   관리자 전용 API 라우트
   GET    /admin/title-map
   GET    /admin/rankings
...
async function _generatePersonMbtiDraft(person, env, opts = {}) {
  ...
  try {
    ...
    const systemPrompt =
      `너는 한국 ${jobLabel}의 MBTI(성격유형)를 조사하는 리서처다. web_search로 이 ${jobLabel}${jobParticle} 방송, 인터뷰, ` +
