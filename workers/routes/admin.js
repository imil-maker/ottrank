// 2026-07-24 rev.5 — admin.js (AI 프로필 생성 프롬프트 재조정: 분량을 데뷔 연차 기준 구간(신인 200~400/중견 400~700/베테랑 700~1000)으로 명확화, 베테랑인데 짧게 쓰는 문제 방지 위해 자체점검 체크리스트에 분량 항목 추가)
/* ══════════════════════════════════════════════════════════════
   관리자 전용 API 라우트
   GET    /admin/title-map
   GET    /admin/rankings
  ...
  if (path === "/admin/persons/ai-draft" && request.method === "POST") {
    ...
    try {
      ...
      const systemPrompt =
        ...
        // [2026-07-24 5차 수정] "정보량에 따라 자연스럽게"라는 모호한 지시로는 부족했음 —
        // 데뷔 30년 넘고 수상·대표작 자료가 충분한 베테랑 배우인데도 공백 제외 280자
        // (신인 수준)로 짧게 쓴 사례가 실제로 확인됨(박희순 배우 테스트). 데뷔 연차를
        // 명확한 기준으로 삼아 구간을 나눠서 지시하도록 강화.
        "분량은 데뷔 연차와 찾은 정보량을 기준으로 명확히 구간을 나눠서 정해라: 데뷔 5년 " +
        "이내이거나 찾을 수 있는 자료 자체가 적은 신인은 200~400자, 데뷔 5~15년 정도인 " +
        "중견은 400~700자, 데뷔 15년이 넘었거나 대표작·수상 이력이 여러 개 확인되는 베테랑· " +
        "원로는 700~1000자를 목표로 써라. 데뷔한 지 오래되고 수상 이력·대표작·활동 이력이 " +
        "여러 개 확인되는 배우인데도 300자 미만으로 짧게 쓰는 것은 잘못이다 — 그런 경우 " +
        "찾은 정보(대표작들, 활동 이력, 이미지 변화 등)를 더 풀어서 최소 600자 이상은 채워라. " +
        "반대로 정말 자료가 없는 신인을 억지로 늘리지는 마라. " +
        "정보가 충분한데도 너무 " +
        "급하게 한두 문장으로 끝내지 마라 — 자료가 있으면 데뷔~현재 흐름, 대표작, 평가, 마무리 " +
        "문장까지 여유 있게 다 담아라. 억지로 글자 수만 채우려고 의미 없는 말을 반복하지는 마라. " +
        ...
        "인물 이름으로 시작하는가, (2) 마지막 문장에 평가 또는 기대감이 담겨 있는가, (3) 이 " +
        "인물의 데뷔 연차·자료량에 비해 분량이 너무 짧지 않은가(베테랑인데 신인 수준으로 짧게 " +
        "쓰지 않았는가). 셋 중 하나라도 아니면 그 부분을 고쳐서 다시 써라. " +
