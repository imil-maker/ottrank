// 2026-07-24 rev.2 — admin.js (AI 프로필 생성 프롬프트 재조정: 마무리 문장 필수화+예시 패턴 추가, 칸·베를린·베니스·아카데미 등 상징적 수상/기록은 프로필에 구체 언급 허용, 수상내역 수집 시 국제 영화제 명시적 확인)
/* ══════════════════════════════════════════════════════════════
   관리자 전용 API 라우트
   GET    /admin/title-map
   GET    /admin/rankings
  ...
  if (path === "/admin/persons/ai-draft" && request.method === "POST") {
    ...
    try {
      ...
      const systemPrompt =
        ...
        "요약(profile)은 이 배우/감독이 어떤 스타일과 강점을 가졌는지, 업계에서 어떤 평가를 " +
        "받는지, 어떤 매력으로 알려져 있는지를 중심으로 한국어 문장으로 서술해라(예: '섬세한 " +
        "감정 표현으로 신뢰받는다', '명품 조연으로 꼽힌다'). 커리어에 중요한 전환점이 된 작품이 " +
        "있다면 자연스럽게 언급해도 되지만, 출연작을 단순히 줄줄이 나열하는 것은 피해라. 분량은 " +
        "찾은 정보량에 따라 자연스럽게 정해라 — 자료가 적은 신인이면 200자 정도로 짧아도 되고, " +
        "자료가 많은 원로 배우면 1000자까지 늘어나도 된다. 억지로 글자 수를 채우지 마라. " +
        ...
        "프로필은 반드시 이 인물에 대한 평가나 기대감을 담은 문장으로 마무리해라 — 마무리 " +
        "문장을 생략하지 마라. 다만 매번 똑같은 문구('꾸준한 사랑을 받고 있다' 등)를 기계적으로 " +
        "반복하지 말고, 아래처럼 이 사람의 실제 이력에 맞는 표현을 골라 다양하게 써라(예시일 " +
        "뿐이니 그대로 베끼지 말고 이 사람 상황에 맞게 변형해라): " +
        "① 연기 스타일·평판이 뚜렷한 베테랑이면 — \"폭발적인 감정 연기와 뛰어난 캐릭터 " +
        "몰입도, 장르를 가리지 않는 연기 스펙트럼으로 국내외 평단과 관객 모두에게 최고의 " +
        "배우로 인정받고 있으며, 한국 영화계를 대표하는 살아있는 전설로 평가받는다\"처럼 " +
        "평가와 기대감을 자연스럽게 엮은 문장. " +
        "② 신인·떠오르는 배우면 — 최근 눈에 띈 작품이나 연기력을 근거로 앞으로가 기대된다는 " +
        "취지의 문장. " +
