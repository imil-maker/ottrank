// 2026-07-27 rev.13 — admin.js (MBTI 판정 기준 완화 — "본인이 직접 밝힌 것만" 요구를 없애고 "검색결과에 그럴듯한 출처로 나오면 인정"으로 바꿈, 근거 없는 추측만 제외. 재미 정보라 지나치게 엄격했던 기준 완화 요청 반영)
/* ══════════════════════════════════════════════════════════════
   관리자 전용 API 라우트
   GET    /admin/title-map
   GET    /admin/rankings
...
async function _generatePersonMbtiDraft(person, env, opts = {}) {
  ...
  try {
    ...
    const systemPrompt =
      `너는 한국 ${jobLabel}의 MBTI(성격유형)를 조사하는 리서처다. web_search로 이 ${jobLabel}${jobParticle} ` +
      "공개적으로 밝힌 MBTI를 찾아라. 이건 재미로 보는 가벼운 정보라 너무 엄격한 기준은 " +
      "필요 없다 — 기사·인터뷰·방송·SNS 등 검색 결과에 나온 정보면 출처가 그럴듯할 때 그대로 " +
      "인정해라(팬이나 커뮤니티가 \"이 사람 성격 보면 OOO 같음\"처럼 근거 없이 추측만 한 것만 " +
      "제외하면 된다). ⚠️ 검색은 딱 1번만 해라 — 검색어에 이름, " +
      "태어난 연도(사용자 메시지에 있으면), '최신 MBTI' 이렇게 넣어서 검색해라(예: " +
      "'홍길동 1990년생 최신 MBTI'). '최신'을 넣는 이유는 MBTI가 시간이 지나 바뀌었다고 " +
      "밝히는 경우가 있어서, 오래된 정보보다 최근에 밝힌 정보를 우선 찾기 위함이다. " +
      "이렇게 하면 동명이인도 걸러지고 결과도 한 번에 정확히 나온다. " +
      "그 한 번의 검색 결과에 그럴듯한 정보가 전혀 없으면 추가로 더 검색하지 말고 바로 " +
      "uncertain으로 답해라. 동명이인 주의: 이름이 같은 다른 사람의 정보를 가져오지 마라. " +
      "사용자 메시지에 태어난 연도가 있으면 그 연도와 일치하는 사람인지 확인해라. " +
      "확신하는지를 \"match\" 필드로 답해라: 그럴듯한 출처에서 MBTI를 찾았으면 \"confirmed\", " +
      "검색 결과가 거의 없거나 순전히 근거 없는 추측뿐이면(동명이인 가능성 포함) " +
      "\"uncertain\"으로 답해라. 완전히 없는 정보를 지어내진 마라 — 확인 안 되면 mbti는 빈 " +
      "문자열로 남겨라. match가 \"uncertain\"이면 왜 못 찾았는지 한 문장(20자 내외)으로 " +
