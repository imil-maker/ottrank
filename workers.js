function x(d,i,t){if(!i.length)return d.slice(0,t).map((r,a)=>({...r,rank:a+1}));let f=new Set(i.map(r=>r.tmdb_id).filter(Boolean)),s=d.filter(r=>!f.has(r.tmdb_id)),o={};for(let r of i){let a=Math.max(1,parseInt(r.rank)||1);o[a]||(o[a]=[]),o[a].push(r)}let n=[],c=0,e=1;for(;n.length<t;){if(o[e]&&o[e].length){let r=o[e].shift();n.push({...r,rank:n.length+1})}else if(c<s.length)n.push({...s[c],rank:n.length+1}),c++;else{let r=Object.values(o).flat();for(let a of r){if(n.length>=t)break;n.push({...a,rank:n.length+1})}break}e++}return n}async function G(d,i,t,f,s){if(d==="/rankings"&&i.method==="GET"){let o=f.searchParams.get("platform"),n=f.searchParams.get("category"),c=f.searchParams.get("date"),e="SELECT * FROM rankings WHERE 1=1",r=[];o&&(e+=" AND platform = ?",r.push(o)),n&&(e+=" AND category = ?",r.push(n)),c?(e+=" AND date = ?",r.push(c)):e+=" AND date = (SELECT MAX(date) FROM rankings)",e+=" ORDER BY platform, category, rank";let{results:a}=await t.DB.prepare(e).bind(...r).all();return new Response(JSON.stringify({ok:!0,data:a}),{headers:s})}if(d==="/rankings/main"&&i.method==="GET")try{let o=f.searchParams.get("date")||null,{results:n}=await t.DB.prepare(`
        SELECT
      ...
      `).bind(o,n).all(),{results:e}=await t.DB.prepare(`
        ...
        WHERE r.platform = ?
          ...
          AND r.date = 'manual'
        ORDER BY oc.platform_order, r.rank
      `).bind(o).all(),r={},a={},l={};for(let _ of c){let g=_.category_slot;r[g]||(r[g]=[]),l[g]||(l[g]=_),r[g].push(_)}for(let _ of e){let g=_.category_slot;a[g]||(a[g]=[]),l[g]||(l[g]=_),a[g].push(_)}let m={},p=new Set([...Object.keys(r),...Object.keys(a)]);for(let _ of p){let g=l[_];if(!g)continue;let k=g.platform_limit||20,E=x((r[_]||[]).sort((w,y)=>w.rank-y.rank),(a[_]||[]).sort((w,y)=>w.rank-y.rank),k);m[_]={platform:g.platform,category_slot:g.category_slot,display_name:g.display_name,platform_order:g.platform_order,memo_label:g.memo_label||null,items:E.map(w=>({rank:w.rank,title_ko:w.title_ko,title_en:w.title_en,tmdb_id:w.tmdb_id,poster_path:w.poster_path,genre:w.genre,tmdb_rating:w.tmdb_rating,release_year:w.release_year,memo:w.memo||null}))}}let u=Object.values(m).sort((_,g)=>_.platform_order-g.platform_order);return new Response(JSON.stringify({ok:!0,data:u}),{headers:s})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:s})}if(d==="/rankings/weekly"&&i.method==="GET")try{let{results:o}=await t.DB.prepare(`
        SELECT
    ...
    `).bind(o).all();return new Response(JSON.stringify({ok:!0,data:n}),{headers:s})}if(d.startsWith("/rankings/platforms/")&&i.method==="GET"){let o=parseInt(d.split("/rankings/platforms/")[1]);if(!o)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:s});try{let{results:n}=await t.DB.prepare(`
        ...
        GROUP BY platform
        ORDER BY rank ASC
      `).bind(o).all();return new Response(JSON.stringify({ok:!0,data:n}),{headers:s})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:s})}}if(d==="/rankings/platforms-batch"&&i.method==="GET"){let o=(f.searchParams.get("tmdb_ids")||"").trim();if(!o)return new Response(JSON.stringify({ok:!1,message:"tmdb_ids required"}),{status:400,headers:s});let n=[...new Set(o.split(",").map(c=>parseInt(c.trim())).filter(c=>Number.isInteger(c)&&c>0))].slice(0,50);if(!n.length)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD55C tmdb_ids\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4"}),{status:400,headers:s});try{let c=n.map(()=>"?").join(","),{results:e}=await t.DB.prepare(`
...
`),p=(await(await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":f.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:4e3,messages:[{role:"user",content:a}]})})).json()).content?.[0]?.text||"[]",u=[];try{let _=p.split("```json").join("").split("```").join("").trim(),g=JSON.parse(_);u=Array.isArray(g)?g:[]}catch{console.error("[REACTION] Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328:",p.slice(0,300)),u=[]}await f.DB.prepare("DELETE FROM reaction_comments WHERE reaction_id = ?").bind(d).run();for(let _=0;_<e.length;_++){let g=e[_],E=(u.find(w=>w.idx===_)||u.find(w=>w.idx===_+1)||u[_]||{}).translated||"";await f.DB.prepare(`
      ...
      `).bind(d,t,g.text.slice(0,1e3),E.slice(0,1e3),g.author.slice(0,100),g.likes,"neutral").run()}console.log(`[REACTION] \u2705 \uC644\uB8CC: reaction=${d} \uB313\uAE00 ${e.length}\uAC1C \uC800\uC7A5`)}catch(s){console.error("[REACTION] \uC624\uB958:",s.message)}}async function v(d,i,t,f,s,o){if(d.startsWith("/videos/")&&!d.includes("/admin")&&i.method==="GET"){let n=parseInt(d.split("/videos/")[1]);if(!n)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:o});try{let{results:c}=await t.DB.prepare("SELECT * FROM title_videos WHERE tmdb_id = ? ORDER BY is_main DESC, created_at DESC").bind(n).all();return c.length===0&&f.waitUntil(Z(n,t)),new Response(JSON.stringify({ok:!0,data:c}),{headers:o})}catch(c){return new Response(JSON.stringify({ok:!1,message:c.message}),{status:500,headers:o})}}if(d==="/admin/videos/crawl"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});try{let n=await i.json(),{tmdb_id:c}=n;if(!c)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:o});let e=await X(parseInt(c),t);return new Response(JSON.stringify({ok:!0,saved:e}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}}if(d==="/admin/videos/batch-crawl"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});try{let n=20;try{let E=await i.json();E?.limit&&Number.isInteger(E.limit)&&E.limit>0&&(n=E.limit)}catch{}let c=30,r=(await t.DB.prepare("SELECT COUNT(*) AS cnt FROM works WHERE yt_crawl_attempted_at >= date('now')").first())?.cnt||0;if(r>=c){let E=await t.DB.prepare(`
        ...
        ORDER BY
          ...
          w.created_at DESC
        LIMIT ?
      `).bind(m,a).all();if(!p.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uB300\uC0C1 \uC791\uD488 \uC5C6\uC74C (\uBAA8\uB450 \uCFE8\uB2E4\uC6B4 \uC911\uC774\uAC70\uB098 \uC601\uC0C1\uC774 \uC774\uBBF8 \uCDA9\uBD84\uD568)"}),{headers:o});let u=[],_=0;for(let E of p)try{let w=await Q(E.tmdb_id,t);_+=w,u.push({tmdb_id:E.tmdb_id,saved:w,ok:!0})}catch(w){console.error(`[BATCH_CRAWL] tmdb_id=${E.tmdb_id} \uC624\uB958:`,w.message),u.push({tmdb_id:E.tmdb_id,saved:0,ok:!1,error:w.message})}let k=(await t.DB.prepare(`
        SELECT COUNT(*) AS cnt
        FROM works w
        WHERE (
          SELECT COUNT(*) FROM title_videos tv WHERE tv.tmdb_id = w.tmdb_id
      ...
      `).first())?.cnt||0;return console.log(`[BATCH_CRAWL] \u2705 \uC644\uB8CC: \uC2DC\uB3C4 ${p.length}\uAC74, \uC800\uC7A5 ${_}\uAC1C, \uB0A8\uC74C ${k}`),new Response(JSON.stringify({ok:!0,attempted:p.length,filled:_,remaining:k,results:u}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}}if(d==="/admin/videos"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});try{let n=await i.json(),{tmdb_id:c,youtube_url:e}=n,{title:r}=n;if(!c||!e)return new Response(JSON.stringify({ok:!1,message:"tmdb_id, youtube_url required"}),{status:400,headers:o});let a=e.match(/(?:v=|youtu\.be\/|embed\/)([a-zA-Z0-9_-]{11})/);if(!a)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD558\uC9C0 \uC54A\uC740 \uC720\uD29C\uBE0C URL"}),{status:400,headers:o});let l=a[1],m=await t.DB.prepare("SELECT id, title FROM title_videos WHERE tmdb_id = ? AND youtube_id = ? LIMIT 1").bind(c,l).first();if(m)return new Response(JSON.stringify({ok:!1,message:`\uC774\uBBF8 \uB4F1\uB85D\uB41C \uC601\uC0C1\uC785\uB2C8\uB2E4. (\uC81C\uBAA9: "${m.title||l}")`}),{status:409,headers:o});if(!r)try{r=(await(await fetch(`https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${l}&format=json`)).json()).title||""}catch{r=""}return await t.DB.prepare("INSERT INTO title_videos (tmdb_id, youtube_url, youtube_id, title, is_main) VALUES (?, ?, ?, ?, 0)").bind(c,e,l,r).run(),new Response(JSON.stringify({ok:!0}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}}if(d.match(/\/admin\/videos\/(\d+)\/main/)&&i.method==="PATCH"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});let n=parseInt(d.match(/\/admin\/videos\/(\d+)\/main/)[1]);try{let{results:c}=await t.DB.prepare("SELECT tmdb_id FROM title_videos WHERE id = ?").bind(n).all();if(!c.length)return new Response(JSON.stringify({ok:!1,message:"\uC5C6\uC74C"}),{status:404,headers:o});let e=c[0].tmdb_id;return await t.DB.batch([t.DB.prepare("UPDATE title_videos SET is_main = 0 WHERE tmdb_id = ?").bind(e),t.DB.prepare("UPDATE title_videos SET is_main = 1 WHERE id = ?").bind(n)]),new Response(JSON.stringify({ok:!0}),{headers:o})}catch(c){return new Response(JSON.stringify({ok:!1,message:c.message}),{status:500,headers:o})}}if(d.match(/\/admin\/videos\/(\d+)$/)&&i.method==="DELETE"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});let n=parseInt(d.match(/\/admin\/videos\/(\d+)$/)[1]);try{return await t.DB.prepare("DELETE FROM title_videos WHERE id = ?").bind(n).run(),new Response(JSON.stringify({ok:!0}),{headers:o})}catch(c){return new Response(JSON.stringify({ok:!1,message:c.message}),{status:500,headers:o})}}if(d.startsWith("/imdb/")&&d!=="/imdb/save"&&i.method==="GET"){let n=d.split("/imdb/")[1];if(!n||!/^tt\d+$/.test(n))return new Response(JSON.stringify({ok:!1,message:"invalid imdb_id"}),{status:400,headers:o});try{let c=await t.DB.prepare("SELECT imdb_rating, imdb_votes, imdb_updated FROM works WHERE imdb_id = ? LIMIT 1").bind(n).first();if(c?.imdb_rating){let l=new Date(c.imdb_updated||0);if((Date.now()-l.getTime())/(1e3*60*60*24)<7)return new Response(JSON.stringify({ok:!0,source:"cache",rating:c.imdb_rating.toFixed(1),votes:c.imdb_votes||""}),{headers:o})}let e=t.OMDB_API_KEY;if(!e)return new Response(JSON.stringify({ok:!1,message:"OMDB key not configured"}),{status:500,headers:o});let a=await(await fetch(`https://www.omdbapi.com/?i=${n}&apikey=${e}`)).json();if(a.Response!=="False"){let l=parseFloat(a.imdbRating);if(!isNaN(l)){let m=a.imdbVotes||"",p=new Date().toISOString();return await t.DB.prepare("UPDATE works SET imdb_rating = ?, imdb_votes = ?, imdb_updated = ? WHERE imdb_id = ?").bind(l,m,p,n).run(),new Response(JSON.stringify({ok:!0,source:"omdb",rating:l.toFixed(1),votes:m}),{headers:o})}}return new Response(JSON.stringify({ok:!1,message:"rating not available"}),{status:404,headers:o})}catch(c){return console.error("[IMDB GET]",c),new Response(JSON.stringify({ok:!1,message:c.message}),{status:500,headers:o})}}if(d==="/imdb/save"&&i.method==="POST")try{let n=await i.json(),{tmdb_id:c,imdb_id:e}=n;return!c||!e?new Response(JSON.stringify({ok:!1,message:"tmdb_id and imdb_id required"}),{status:400,headers:o}):/^tt\d+$/.test(e)?(await t.DB.prepare("UPDATE works SET imdb_id = ? WHERE tmdb_id = ?").bind(e,parseInt(c)).run(),new Response(JSON.stringify({ok:!0}),{headers:o})):new Response(JSON.stringify({ok:!1,message:"invalid imdb_id format"}),{status:400,headers:o})}catch(n){return console.error("[IMDB SAVE]",n),new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}if(d==="/youtube/trending"&&i.method==="GET")try{let{results:n}=await t.DB.prepare("SELECT * FROM youtube_trending ORDER BY rank ASC").all();if(n.length>0){let p=new Date(n[0].collected_at);if((Date.now()-p.getTime())/(1e3*60*60)<6)return new Response(JSON.stringify({ok:!0,data:n,cached:!0}),{headers:o})}let c=`https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&chart=mostPopular&regionCode=KR&maxResults=50&key=${t.YOUTUBE_API_KEY}`,e=await fetch(c),r=await e.json();if(!e.ok||!r.items?.length)return n.length>0?new Response(JSON.stringify({ok:!0,data:n,cached:!0}),{headers:o}):new Response(JSON.stringify({ok:!1,message:"YouTube API \uC624\uB958"}),{status:500,headers:o});let a=new Date().toISOString(),l=r.items.map((p,u)=>({rank:u+1,video_id:p.id,title:p.snippet?.title||"",channel:p.snippet?.channelTitle||"",thumbnail:p.snippet?.thumbnails?.medium?.url||p.snippet?.thumbnails?.default?.url||"",view_count:parseInt(p.statistics?.viewCount||0),collected_at:a}));await t.DB.prepare("DELETE FROM youtube_trending").run();let m=l.map(p=>t.DB.prepare(`
        ...
        ORDER BY
          ...
          title_ko ASC
        LIMIT ?
      `).bind(`%${e}%`,`%${e}%`,`${e}%`,c).all();return new Response(JSON.stringify({ok:!0,data:r}),{headers:o})}catch(r){return new Response(JSON.stringify({ok:!1,message:r.message}),{status:500,headers:o})}}if(d==="/works/register"&&i.method==="POST")try{let n=await i.json(),{tmdb_id:c,title_ko:e,title_en:r,poster_path:a,media_type:l,genre:m,original_language:p,tmdb_rating:u,release_date:_}=n;if(!c||!e)return new Response(JSON.stringify({ok:!1,message:"tmdb_id, title_ko required"}),{status:400,headers:o});let g=r&&/[\uAC00-\uD7A3]/.test(r),E=r&&/[a-zA-Z]/.test(r)&&!g?r:null,w=u??null,y=_||null,b=new Date().toISOString();return await t.DB.prepare(`
        INSERT INTO works (
      ...
      `).bind(n).all(),l=new Map;try{let p=await t.DB.prepare("SELECT MAX(date) as d FROM rankings WHERE date != 'manual'").first();if(p?.d){let{results:u}=await t.DB.prepare(`
            SELECT tmdb_id, COUNT(DISTINCT platform) as cnt
            FROM rankings
            WHERE date = ?
            GROUP BY tmdb_id
          `).bind(p.d).all();for(let _ of u)l.set(_.tmdb_id,_.cnt)}}catch{}let m=[];for(let p of a){let u=(p.variety_genre||"").split(",").map(w=>w.trim()).filter(Boolean),_=r.filter(w=>u.includes(w)).length;if(!_)continue;let g=null;if(r.length===2?g=_===2?92:82:r.length===1&&(g=_===1?87:null),!g)continue;let k=l.get(p.tmdb_id)||0,E=Math.min(g+k,99);m.push({tmdb_id:p.tmdb_id,title_ko:p.title_ko,title_en:p.title_en,poster_path:p.poster_path,tmdb_rating:p.tmdb_rating,release_year:p.release_year,match_pct:E,media_type:p.media_type||null})}return m.sort((p,u)=>u.match_pct-p.match_pct||(u.release_year||0)-(p.release_year||0)||(u.tmdb_rating||0)-(p.tmdb_rating||0)),new Response(JSON.stringify({ok:!0,data:m.slice(0,c)}),{headers:o})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:o})}}if(d.startsWith("/works/")&&i.method==="GET"){let n=d.split("/works/")[1];if(!n)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:o});let{results:c}=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(parseInt(n)).all();if(!c.length)return new Response(JSON.stringify({ok:!1,message:"Not found"}),{status:404,headers:o});let e={...c[0]};if(!e.mbti_tags&&e.genre){let _=yt(e.genre);_&&(f.waitUntil(t.DB.prepare("UPDATE works SET mbti_tags = ? WHERE tmdb_id = ?").bind(_,parseInt(n)).run()),e.mbti_tags=_)}let r=7200*60*1e3,a=2400*60*60*1e3,l=!1;try{let{results:_}=await t.DB.prepare(`
        SELECT 1 FROM rankings
        WHERE tmdb_id = ? AND date = (SELECT MAX(date) FROM rankings WHERE date != 'manual')
        LIMIT 1
      `).bind(parseInt(n)).all();l=!!(_&&_.length)}catch{l=!1}let m=l?r:a;if(!e.keyword_preview_updated_at||Date.now()-new Date(e.keyword_preview_updated_at).getTime()>m){let _={keyword:null,items:[]};if(e.keywords&&e.keywords!=="__NONE__"){let k=e.keywords.split(",").map(E=>E.trim()).filter(Boolean).slice(0,10);if(k.length)try{let E=k.map(y=>t.DB.prepare(`
                ...
                ORDER BY
                  ...
                  w.tmdb_rating DESC
                LIMIT 20
              `).bind(y.toLowerCase(),parseInt(n))),w=await t.DB.batch(E);for(let y=0;y<k.length;y++){let b=w[y]?.results||[];if(b.length>=3){_={keyword:k[y],items:b};break}}}catch{}}let g=new Date().toISOString();e.keyword_preview=JSON.stringify(_),e.keyword_preview_updated_at=g,f.waitUntil(t.DB.prepare("UPDATE works SET keyword_preview = ?, keyword_preview_updated_at = ? WHERE tmdb_id = ?").bind(e.keyword_preview,g,parseInt(n)).run())}try{let{results:_}=await t.DB.prepare(`
      ...
      `).bind(parseInt(n)).all();e.pinned_similar=_||[]}catch{e.pinned_similar=[]}if(!e.keyword_ko_map_updated_at||Date.now()-new Date(e.keyword_ko_map_updated_at).getTime()>m){let _={},g=!1;if(e.keywords&&e.keywords!=="__NONE__"){let k=e.keywords.split(",").map(E=>E.trim().toLowerCase()).filter(Boolean);if(k.length)try{let E=k.map(()=>"?").join(","),{results:w}=await t.DB.prepare(`SELECT keyword_en, keyword_ko FROM keyword_translation WHERE keyword_en IN (${E}) AND source = 'admin'`).bind(...k).all();for(let y of w||[])_[y.keyword_en]=y.keyword_ko}catch{g=!0}}if(e.keyword_ko_map=_,!g){let k=new Date().toISOString();f.waitUntil(t.DB.prepare("UPDATE works SET keyword_ko_map = ?, keyword_ko_map_updated_at = ? WHERE tmdb_id = ?").bind(JSON.stringify(_),k,parseInt(n)).run())}}else try{e.keyword_ko_map=e.keyword_ko_map?JSON.parse(e.keyword_ko_map):{}}catch{e.keyword_ko_map={}}return new Response(JSON.stringify({ok:!0,data:e}),{headers:o})}if(d==="/search/keyword"&&i.method==="GET"){let n=(s.searchParams.get("keyword")||"").trim().toLowerCase(),c=Math.min(parseInt(s.searchParams.get("limit")||"20"),40);if(!n)return new Response(JSON.stringify({ok:!1,message:"keyword required"}),{status:400,headers:o});try{let{results:e}=await t.DB.prepare(`
      ...
      `).bind(l,m,p).run();let _=await t.DB.prepare("SELECT id, nickname, last_login_bonus_date FROM users WHERE provider = 'naver' AND provider_id = ?").bind(l).first(),g=!u||!u.nickname||u.nickname.trim()==="",k=crypto.randomUUID(),E=new Date(Date.now()+720*60*60*1e3).toISOString();await t.DB.prepare("INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)").bind(k,_.id,E).run();let w=s.searchParams.get("state")||"",y="";try{y=w?decodeURIComponent(w):""}catch{}if(y.startsWith("/")||(y=""),!g){let N=new Date(Date.now()+324e5).toISOString().slice(0,10);_.last_login_bonus_date!==N&&(await H(_.id,3,"login",t),await t.DB.prepare("UPDATE users SET last_login_bonus_date = ? WHERE id = ?").bind(N,_.id).run())}let b=g?`https://ottrank.kr/signup.html?sid=${k}`+(y?`&redirect=${encodeURIComponent(y)}`:""):`https://ottrank.kr/mypage.html?sid=${k}`;return new Response(null,{status:302,headers:{Location:b,"Set-Cookie":`session=${k}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${720*60*60}`}})}catch(n){return console.error("[AUTH] \uB124\uC774\uBC84 \uCF5C\uBC31 \uC624\uB958:",n.message),Response.redirect("https://ottrank.kr?login=fail",302)}}if(d==="/auth/kakao"&&i.method==="GET"){let o=s.searchParams.get("redirect")||"",n=o?encodeURIComponent(o):"",c="https://kauth.kakao.com/oauth/authorize?client_id="+t.KAKAO_CLIENT_ID+"&redirect_uri="+encodeURIComponent("https://ottrank-api.tdidream.workers.dev/auth/kakao/callback")+"&response_type=code"+(n?"&state="+n:"");return Response.redirect(c,302)}if(d==="/auth/kakao/callback"&&i.method==="GET"){let o=s.searchParams.get("code");if(!o)return Response.redirect("https://ottrank.kr?login=fail",302);try{let c=await(await fetch("https://kauth.kakao.com/oauth/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"authorization_code",client_id:t.KAKAO_CLIENT_ID,client_secret:t.KAKAO_CLIENT_SECRET,redirect_uri:"https://ottrank-api.tdidream.workers.dev/auth/kakao/callback",code:o})})).json();if(!c.access_token)return Response.redirect("https://ottrank.kr?login=fail",302);let r=await(await fetch("https://kapi.kakao.com/v2/user/me",{headers:{Authorization:"Bearer "+c.access_token}})).json(),a=String(r.id),l=r.kakao_account?.profile?.profile_image_url||"",m=r.kakao_account?.email||"",p=await t.DB.prepare("SELECT id, nickname FROM users WHERE provider = 'kakao' AND provider_id = ?").bind(a).first();await t.DB.prepare(`
        ...
        ON CONFLICT(provider, provider_id) DO UPDATE SET
          ...
          avatar_url = excluded.avatar_url,
          last_login = datetime('now')
      `).bind(a,m,l).run();let u=await t.DB.prepare("SELECT id, nickname, last_login_bonus_date FROM users WHERE provider = 'kakao' AND provider_id = ?").bind(a).first(),_=!p||!p.nickname||p.nickname.trim()==="",g=crypto.randomUUID(),k=new Date(Date.now()+720*60*60*1e3).toISOString();await t.DB.prepare("INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)").bind(g,u.id,k).run();let E=s.searchParams.get("state")||"",w=E?decodeURIComponent(E):"";if(!_){let b=new Date(Date.now()+324e5).toISOString().slice(0,10);u.last_login_bonus_date!==b&&(await H(u.id,3,"login",t),await t.DB.prepare("UPDATE users SET last_login_bonus_date = ? WHERE id = ?").bind(b,u.id).run())}let y=_?`https://ottrank.kr/signup.html?sid=${g}`+(w?`&redirect=${encodeURIComponent(w)}`:""):`https://ottrank.kr/mypage.html?sid=${g}`;return new Response(null,{status:302,headers:{Location:y,"Set-Cookie":`session=${g}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${720*60*60}`}})}catch(n){return console.error("[AUTH] \uCE74\uCE74\uC624 \uCF5C\uBC31 \uC624\uB958:",n.message),Response.redirect("https://ottrank.kr?login=fail",302)}}if(d==="/auth/me"&&i.method==="GET")try{let c=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!c)return new Response(JSON.stringify({ok:!1}),{headers:f});let e=await t.DB.prepare("SELECT * FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(c).first();if(!e)return new Response(JSON.stringify({ok:!1}),{headers:f});let r=await t.DB.prepare("SELECT id, nickname, email, avatar_url, provider, grade, total_likes_received, mbti, ott_points, created_at, last_login_bonus_date FROM users WHERE id = ?").bind(e.user_id).first();if(!r)return new Response(JSON.stringify({ok:!1}),{headers:f});let a=new Date(Date.now()+540*60*1e3).toISOString().slice(0,10);r.last_login_bonus_date!==a&&(await H(r.id,3,"login",t),await t.DB.prepare("UPDATE users SET last_login_bonus_date = ? WHERE id = ?").bind(a,r.id).run(),r.ott_points=(r.ott_points||0)+3,r.last_login_bonus_date=a);let l=await t.DB.prepare("SELECT grade_name, grade_key, emoji_url, sort_order FROM grade_settings WHERE grade_key = ?").bind(r.grade||"rookie").first();return new Response(JSON.stringify({ok:!0,user:{...r,gradeInfo:l||null}}),{headers:f})}catch{return new Response(JSON.stringify({ok:!1}),{headers:f})}if(d==="/auth/random-nickname"&&i.method==="GET")try{let n=(await t.DB.prepare(`
        ...
        ORDER BY RANDOM()
        LIMIT 1
      `).first())?.title_ko||"\uB4DC\uB77C\uB9C8\uD32C",c=$[Math.floor(Math.random()*$.length)],e=Math.floor(Math.random()*9e3)+1e3,r=`${c}${n}${e}`;return r.length>20&&(r=`${c}${n.slice(0,6)}${e}`),new Response(JSON.stringify({ok:!0,nickname:r}),{headers:f})}catch{let n=$[Math.floor(Math.random()*$.length)],c=Math.floor(Math.random()*9e3)+1e3;return new Response(JSON.stringify({ok:!0,nickname:`${n}\uC2DC\uB124\uB9C8${c}`}),{headers:f})}if(d==="/auth/nickname"&&i.method==="POST")try{let o=await i.json(),{nickname:n,sid:c,mbti:e}=o,r=c||h(i);if(!r)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778\uC774 \uD544\uC694\uD574\uC694"}),{status:401,headers:f});let a=await t.DB.prepare("SELECT * FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(r).first();if(!a)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158\uC774 \uB9CC\uB8CC\uB410\uC5B4\uC694"}),{status:401,headers:f});if(!n||n.trim().length<2)return new Response(JSON.stringify({ok:!1,message:"\uB2C9\uB124\uC784\uC740 2\uC790 \uC774\uC0C1 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:f});if(n.trim().length>20)return new Response(JSON.stringify({ok:!1,message:"\uB2C9\uB124\uC784\uC740 20\uC790 \uC774\uB0B4\uB85C \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:f});if(!/^[가-힣a-zA-Z0-9]+$/.test(n.trim()))return new Response(JSON.stringify({ok:!1,message:"\uD55C\uAE00, \uC601\uBB38, \uC22B\uC790\uB9CC \uC0AC\uC6A9\uD560 \uC218 \uC788\uC5B4\uC694"}),{status:400,headers:f});if(await t.DB.prepare("SELECT id FROM users WHERE nickname = ? AND id != ?").bind(n.trim(),a.user_id).first())return new Response(JSON.stringify({ok:!1,message:"\uC774\uBBF8 \uC0AC\uC6A9 \uC911\uC778 \uB2C9\uB124\uC784\uC774\uC5D0\uC694"}),{status:400,headers:f});let p=e&&["INTJ","INTP","ENTJ","ENTP","INFJ","INFP","ENFJ","ENFP","ISTJ","ISFJ","ESTJ","ESFJ","ISTP","ISFP","ESTP","ESFP"].includes(e)?e:null;return await t.DB.prepare("UPDATE users SET nickname = ?, mbti = ? WHERE id = ?").bind(n.trim(),p,a.user_id).run(),await t.DB.prepare("SELECT id FROM user_point_logs WHERE user_id = ? AND reason = 'signup' LIMIT 1").bind(a.user_id).first()||await H(a.user_id,30,"signup",t),p&&(await t.DB.prepare("SELECT id FROM user_point_logs WHERE user_id = ? AND reason = 'mbti_register' LIMIT 1").bind(a.user_id).first()||await H(a.user_id,20,"mbti_register",t)),new Response(JSON.stringify({ok:!0}),{headers:f})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:f})}if(d==="/auth/nickname"&&i.method==="PUT")try{let n=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!n)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:f});let c=await t.DB.prepare("SELECT * FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(n).first();if(!c)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:f});let e=await i.json(),{nickname:r}=e;return!r||r.trim().length<2?new Response(JSON.stringify({ok:!1,message:"\uB2C9\uB124\uC784\uC740 2\uC790 \uC774\uC0C1 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:f}):r.trim().length>20?new Response(JSON.stringify({ok:!1,message:"\uB2C9\uB124\uC784\uC740 20\uC790 \uC774\uB0B4\uB85C \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:f}):/^[가-힣a-zA-Z0-9]+$/.test(r.trim())?await t.DB.prepare("SELECT id FROM users WHERE nickname = ? AND id != ?").bind(r.trim(),c.user_id).first()?new Response(JSON.stringify({ok:!1,message:"\uC774\uBBF8 \uC0AC\uC6A9 \uC911\uC778 \uB2C9\uB124\uC784\uC774\uC5D0\uC694"}),{status:400,headers:f}):(await t.DB.prepare("UPDATE users SET nickname = ? WHERE id = ?").bind(r.trim(),c.user_id).run(),new Response(JSON.stringify({ok:!0}),{headers:f})):new Response(JSON.stringify({ok:!1,message:"\uD55C\uAE00, \uC601\uBB38, \uC22B\uC790\uB9CC \uC0AC\uC6A9\uD560 \uC218 \uC788\uC5B4\uC694"}),{status:400,headers:f})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:f})}if(d==="/auth/withdraw"&&i.method==="DELETE")try{let n=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!n)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:f});let c=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(n).first();if(!c)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:f});let e=c.user_id;return await t.DB.prepare("DELETE FROM sessions  WHERE user_id = ?").bind(e).run(),await t.DB.prepare("DELETE FROM wishlist  WHERE user_id = ?").bind(e).run(),await t.DB.prepare("DELETE FROM reviews   WHERE user_id = ?").bind(e).run(),await t.DB.prepare("DELETE FROM posts     WHERE user_id = ?").bind(e).run(),await t.DB.prepare("DELETE FROM users     WHERE id = ?").bind(e).run(),new Response(JSON.stringify({ok:!0}),{headers:{...f,"Set-Cookie":"session=; Path=/; HttpOnly; Secure; Max-Age=0"}})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:f})}if(d==="/auth/mbti"&&i.method==="PATCH")try{let n=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!n)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:f});let c=await t.DB.prepare("SELECT * FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(n).first();if(!c)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:f});let e=await i.json(),{mbti:r}=e,l=r&&["INTJ","INTP","ENTJ","ENTP","INFJ","INFP","ENFJ","ENFP","ISTJ","ISFJ","ESTJ","ESFJ","ISTP","ISFP","ESTP","ESFP"].includes(r)?r:null,m=await t.DB.prepare("SELECT mbti FROM users WHERE id = ?").bind(c.user_id).first();await t.DB.prepare("UPDATE users SET mbti = ? WHERE id = ?").bind(l,c.user_id).run();let p=!!m?.mbti,u=!!l;return!p&&u?await t.DB.prepare("SELECT id FROM user_point_logs WHERE user_id = ? AND reason = 'mbti_register' LIMIT 1").bind(c.user_id).first()||await H(c.user_id,20,"mbti_register",t):p&&!u&&await H(c.user_id,-20,"mbti_unregister",t),new Response(JSON.stringify({ok:!0,mbti:l}),{headers:f})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:f})}if(d==="/auth/logout"&&i.method==="POST")try{let c=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);return c&&await t.DB.prepare("DELETE FROM sessions WHERE id = ?").bind(c).run(),new Response(JSON.stringify({ok:!0}),{headers:{...f,"Set-Cookie":"session=; Path=/; HttpOnly; Secure; Max-Age=0"}})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:f})}return null}async function et(d,i,t,f,s){if(d==="/wishlist"&&i.method==="GET")try{let c=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!c)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:s});let e=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(c).first();if(!e)return new Response(JSON.stringify({ok:!1}),{status:401,headers:s});let{results:r}=await t.DB.prepare("SELECT * FROM wishlist WHERE user_id = ? ORDER BY created_at DESC").bind(e.user_id).all();return new Response(JSON.stringify({ok:!0,data:r}),{headers:s})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:s})}if(d==="/wishlist"&&i.method==="POST")try{let c=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!c)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:s});let e=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(c).first();if(!e)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:s});let r=await i.json(),{tmdb_id:a,title_ko:l,poster_path:m,release_year:p,category:u}=r;return a?await t.DB.prepare("SELECT id FROM wishlist WHERE user_id = ? AND tmdb_id = ?").bind(e.user_id,parseInt(a)).first()?(await t.DB.prepare("DELETE FROM wishlist WHERE user_id = ? AND tmdb_id = ?").bind(e.user_id,parseInt(a)).run(),f.waitUntil(F(e.user_id,t)),new Response(JSON.stringify({ok:!0,wishlisted:!1}),{headers:s})):(await t.DB.prepare("INSERT INTO wishlist (user_id, tmdb_id, title_ko, poster_path, release_year, category) VALUES (?, ?, ?, ?, ?, ?)").bind(e.user_id,parseInt(a),l||"",m||"",p||"",u||"movie").run(),f.waitUntil(H(e.user_id,1,"wishlist",t)),f.waitUntil(F(e.user_id,t)),new Response(JSON.stringify({ok:!0,wishlisted:!0}),{headers:s})):new Response(JSON.stringify({ok:!1,message:"tmdb_id \uD544\uC694"}),{status:400,headers:s})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:s})}if(d.match(/^\/wishlist\/check\/\d+$/)&&i.method==="GET")try{let n=parseInt(d.split("/")[3]),e=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!e)return new Response(JSON.stringify({ok:!0,wishlisted:!1}),{headers:s});let r=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(e).first();if(!r)return new Response(JSON.stringify({ok:!0,wishlisted:!1}),{headers:s});let a=await t.DB.prepare("SELECT id FROM wishlist WHERE user_id = ? AND tmdb_id = ?").bind(r.user_id,n).first();return new Response(JSON.stringify({ok:!0,wishlisted:!!a}),{headers:s})}catch{return new Response(JSON.stringify({ok:!0,wishlisted:!1}),{headers:s})}if(d.match(/^\/reviews\/\d+$/)&&i.method==="GET")try{let n=parseInt(d.split("/")[2]),e=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i),r=-1;if(e){let l=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(e).first();l&&(r=l.user_id)}let{results:a}=await t.DB.prepare(`
      ...
      `).bind(n).first();return c?new Response(JSON.stringify({ok:!0,data:c}),{headers:o}):new Response(JSON.stringify({ok:!1,message:"Not found"}),{status:404,headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}if(d==="/posts"&&i.method==="POST")try{let c=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!c)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:o});let e=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(c).first();if(!e)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:o});let r=await i.json(),{board_type:a,title:l,content:m}=r;if(!["recommend","free","community"].includes(a))return new Response(JSON.stringify({ok:!1,message:"\uC62C\uBC14\uB978 \uAC8C\uC2DC\uD310\uC744 \uC120\uD0DD\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});if(!l||l.trim().length<2)return new Response(JSON.stringify({ok:!1,message:"\uC81C\uBAA9\uC740 2\uC790 \uC774\uC0C1 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});if(l.trim().length>100)return new Response(JSON.stringify({ok:!1,message:"\uC81C\uBAA9\uC740 100\uC790 \uC774\uB0B4\uB85C \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});if(!m||m.trim().length<5)return new Response(JSON.stringify({ok:!1,message:"\uB0B4\uC6A9\uC740 5\uC790 \uC774\uC0C1 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});let p=await t.DB.prepare("INSERT INTO posts (board_type, user_id, title, content) VALUES (?, ?, ?, ?)").bind(a,e.user_id,l.trim(),m.trim()).run();return f.waitUntil(F(e.user_id,t)),new Response(JSON.stringify({ok:!0,id:p.meta?.last_row_id}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}if(d.match(/^\/posts\/\d+$/)&&i.method==="PATCH")try{let n=parseInt(d.split("/")[2]),e=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!e)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:o});let r=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(e).first();if(!r)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:o});let a=await t.DB.prepare("SELECT user_id FROM posts WHERE id = ?").bind(n).first();if(!a)return new Response(JSON.stringify({ok:!1,message:"\uAC8C\uC2DC\uAE00 \uC5C6\uC74C"}),{status:404,headers:o});if(a.user_id!==r.user_id)return new Response(JSON.stringify({ok:!1,message:"\uAD8C\uD55C \uC5C6\uC74C"}),{status:403,headers:o});let l=await i.json(),{title:m,content:p}=l;return await t.DB.prepare("UPDATE posts SET title = ?, content = ?, updated_at = datetime('now') WHERE id = ?").bind(m.trim(),p.trim(),n).run(),new Response(JSON.stringify({ok:!0}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}if(d.match(/^\/posts\/\d+$/)&&i.method==="DELETE")try{let n=parseInt(d.split("/")[2]),e=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!e)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:o});let r=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(e).first();if(!r)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:o});let a=await t.DB.prepare("SELECT user_id FROM posts WHERE id = ?").bind(n).first();return a?a.user_id!==r.user_id?new Response(JSON.stringify({ok:!1,message:"\uAD8C\uD55C \uC5C6\uC74C"}),{status:403,headers:o}):(await t.DB.prepare("DELETE FROM posts WHERE id = ?").bind(n).run(),f.waitUntil(F(r.user_id,t)),new Response(JSON.stringify({ok:!0}),{headers:o})):new Response(JSON.stringify({ok:!1,message:"\uAC8C\uC2DC\uAE00 \uC5C6\uC74C"}),{status:404,headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}if(d.match(/^\/posts\/\d+\/like$/)&&i.method==="POST")try{let n=parseInt(d.split("/")[2]),c=await t.DB.prepare("SELECT user_id FROM posts WHERE id = ?").bind(n).first();return await t.DB.prepare("UPDATE posts SET like_count = like_count + 1 WHERE id = ?").bind(n).run(),c?.user_id&&(await t.DB.prepare("UPDATE users SET total_likes_received = total_likes_received + 1 WHERE id = ?").bind(c.user_id).run(),f.waitUntil(F(c.user_id,t))),new Response(JSON.stringify({ok:!0}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}return null}async function Y(d,i,t,f,s){let o=d.match(/^\/work-ott\/(\d+)$/);if(o&&i.method==="GET"){let e=parseInt(o[1]);try{let{results:r}=await t.DB.prepare(`SELECT id, tmdb_id, ott_key, action, created_at
         ...
         DO UPDATE SET action = excluded.action,
                       created_at = datetime('now')`).bind(r,a,l).run(),new Response(JSON.stringify({ok:!0}),{headers:s})):new Response(JSON.stringify({ok:!1,error:"action\uC740 'add' \uB610\uB294 'remove'\uB9CC \uD5C8\uC6A9"}),{status:400,headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,error:e.message}),{status:500,headers:s})}}let n=d.match(/^\/work-ott\/(\d+)$/);if(n&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,error:"Unauthorized"}),{status:401,headers:s});let e=parseInt(n[1]);try{return await t.DB.prepare("DELETE FROM work_ott_overrides WHERE id = ?").bind(e).run(),new Response(JSON.stringify({ok:!0}),{headers:s})}catch(r){return new Response(JSON.stringify({ok:!1,error:r.message}),{status:500,headers:s})}}if(d==="/admin/title-map"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=parseInt(f.searchParams.get("page")||"1"),r=50,a=(e-1)*r,{results:l}=await t.DB.prepare("SELECT * FROM title_map ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(r,a).all();return new Response(JSON.stringify({ok:!0,data:l}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/rankings"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{platform:r,category_slot:a,date:l,tmdb_id:m,rank:p,title_ko:u,title_en:_,media_type:g,is_manual:k}=e;if(!r||!a||!l||!m||!u)return new Response(JSON.stringify({ok:!1,message:"platform, category_slot, date, tmdb_id, title_ko \uD544\uC218"}),{status:400,headers:s});let E=null,w=u||null,y=_||null,b=null,N=null,R=null,D=g==="tv"||g==="movie"?g:null;try{let T=D?[D]:["tv","movie"];for(let L of T){let C=await fetch(`https://api.themoviedb.org/3/${L}/${m}?language=ko-KR&api_key=${t.TMDB_API_KEY}`);if(!C.ok)continue;let I=await C.json();if(!(!I.poster_path&&!I.name&&!I.title)){if(E=I.poster_path||null,b=parseInt((I.first_air_date||I.release_date||"").slice(0,4))||null,R=I.vote_average?parseFloat(I.vote_average.toFixed(1)):null,N=(I.genres||[]).map(B=>B.name).join(", ")||null,D||(D=L),w||(w=I.name||I.title||null),!y){let B=await fetch(`https://api.themoviedb.org/3/${L}/${m}?language=en-US&api_key=${t.TMDB_API_KEY}`);if(B.ok){let J=await B.json(),M=J.original_title||J.original_name||"",W=J.title||J.name||"";y=/[\uAC00-\uD7A3]/.test(M)?W:M||W}}break}}}catch{}await t.DB.prepare(`
      ...
      `).bind(parseInt(m),w||"",y||"",E,D,w||null,y||null,E,D).run();let S=parseInt(p)||null;return S||(S=((await t.DB.prepare("SELECT MAX(rank) as max_rank FROM rankings WHERE platform = ? AND category_slot = ? AND date = ?").bind(r,a,l).first())?.max_rank||0)+1),await t.DB.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(r,a,a,l,-S,parseInt(m),w||"",y||"",E,b,N,R,k?1:0,a).run(),await t.DB.prepare("UPDATE rankings SET rank = ? WHERE platform = ? AND category_slot = ? AND date = ? AND rank = ?").bind(S,r,a,l,-S).run(),y&&w&&await t.DB.prepare(`
          INSERT INTO title_map (title_en, title_ko, tmdb_id)
          VALUES (?, ?, ?)
          ON CONFLICT(title_en) DO UPDATE SET
            title_ko = excluded.title_ko,
            tmdb_id  = COALESCE(excluded.tmdb_id, tmdb_id)
        `).bind(y.trim(),w.trim(),parseInt(m)).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('ranking_add', ?, ?, ?, ?)").bind(r,a,String(m),JSON.stringify({rank:S,title_ko:w,date:l})).run(),new Response(JSON.stringify({ok:!0,rank:S,poster_path:E,title_ko:w,title_en:y}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/rankings"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});let e=f.searchParams.get("date"),r=f.searchParams.get("manual"),a,l;r==="true"?(a="SELECT * FROM rankings WHERE date = 'manual' ORDER BY platform, category_slot, rank",l=null):e?(a="SELECT * FROM rankings WHERE date = ? ORDER BY platform, category_slot, rank",l=e):(a="SELECT * FROM rankings WHERE date = (SELECT MAX(date) FROM rankings WHERE date != 'manual') ORDER BY platform, category_slot, rank",l=null);let{results:m}=l?await t.DB.prepare(a).bind(l).all():await t.DB.prepare(a).all();return new Response(JSON.stringify({ok:!0,data:m}),{headers:s})}if(d==="/admin/fix"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{id:r,tmdb_id:a,title_ko:l,title_en:m,delete_duplicates:p,media_type:u}=e,_=e.season!==void 0?e.season:void 0,g=e.poster_path||null;if(!r)return new Response(JSON.stringify({ok:!1,message:"id required"}),{status:400,headers:s});let k=null,E=l||null,w=m||null,y=await t.DB.prepare("SELECT title_ko, title_en, poster_path FROM rankings WHERE id = ?").bind(parseInt(r)).first();if(a)try{let D=u==="movie"?["movie"]:u==="tv"?["tv"]:["tv","movie"];for(let S of D){let T=await fetch(`https://api.themoviedb.org/3/${S}/${a}?language=ko-KR&api_key=${t.TMDB_API_KEY}`);if(!T.ok)continue;let L=await T.json();if(!(!L.poster_path&&!L.name&&!L.title)){if(k=L.poster_path||null,E||(E=L.name||L.title||null),!w){let C=await fetch(`https://api.themoviedb.org/3/${S}/${a}?language=en-US&api_key=${t.TMDB_API_KEY}`);if(C.ok){let I=await C.json(),B=I.original_title||I.original_name||"",J=I.title||I.name||"";w=/[\uAC00-\uD7A3]/.test(B)?J:B||J}}break}}}catch{}g&&(k=g);let b=_!==void 0?_!==null?parseInt(_):null:void 0;if(await t.DB.prepare(`
        UPDATE rankings
        SET tmdb_id     = COALESCE(?, tmdb_id),
            ...
            is_manual   = 1
        WHERE id = ?
      `).bind(a?parseInt(a):null,E,w,k,...b!==void 0?[b]:[],parseInt(r)).run(),a){p&&(w&&await t.DB.prepare("DELETE FROM works WHERE title_en = ? AND tmdb_id != ?").bind(w,parseInt(a)).run(),E&&/[\uAC00-\uD7A3]/.test(E)&&await t.DB.prepare("DELETE FROM works WHERE title_ko = ? AND tmdb_id != ?").bind(E,parseInt(a)).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, memo) VALUES ('works_delete', ?, ?)").bind(String(a),`\uC911\uBCF5 \uC0AD\uC81C: title_en="${w}" title_ko="${E}"`).run());let D=u==="tv"||u==="movie"?u:null,S=g?null:k;await t.DB.prepare(`
      ...
      `).bind(r,a,E,l,m||l,p||20,u||10,_||20,g??1).run();let w=await t.DB.prepare("SELECT * FROM ott_categories WHERE platform = ? AND category_slot = ?").bind(r,a).first();return await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, after_value) VALUES ('category_create', ?, ?, ?)").bind(r,a,JSON.stringify(e)).run(),new Response(JSON.stringify({ok:!0,data:w}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/review-queue/count"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await t.DB.prepare("SELECT COUNT(*) as count FROM review_queue WHERE status = 'pending'").first();return new Response(JSON.stringify({ok:!0,count:e?.count||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/review-queue"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=f.searchParams.get("status")||"pending",r=f.searchParams.get("platform"),a="SELECT * FROM review_queue WHERE status = ?",l=[e];r&&(a+=" AND platform = ?",l.push(r)),a+=" ORDER BY crawled_date DESC, platform, category_slot, rank";let{results:m}=await t.DB.prepare(a).bind(...l).all();return new Response(JSON.stringify({ok:!0,data:m}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d.match(/^\/admin\/review-queue\/\d+\/resolve$/)&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=parseInt(d.split("/")[3]),r=await i.json(),{tmdb_id:a,title_ko:l,title_en:m}=r;if(!a)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:s});let p=await t.DB.prepare("SELECT * FROM review_queue WHERE id = ?").bind(e).first();if(!p)return new Response(JSON.stringify({ok:!1,message:"Queue item not found"}),{status:404,headers:s});let u=null,_=l,g=m;try{for(let E of["tv","movie"]){let w=await fetch(`https://api.themoviedb.org/3/${E}/${a}?language=ko-KR&api_key=${t.TMDB_API_KEY}`);if(w.ok){let y=await w.json();if(y.name||y.title){u=y.poster_path||null,_||(_=y.name||y.title);break}}}if(!g)for(let E of["tv","movie"]){let w=await fetch(`https://api.themoviedb.org/3/${E}/${a}?language=en-US&api_key=${t.TMDB_API_KEY}`);if(w.ok){let y=await w.json();if(y.name||y.title){g=y.title||y.name;break}}}}catch{}if(r.delete_duplicates===!0&&(g||p.title_en)){let E=g||p.title_en;await t.DB.prepare("DELETE FROM works WHERE title_en = ? AND tmdb_id != ?").bind(E,parseInt(a)).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value, memo) VALUES ('works_delete', ?, ?, ?)").bind(String(a),JSON.stringify({title_en:E}),`\uC911\uBCF5 \uC0AD\uC81C: title_en="${E}" tmdb_id!=${a}`).run()}return await t.DB.prepare(`
        ...
        ON CONFLICT(tmdb_id) DO UPDATE SET
          ...
          confidence_score = 100,
          updated_at       = datetime('now')
      `).bind(parseInt(a),_||"",g||"",u,_||null,g||null,u).run(),await t.DB.prepare(`
        UPDATE rankings SET
          tmdb_id     = ?,
          title_ko    = COALESCE(?, title_ko),
          title_en    = COALESCE(?, title_en),
      ...
      `).bind(parseInt(a),_||null,g||null,u,p.platform,p.category_slot,p.rank,p.crawled_date).run(),await t.DB.prepare(`
        UPDATE review_queue SET
          status           = 'resolved',
          resolved_tmdb_id = ?,
          resolved_at      = datetime('now')
        WHERE id = ?
      `).bind(parseInt(a),e).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('queue_resolve', ?, ?, ?, ?)").bind(p.platform,p.category_slot,String(a),JSON.stringify({tmdb_id:a,title_ko:_,title_en:g})).run(),new Response(JSON.stringify({ok:!0,poster_path:u,title_ko:_,title_en:g}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/rank-override"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{platform:r,category_slot:a,date:l,tmdb_id:m,original_rank:p,override_rank:u,reason:_}=e;return!r||!a||!l||!m||!u?new Response(JSON.stringify({ok:!1,message:"\uD544\uC218 \uD30C\uB77C\uBBF8\uD130 \uB204\uB77D"}),{status:400,headers:s}):(await t.DB.prepare(`
        ...
        ON CONFLICT(platform, category_slot, date, tmdb_id) DO UPDATE SET
          ...
          updated_at    = datetime('now')
      `).bind(r,a,l,parseInt(m),p||0,parseInt(u),_||null).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, before_value, after_value) VALUES ('rank_override', ?, ?, ?, ?, ?)").bind(r,a,String(m),JSON.stringify({rank:p}),JSON.stringify({rank:u,reason:_})).run(),new Response(JSON.stringify({ok:!0}),{headers:s}))}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/rank-override"&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{platform:r,category_slot:a,date:l,tmdb_id:m}=e;return await t.DB.prepare("DELETE FROM rank_overrides WHERE platform = ? AND category_slot = ? AND date = ? AND tmdb_id = ?").bind(r,a,l,parseInt(m)).run(),new Response(JSON.stringify({ok:!0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d.match(/^\/admin\/works\/\d+$/)&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=parseInt(d.split("/")[3]),r=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(e).first();return r?new Response(JSON.stringify({ok:!0,data:r}),{headers:s}):new Response(JSON.stringify({ok:!1,message:"Not found"}),{status:404,headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=f.searchParams.get("q")||"",r=f.searchParams.get("filter")||"",a=f.searchParams.get("date")||"",l=f.searchParams.get("sort")||"recent",m=parseInt(f.searchParams.get("page")||"1"),p=50,u=(m-1)*p,_=l==="updated"?"updated_at DESC, id DESC":"COALESCE(created_at, updated_at) DESC, id DESC",g,k;r==="new_match"&&a?(g=`SELECT * FROM works WHERE first_matched_date = ? AND match_source IN ('auto_claude', 'auto_tmdb') ORDER BY ${_} LIMIT ? OFFSET ?`,k=[a,p,u]):e?(g=`SELECT * FROM works WHERE title_ko LIKE ? OR title_en LIKE ? ORDER BY ${_} LIMIT ? OFFSET ?`,k=[`%${e}%`,`%${e}%`,p,u]):(g=`SELECT * FROM works ORDER BY ${_} LIMIT ? OFFSET ?`,k=[p,u]);let{results:E}=await t.DB.prepare(g).bind(...k).all();return new Response(JSON.stringify({ok:!0,data:E}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d.match(/^\/admin\/works\/\d+$/)&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=parseInt(d.split("/")[3]),r=await i.json(),{title_ko:a,title_en:l,poster_path:m,delete_duplicates:p,media_type:u,mbti_tags:_}=r,g=u==="tv"||u==="movie"?u:null,k=_!==void 0,E=k?_||null:void 0,w=await t.DB.prepare("SELECT title_ko, title_en, poster_path, media_type FROM works WHERE tmdb_id = ?").bind(e).first();if(p&&(l||w?.title_en)){let y=l||w?.title_en;await t.DB.prepare("DELETE FROM works WHERE title_en = ? AND tmdb_id != ?").bind(y,e).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value, memo) VALUES ('works_delete', ?, ?, ?)").bind(String(e),JSON.stringify({title_en:y}),`\uC911\uBCF5 \uC0AD\uC81C: title_en="${y}" tmdb_id!=${e}`).run()}return await t.DB.prepare(`
        UPDATE works SET
          title_ko         = COALESCE(?, title_ko),
          title_en         = COALESCE(?, title_en),
          poster_path      = COALESCE(?, poster_path),
      ...
      `).bind(a||null,l||null,m||null,g,...k?[E]:[],e).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value, after_value) VALUES ('works_update', ?, ?, ?)").bind(String(e),JSON.stringify(w),JSON.stringify(r)).run(),new Response(JSON.stringify({ok:!0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d.match(/^\/admin\/works\/\d+$/)&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=parseInt(d.split("/")[3]),r=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(e).first();return await t.DB.prepare("DELETE FROM works WHERE tmdb_id = ?").bind(e).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value) VALUES ('works_delete', ?, ?)").bind(String(e),JSON.stringify(r)).run(),new Response(JSON.stringify({ok:!0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/new-match-count"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=f.searchParams.get("date")||new Date().toISOString().slice(0,10),r=await t.DB.prepare("SELECT COUNT(*) as count FROM works WHERE first_matched_date = ? AND match_source IN ('auto_claude','auto_tmdb')").bind(e).first();return new Response(JSON.stringify({ok:!0,count:r?.count||0,date:e}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/manual-rankings"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=f.searchParams.get("platform"),r=f.searchParams.get("category_slot");if(!e||!r)return new Response(JSON.stringify({ok:!1,message:"platform, category_slot required"}),{status:400,headers:s});let{results:a}=await t.DB.prepare(`
        ...
        ORDER BY rank ASC
      `).bind(e,r).all();return new Response(JSON.stringify({ok:!0,data:a}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/manual-rankings"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{platform:r,category_slot:a,source_name:l,tmdb_id:m,rank:p,memo:u}=e,_=e.season!==void 0?e.season:null;if(!r||!a||!m||!p)return new Response(JSON.stringify({ok:!1,message:"platform, category_slot, tmdb_id, rank required"}),{status:400,headers:s});let g=e.title_ko||"",k=e.title_en||"",E=e.poster_path||null,w=e.genre||null,y=e.overview||null,b=e.release_year||null,N=e.tmdb_rating??null,R=e.media_type==="tv"||e.media_type==="movie"?e.media_type:null;if(!g||!E||!k){let S=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(parseInt(m)).first();S&&(g=g||S.title_ko||"",k=k||S.title_en||"",E=E||S.poster_path||null,w=w||S.genre||null,y=y||S.overview||null,b=b||S.release_year||null,N=N??S.tmdb_rating??null)}if(!k)try{let S=R?[R]:["tv","movie"];for(let T of S){let L=await fetch(`https://api.themoviedb.org/3/${T}/${m}?language=en-US&api_key=${t.TMDB_API_KEY}`);if(!L.ok)continue;let C=await L.json();if(!C.name&&!C.title)continue;let I=C.original_title||C.original_name||"",B=C.title||C.name||"";k=/[\uAC00-\uD7A3]/.test(I)?B:I||B;break}}catch{}await t.DB.prepare(`
        INSERT INTO rankings
      ...
      `).bind(r,a,a,l||"",parseInt(p),g,k,parseInt(m),E,w,y,b,N,u||null,_!==null?parseInt(_):null).run();let D=new Date().toISOString();return await t.DB.prepare(`
      ...
      `).all();if(!e.length)return new Response(JSON.stringify({ok:!0,updated:0,message:"\uB3D9\uAE30\uD654\uD560 \uB370\uC774\uD130 \uC5C6\uC74C"}),{headers:s});let r=e.map(a=>t.DB.prepare("UPDATE rankings SET tmdb_rating = (SELECT tmdb_rating FROM works WHERE tmdb_id = ?) WHERE id = ?").bind(a.tmdb_id,a.id));return await t.DB.batch(r),new Response(JSON.stringify({ok:!0,updated:e.length}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/collect-keywords"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||20,50),{results:a}=await t.DB.prepare(`
        ...
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,processed:0,attempted:0,remaining:0,message:"\uC218\uC9D1\uD560 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let l=0,m=0,p=[];for(let _ of a){let g=_.media_type?[_.media_type]:["tv","movie"],k="",E=!1;for(let w of g)try{let y=await fetch(`https://api.themoviedb.org/3/${w}/${_.tmdb_id}/keywords?api_key=${t.TMDB_API_KEY}`);if(!y.ok)continue;E=!0;let b=await y.json(),N=b.keywords||b.results||[];if(N.length){k=N.map(R=>R.name).filter(Boolean).join(",");break}}catch{}k?(p.push(t.DB.prepare("UPDATE works SET keywords = ? WHERE tmdb_id = ?").bind(k,_.tmdb_id)),l++):E?p.push(t.DB.prepare("UPDATE works SET keywords = ? WHERE tmdb_id = ?").bind("__NONE__",_.tmdb_id)):m++}p.length&&await t.DB.batch(p);let u=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE keywords IS NULL OR keywords = ''").first();return new Response(JSON.stringify({ok:!0,processed:l,attempted:a.length,skippedRetry:m,remaining:u?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/backfill-normalize-keywords"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||200,300),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, keywords FROM works
        WHERE keywords_normalized_at IS NULL
          AND keywords IS NOT NULL AND keywords != ''
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,processed:0,attempted:0,remaining:0,message:"\uC815\uADDC\uD654\uD560 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let l=[],m=0,p=new Date().toISOString();for(let _ of a){if(_.keywords&&_.keywords!=="__NONE__"){let g=new Set(_.keywords.split(",").map(k=>k.trim().toLowerCase()).filter(Boolean));if(g.size){for(let k of g)l.push(t.DB.prepare("INSERT OR IGNORE INTO work_keywords (tmdb_id, keyword) VALUES (?, ?)").bind(_.tmdb_id,k)),l.push(t.DB.prepare("INSERT OR IGNORE INTO keyword_translation (keyword_en) VALUES (?)").bind(k));m++}}l.push(t.DB.prepare("UPDATE works SET keywords_normalized_at = ? WHERE tmdb_id = ?").bind(p,_.tmdb_id))}l.length&&await t.DB.batch(l);let u=await t.DB.prepare(`
        SELECT COUNT(*) as cnt FROM works
        WHERE keywords_normalized_at IS NULL
          AND keywords IS NOT NULL AND keywords != ''
      `).first();return new Response(JSON.stringify({ok:!0,processed:m,attempted:a.length,remaining:u?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/keywords/translate"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});if(!t.ANTHROPIC_API_KEY)return new Response(JSON.stringify({ok:!1,message:"ANTHROPIC_API_KEY\uAC00 Workers Secrets\uC5D0 \uC124\uC815\uB418\uC5B4 \uC788\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4"}),{status:500,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||40,60),{results:a}=await t.DB.prepare(`
...
${p}`,g=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":t.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:2e3,system:u,messages:[{role:"user",content:_}]})});if(!g.ok){let S=await g.text().catch(()=>"");return new Response(JSON.stringify({ok:!1,message:`Claude API \uC624\uB958 (status ${g.status})`,detail:S.slice(0,300)}),{status:502,headers:s})}let E=((await g.json()).content||[]).filter(S=>S.type==="text").map(S=>S.text).join(""),w;try{let S=E.replace(/```json|```/g,"").trim();w=JSON.parse(S)}catch{return new Response(JSON.stringify({ok:!1,message:"Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694",raw:E.slice(0,300)}),{status:502,headers:s})}Array.isArray(w)||(w=[]);let y=new Set(l),b=new Map;for(let S of w){let T=parseInt(S.tmdb_id);if(!T)continue;let L=Array.isArray(S.tags)?S.tags.filter(C=>y.has(C)).slice(0,2):[];b.set(T,L)}let N=[],R=0;for(let S of m){if(!b.has(S.tmdb_id))continue;let T=b.get(S.tmdb_id);N.push(t.DB.prepare("UPDATE works SET variety_genre = ?, variety_genre_source = 'auto' WHERE tmdb_id = ?").bind(T.length?T.join(","):null,S.tmdb_id)),R++}N.length&&await t.DB.batch(N);let D=await t.DB.prepare(`
      ...
      `).bind(e).all(),a=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE variety_genre_source = 'auto'").first();return new Response(JSON.stringify({ok:!0,items:r,remaining:a?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/variety-review"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),a=(Array.isArray(e.items)?e.items:[]).filter(p=>p&&p.tmdb_id&&Array.isArray(p.tags));if(!a.length)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD55C \uD56D\uBAA9\uC774 \uC5C6\uC5B4\uC694"}),{status:400,headers:s});let l=a.map(p=>{let u=p.tags.filter(Boolean).slice(0,2);return t.DB.prepare("UPDATE works SET variety_genre = ?, variety_genre_source = 'admin' WHERE tmdb_id = ?").bind(u.length?u.join(","):null,parseInt(p.tmdb_id))});await t.DB.batch(l);let m=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE variety_genre_source = 'auto'").first();return new Response(JSON.stringify({ok:!0,updated:a.length,remaining:m?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/variety-review/skip"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Array.isArray(e.tmdb_ids)?e.tmdb_ids.map(m=>parseInt(m)).filter(m=>Number.isInteger(m)):[];if(!r.length)return new Response(JSON.stringify({ok:!1,message:"tmdb_ids required"}),{status:400,headers:s});let a=new Date().toISOString(),l=r.map(m=>t.DB.prepare("UPDATE works SET variety_review_skipped_at = ? WHERE tmdb_id = ?").bind(a,m));return await t.DB.batch(l),new Response(JSON.stringify({ok:!0,skipped:r.length}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/pinned-similar"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=parseInt(e.tmdb_id),a=parseInt(e.related_tmdb_id),l=parseInt(e.pinned_pct);if((!l||l<1||l>99)&&(l=99),!r||!a)return new Response(JSON.stringify({ok:!1,message:"\uB450 \uC791\uD488\uC758 tmdb_id\uAC00 \uBAA8\uB450 \uD544\uC694\uD569\uB2C8\uB2E4"}),{status:400,headers:s});if(r===a)return new Response(JSON.stringify({ok:!1,message:"\uAC19\uC740 \uC791\uD488\uB07C\uB9AC\uB294 \uC5F0\uACB0\uD560 \uC218 \uC5C6\uC5B4\uC694"}),{status:400,headers:s});let{results:m}=await t.DB.prepare("SELECT tmdb_id FROM works WHERE tmdb_id IN (?, ?)").bind(r,a).all();return m.length<2?new Response(JSON.stringify({ok:!1,message:"works \uD14C\uC774\uBE14\uC5D0 \uC5C6\uB294 \uC791\uD488\uC774 \uD3EC\uD568\uB418\uC5B4 \uC788\uC5B4\uC694"}),{status:400,headers:s}):(await t.DB.batch([t.DB.prepare(`
        ...
        `).bind(r,a,l),t.DB.prepare(`
          ...
          VALUES (?, ?, ?)
          ON CONFLICT(tmdb_id, related_tmdb_id) DO UPDATE SET pinned_pct = excluded.pinned_pct
        `).bind(a,r,l)]),new Response(JSON.stringify({ok:!0,pinned_pct:l}),{headers:s}))}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d.startsWith("/admin/works/pinned-similar/")&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=parseInt(d.split("/admin/works/pinned-similar/")[1]);if(!e)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:s});let{results:r}=await t.DB.prepare(`
      ...
      `).bind(r,a,a,r).run(),new Response(JSON.stringify({ok:!0}),{headers:s}))}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/persons/collect"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||20,50),{results:a}=await t.DB.prepare(`
        ...
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,worksScanned:0,personsFound:0,remaining:0,message:"\uC2A4\uCE94\uD560 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let l=new Map,m=[];for(let _ of a){m.push(_.tmdb_id);let g=_.media_type==="tv"?"tv":"movie",k=g==="tv"?"aggregate_credits":"credits";try{let E=await fetch(`https://api.themoviedb.org/3/${g}/${_.tmdb_id}/${k}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json();for(let y of(w.cast||[]).slice(0,15))y.id&&y.name&&!l.has(y.id)&&l.set(y.id,{name:y.name,job:"act"});for(let y of w.crew||[])(y.job==="Director"||y.job==="Creator"||y.department==="Directing"||(y.jobs||[]).some(N=>N.job==="Director"||N.job==="Creator"))&&y.id&&y.name&&l.set(y.id,{name:y.name,job:"direct"})}catch{}}let p=[];for(let[_,g]of l)p.push(t.DB.prepare(`INSERT INTO persons (tmdb_id, name, job) VALUES (?, ?, ?)
             ON CONFLICT(tmdb_id) DO NOTHING`).bind(_,g.name,g.job));for(let _ of m)p.push(t.DB.prepare("UPDATE works SET credits_scanned = 1 WHERE tmdb_id = ?").bind(_));p.length&&await t.DB.batch(p);let u=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE credits_scanned IS NULL OR credits_scanned = 0").first();return new Response(JSON.stringify({ok:!0,worksScanned:a.length,personsFound:l.size,remaining:u?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/backfill-language"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE original_language IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let l=[],m=0;for(let u of a){let _=u.media_type?[u.media_type]:["tv","movie"],g=null;for(let k of _)try{let E=await fetch(`https://api.themoviedb.org/3/${k}/${u.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json();if(w.original_language){g=w.original_language;break}}catch{}g?(l.push(t.DB.prepare("UPDATE works SET original_language = ? WHERE tmdb_id = ?").bind(g,u.tmdb_id)),m++):l.push(t.DB.prepare("UPDATE works SET original_language = 'unknown' WHERE tmdb_id = ?").bind(u.tmdb_id))}l.length&&await t.DB.batch(l);let p=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE original_language IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:m,remaining:p?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/backfill-release-year"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE release_year IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let l=[],m=0;for(let u of a){let _=u.media_type?[u.media_type]:["tv","movie"],g=null;for(let k of _)try{let E=await fetch(`https://api.themoviedb.org/3/${k}/${u.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json(),y=w.release_date||w.first_air_date||"",b=parseInt(y.slice(0,4));if(b){g=b;break}}catch{}g?(l.push(t.DB.prepare("UPDATE works SET release_year = ? WHERE tmdb_id = ?").bind(g,u.tmdb_id)),m++):l.push(t.DB.prepare("UPDATE works SET release_year = 0 WHERE tmdb_id = ?").bind(u.tmdb_id))}l.length&&await t.DB.batch(l);let p=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE release_year IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:m,remaining:p?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/backfill-rating"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE tmdb_rating IS NULL AND rating_updated_at IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let l=[],m=0,p=new Date().toISOString();for(let _ of a){let g=_.media_type?[_.media_type]:["tv","movie"],k=null,E=null,w=!1;for(let y of g)try{let b=await fetch(`https://api.themoviedb.org/3/${y}/${_.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!b.ok)continue;let N=await b.json();w=!0,k=N.vote_average??null,E=N.release_date||N.first_air_date||null;break}catch{}w?(l.push(t.DB.prepare("UPDATE works SET tmdb_rating = ?, release_date = ?, rating_updated_at = ? WHERE tmdb_id = ?").bind(k,E,p,_.tmdb_id)),k!==null&&m++):l.push(t.DB.prepare("UPDATE works SET rating_updated_at = ? WHERE tmdb_id = ?").bind(p,_.tmdb_id))}l.length&&await t.DB.batch(l);let u=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE tmdb_rating IS NULL AND rating_updated_at IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:m,remaining:u?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(d==="/admin/works/batch-imdb-search"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=30;try{let g=await i.json();g?.limit&&Number.isInteger(g.limit)&&g.limit>0&&(e=g.limit)}catch{}let r=t.OMDB_API_KEY;if(!r)return new Response(JSON.stringify({ok:!1,message:"OMDB key not configured"}),{status:500,headers:s});let l=(await t.DB.prepare("SELECT MAX(date) AS latest_date FROM rankings WHERE date != 'manual'").first())?.latest_date||null,{results:m}=await t.DB.prepare(`
        ...
        ORDER BY
          ...
          w.created_at DESC
        LIMIT ?
      `).bind(l,e).all();if(!m.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uB300\uC0C1 \uC791\uD488 \uC5C6\uC74C (\uBAA8\uB450 \uB9E4\uCE6D \uC644\uB8CC\uB410\uAC70\uB098 \uCFE8\uB2E4\uC6B4 \uC911)"}),{headers:s});let p=0,u=new Date().toISOString();for(let g of m)try{if(!g.title_en){await t.DB.prepare("UPDATE works SET imdb_search_attempted_at = ? WHERE tmdb_id = ?").bind(u,g.tmdb_id).run();continue}let k=g.media_type==="movie"?"movie":"series",E=new URLSearchParams({t:g.title_en,type:k,apikey:r});g.release_year&&E.set("y",String(g.release_year));let y=await(await fetch(`https://www.omdbapi.com/?${E.toString()}`)).json();if(y.Response!=="False"&&/^tt\d+$/.test(y.imdbID||"")){let b=parseFloat(y.imdbRating);if(isNaN(b))await t.DB.prepare("UPDATE works SET imdb_id = ?, imdb_search_attempted_at = ? WHERE tmdb_id = ?").bind(y.imdbID,u,g.tmdb_id).run();else{let N=y.imdbVotes||"";await t.DB.prepare("UPDATE works SET imdb_id = ?, imdb_rating = ?, imdb_votes = ?, imdb_updated = ?, imdb_search_attempted_at = ? WHERE tmdb_id = ?").bind(y.imdbID,b,N,u,u,g.tmdb_id).run()}p++}else await t.DB.prepare("UPDATE works SET imdb_search_attempted_at = ? WHERE tmdb_id = ?").bind(u,g.tmdb_id).run()}catch(k){console.error(`[IMDB_BATCH_SEARCH] tmdb_id=${g.tmdb_id} \uC624\uB958:`,k.message)}let _=await t.DB.prepare(`
    ...
    INSERT OR IGNORE INTO title_videos (tmdb_id, youtube_url, youtube_id, title, is_main)
    VALUES (?, ?, ?, ?, 0)
  `).bind(d,`https://www.youtube.com/watch?v=${t}`,t,f||"").run(),console.log(`[CONTENTS_LINK] \u2705 tmdb_id=${d} youtube_id=${t} title_videos \uBCF5\uC0AC \uC644\uB8CC`)}async function St(d,i,t){let f=d.searchParams.get("platform"),s=d.searchParams.get("type"),o=Math.min(parseInt(d.searchParams.get("limit")||"20"),50),n=["is_hidden = 0"],c=[];f&&(n.push("platform = ?"),c.push(f)),s&&(n.push("type = ?"),c.push(s));let e=n.join(" AND ");c.push(o);let{results:r}=await i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
     ...
     LIMIT 5`).all();return A({ok:!0,items:t??[]},200,i)}async function bt(d,i,t){let f=d.searchParams.get("platform"),s=d.searchParams.get("type"),o=Math.max(parseInt(d.searchParams.get("page")||"1"),1),n=30,c=(o-1)*n,e=["is_hidden = 0"],r=[];f&&(e.push("platform = ?"),r.push(f)),s&&(e.push("type = ?"),r.push(s));let a=e.join(" AND "),l=[...r],m=[...r,n,c],[p,u]=await i.DB.batch([i.DB.prepare(`SELECT COUNT(*) as total FROM ott_contents WHERE ${a}`).bind(...l),i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
              tmdb_id, tmdb_type, thumbnail, published_at, view_count
       FROM ott_contents
       WHERE ${a}
       ORDER BY published_at DESC
       LIMIT ? OFFSET ?`).bind(...m)]),_=p.results?.[0]?.total??0,g=u.results??[];return A({ok:!0,items:g,pagination:{page:o,pageSize:n,total:_,totalPages:Math.ceil(_/n)}},200,t)}async function Nt(d,i,t){let f=await i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
            tmdb_id, tmdb_type, thumbnail, published_at, view_count,
     ...
     ORDER BY c.created_at ASC`).bind(d).all();return A({ok:!0,comments:f??[]},200,t)}async function Dt(d,i,t){let f=d.headers.get("Authorization")||"",s=f.startsWith("Bearer ")?f.slice(7).trim():null,o=h(d),n=s||o;if(!n)return A({ok:!1,error:"\uB85C\uADF8\uC778\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."},401,t);let c=await i.DB.prepare(`SELECT s.user_id AS id, u.nickname
     FROM sessions s
     JOIN users u ON u.id = s.user_id
     WHERE s.id = ?
...
2. \uB2E8\uC21C \uC21C\uC704 \uB098\uC5F4\uD615(TOP N) \uC81C\uBAA9\uC740 \uC9C0\uC591\uD558\uACE0 \uD654\uC81C\uC131\uC5D0 \uC9D1\uC911`}};async function z(d,i,t=null){let f=t?`SELECT category_slot, display_name, platform_limit, source_name
       ...
       WHERE platform = ?
         AND is_active = 1
         AND platform_section IS NOT NULL
       ORDER BY platform_order ASC`,s=t?await i.DB.prepare(f).bind(d,t).all():await i.DB.prepare(f).bind(d).all();if(!s.results||s.results.length===0)return[];let o=[];for(let n of s.results){let c=n.platform_limit||10,e=await i.DB.prepare(`SELECT
         r.rank,
       ...
       ORDER BY
         ...
         r.rank ASC
       LIMIT ?`).bind(d,n.category_slot,d,n.category_slot,c).all();e.results&&e.results.length>0&&o.push({category_slot:n.category_slot,display_name:n.display_name,source_name:n.source_name||"",items:e.results})}return o}function Mt(d,i){let f=`[${P[i]||i} \uD604\uC7AC \uB7AD\uD0B9 \uB370\uC774\uD130]

`;return d.forEach(s=>{!s.items||s.items.length===0||(f+=`## ${s.display_name}
`,s.items.forEach((o,n)=>{let c=o.title_ko||o.title_en||"\uC81C\uBAA9 \uC5C6\uC74C",e=o.tmdb_rating?` (\uC624\uB728\uB791 \uD3C9\uC810: ${o.tmdb_rating})`:"",r=o.release_year?` [${o.release_year}\uB144]`:"",a=o.genre?` | \uC7A5\uB974: ${o.genre}`:"";f+=`${n+1}\uC704. ${c}${r}${e}${a}
`}),f+=`
...
`)}async function pt(d,i,t,f,s){if(i.method==="GET"&&d==="/blog-gen/image"){let o=f.searchParams.get("path")||"",n=f.searchParams.get("size")||"w780";if(!o)return new Response(JSON.stringify({ok:!1,error:"path \uD30C\uB77C\uBBF8\uD130 \uD544\uC694"}),{status:400,headers:s});try{let c=`https://image.tmdb.org/t/p/${n}${o}`,e=await fetch(c);if(!e.ok)throw new Error(`\uC774\uBBF8\uC9C0 \uB85C\uB4DC \uC2E4\uD328: ${e.status}`);let r=await e.arrayBuffer(),a=e.headers.get("content-type")||"image/jpeg";return new Response(r,{status:200,headers:{"Content-Type":a,"Access-Control-Allow-Origin":s["Access-Control-Allow-Origin"],"Cache-Control":"public, max-age=86400"}})}catch(c){return new Response(JSON.stringify({ok:!1,error:c.message}),{status:500,headers:s})}}if(i.method==="GET"&&d==="/blog-gen/preview"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,error:"Unauthorized"}),{status:401,headers:s});let o=f.searchParams.get("platform")||"netflix",n=f.searchParams.get("categorySlot")||null;if(!P[o])return new Response(JSON.stringify({ok:!1,error:"\uC9C0\uC6D0\uD558\uC9C0 \uC54A\uB294 \uD50C\uB7AB\uD3FC\uC785\uB2C8\uB2E4."}),{status:400,headers:s});try{let c=await z(o,t,n);return new Response(JSON.stringify({ok:!0,data:c}),{headers:s})}catch(c){return new Response(JSON.stringify({ok:!1,error:c.message}),{status:500,headers:s})}}if(i.method==="POST"&&d==="/blog-gen/suggest"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,error:"Unauthorized"}),{status:401,headers:s});let o=t.ANTHROPIC_API_KEY;if(!o)return new Response(JSON.stringify({ok:!1,error:"ANTHROPIC_API_KEY \uD658\uACBD \uBCC0\uC218\uAC00 \uC124\uC815\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4."}),{status:500,headers:s});let n;try{n=await i.json()}catch{return new Response(JSON.stringify({ok:!1,error:"\uC798\uBABB\uB41C \uC694\uCCAD \uD615\uC2DD\uC785\uB2C8\uB2E4."}),{status:400,headers:s})}let{platform:c="netflix",topicType:e="ranking",categorySlot:r="all"}=n;try{let a=[],l=c==="all"?["netflix","tving"]:[c],m=c!=="all"&&r&&r!=="all"?r:null;for(let T of l){if(T!=="all"&&!P[T])continue;let L=await z(T,t,m);a.push(...L)}let p="";a.length>0?p=a.map(T=>`[${T.display_name}]
...
`):p="\uD604\uC7AC \uB7AD\uD0B9 \uB370\uC774\uD130 \uC5C6\uC74C. OTT \uC778\uAE30 \uCF58\uD150\uCE20 \uC77C\uBC18 \uD2B8\uB80C\uB4DC \uAE30\uBC18\uC73C\uB85C \uCD94\uCC9C\uD574\uC8FC\uC138\uC694.";let u=c==="all"?"\uB137\uD50C\uB9AD\uC2A4\xB7\uD2F0\uBE59":P[c]||c,_=lt(),g=(()=>{if(a.length===1){let T=a[0].display_name||"";if(T.includes("\uC601\uD654"))return"\uC601\uD654";if(T.includes("\uB4DC\uB77C\uB9C8")||T.includes("TV")||T.includes("\uC2DC\uB9AC\uC988"))return"\uB4DC\uB77C\uB9C8"}return"\uB4DC\uB77C\uB9C8\xB7\uC601\uD654"})(),k=dt[e]||dt.ranking,E=k.examples.map(T=>"- "+T.replace(/{platform}/g,u).replace(/{media}/g,g).replace(/{week}/g,_)).join(`
`),w=k.rule.replace(/{platform}/g,u).replace(/{week}/g,_),y=`\uB2F9\uC2E0\uC740 \uB124\uC774\uBC84 \uBE14\uB85C\uADF8 SEO \uC804\uBB38\uAC00\uC785\uB2C8\uB2E4.
...
\uB9C8\uD06C\uB2E4\uC6B4 \uAE30\uD638 \uC5C6\uC774 \uC77C\uBC18 \uD14D\uC2A4\uD2B8\uB85C, \uB2E8\uB77D \uAD6C\uBD84\uC740 \uBE48 \uC904\uB85C\uB9CC \uD574\uC8FC\uC138\uC694.`,L=await Ht(T,o,{useWebSearch:R,maxTokens:R?5e3:4096});if(!L)throw new Error("AI \uC751\uB2F5\uC774 \uBE44\uC5B4\uC788\uC2B5\uB2C8\uB2E4. \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694.");return new Response(JSON.stringify({ok:!0,post:L,rankingData:E,meta:{platform:c,platformName:b,weekInfo:y,categorySlot:k||"all",categoryLabel:k&&E.length===1?E[0].display_name:"\uC804\uCCB4",generatedAt:new Date().toISOString()}}),{headers:s})}catch(E){return new Response(JSON.stringify({ok:!1,error:E.message}),{status:500,headers:s})}}return null}var Ft=["ad","bug"],Wt=["pending","answered","resolved"],Ut=5,mt=30;async function _t(d,i,t,f,s,o){if(d==="/inquiry"&&i.method==="POST")try{let e=await i.json(),{type:r,name:a,email:l,phone:m,title:p,content:u,page_url:_,website:g}=e;if(g)return new Response(JSON.stringify({ok:!0}),{headers:o});if(!Ft.includes(r))return new Response(JSON.stringify({ok:!1,message:"type\uC740 ad \uB610\uB294 bug\uC5EC\uC57C \uD569\uB2C8\uB2E4"}),{status:400,headers:o});if(!p||!p.trim()||!u||!u.trim())return new Response(JSON.stringify({ok:!1,message:"\uC81C\uBAA9\uACFC \uB0B4\uC6A9\uC740 \uD544\uC218\uC785\uB2C8\uB2E4"}),{status:400,headers:o});if(r==="ad"){if(!a||!a.trim())return new Response(JSON.stringify({ok:!1,message:"\uB2F4\uB2F9\uC790\uBA85 \uB610\uB294 \uC5C5\uCCB4\uBA85\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});if(!l||!l.trim())return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o})}if(l&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(l))return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C \uD615\uC2DD\uC774 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4"}),{status:400,headers:o});let k=r,E=String(p).slice(0,200),w=String(u).slice(0,5e3),y=a?String(a).slice(0,100):null,b=l?String(l).slice(0,200):null,N=m?String(m).slice(0,30):null,R=_?String(_).slice(0,500):null,D=i.headers.get("User-Agent")||null,S=i.headers.get("CF-Connecting-IP")||null,T=null;try{let L=i.headers.get("Authorization")||"",I=(L.startsWith("Bearer ")?L.slice(7).trim():null)||h(i);if(I){let B=await t.DB.prepare("SELECT user_id AS id FROM sessions WHERE id = ? LIMIT 1").bind(I).first();B&&(T=B.id)}}catch{}return S&&((await t.DB.prepare(`SELECT COUNT(*) as cnt FROM inquiries
    ...
    `,{results:c}=await i.DB.prepare(n).bind(o).all(),{results:e}=await i.DB.prepare("SELECT tmdb_id, boost_value, is_pinned, pinned_score, pinned_platform FROM admin_boosts").all(),r=new Map((e||[]).map(u=>[u.tmdb_id,u]));if((!c||c.length===0)&&r.size===0)return new Response(JSON.stringify({ok:!1,error:"\uACC4\uC0B0\uD560 \uB7AD\uD0B9 \uB370\uC774\uD130\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4."}),{status:404,headers:t});let a=[],l=new Set;for(let u of c||[]){l.add(u.tmdb_id);let _=r.get(u.tmdb_id);_&&_.is_pinned?a.push({tmdb_id:u.tmdb_id,best_platform:_.pinned_platform||u.best_platform,best_rank:u.best_rank,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:_.pinned_score??0}):a.push(u)}for(let[u,_]of r)l.has(u)||(_.is_pinned?a.push({tmdb_id:u,best_platform:_.pinned_platform||"manual",best_rank:null,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:_.pinned_score??0}):_.boost_value&&a.push({tmdb_id:u,best_platform:_.pinned_platform||"manual",best_rank:null,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:_.boost_value}));a.sort((u,_)=>_.weighted_score+_.admin_boost-(u.weighted_score+u.admin_boost));let m=new Date(Date.now()+540*60*1e3).toISOString().slice(0,19).replace("T"," "),p=[i.DB.prepare("DELETE FROM hot100_scores")];for(let u of a){let _=u.weighted_score+u.admin_boost;p.push(i.DB.prepare(`INSERT INTO hot100_scores
