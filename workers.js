function $(d,i,t){if(!i.length)return d.slice(0,t).map((r,a)=>({...r,rank:a+1}));let g=new Set(i.map(r=>r.tmdb_id).filter(Boolean)),e=d.filter(r=>!g.has(r.tmdb_id)),o={};for(let r of i){let a=Math.max(1,parseInt(r.rank)||1);o[a]||(o[a]=[]),o[a].push(r)}let n=[],c=0,s=1;for(;n.length<t;){if(o[s]&&o[s].length){let r=o[s].shift();n.push({...r,rank:n.length+1})}else if(c<e.length)n.push({...e[c],rank:n.length+1}),c++;else{let r=Object.values(o).flat();for(let a of r){if(n.length>=t)break;n.push({...a,rank:n.length+1})}break}s++}return n}async function G(d,i,t,g,e){if(d==="/rankings"&&i.method==="GET"){let o=g.searchParams.get("platform"),n=g.searchParams.get("category"),c=g.searchParams.get("date"),s="SELECT * FROM rankings WHERE 1=1",r=[];o&&(s+=" AND platform = ?",r.push(o)),n&&(s+=" AND category = ?",r.push(n)),c?(s+=" AND date = ?",r.push(c)):s+=" AND date = (SELECT MAX(date) FROM rankings)",s+=" ORDER BY platform, category, rank";let{results:a}=await t.DB.prepare(s).bind(...r).all();return new Response(JSON.stringify({ok:!0,data:a}),{headers:e})}if(d==="/rankings/main"&&i.method==="GET")try{let o=g.searchParams.get("date")||null,{results:n}=await t.DB.prepare(`
      ...
      `).bind(o,n).all(),{results:s}=await t.DB.prepare(`
        ...
        WHERE r.platform = ?
          ...
          AND r.date = 'manual'
        ORDER BY oc.platform_order, r.rank
      `).bind(o).all(),r={},a={},l={};for(let u of c){let m=u.category_slot;r[m]||(r[m]=[]),l[m]||(l[m]=u),r[m].push(u)}for(let u of s){let m=u.category_slot;a[m]||(a[m]=[]),l[m]||(l[m]=u),a[m].push(u)}let _={},p=new Set([...Object.keys(r),...Object.keys(a)]);for(let u of p){let m=l[u];if(!m)continue;let k=m.platform_limit||20,E=$((r[u]||[]).sort((w,y)=>w.rank-y.rank),(a[u]||[]).sort((w,y)=>w.rank-y.rank),k);_[u]={platform:m.platform,category_slot:m.category_slot,display_name:m.display_name,platform_order:m.platform_order,memo_label:m.memo_label||null,items:E.map(w=>({rank:w.rank,title_ko:w.title_ko,title_en:w.title_en,tmdb_id:w.tmdb_id,poster_path:w.poster_path,genre:w.genre,tmdb_rating:w.tmdb_rating,release_year:w.release_year,memo:w.memo||null}))}}let f=Object.values(_).sort((u,m)=>u.platform_order-m.platform_order);return new Response(JSON.stringify({ok:!0,data:f}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}if(d==="/rankings/weekly"&&i.method==="GET")try{let{results:o}=await t.DB.prepare(`
        SELECT
  ...
  </url>`);for(let _ of s){let p=`${o}/title/1-${n}${_.tmdb_id}`;a.push(`  <url>
    <loc>${p}</loc>
...
`),p=(await(await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":g.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:4e3,messages:[{role:"user",content:a}]})})).json()).content?.[0]?.text||"[]",f=[];try{let u=p.split("```json").join("").split("```").join("").trim(),m=JSON.parse(u);f=Array.isArray(m)?m:[]}catch{console.error("[REACTION] Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328:",p.slice(0,300)),f=[]}await g.DB.prepare("DELETE FROM reaction_comments WHERE reaction_id = ?").bind(d).run();for(let u=0;u<s.length;u++){let m=s[u],E=(f.find(w=>w.idx===u)||f.find(w=>w.idx===u+1)||f[u]||{}).translated||"";await g.DB.prepare(`
      ...
      `).bind(n).all(),{results:_}=await t.DB.prepare("SELECT * FROM pick_lists WHERE user_id = ? AND is_public = 1 ORDER BY created_at DESC").bind(n).all(),p=await Promise.all(_.map(async f=>{let{results:u}=await t.DB.prepare("SELECT * FROM pick_list_works WHERE pick_list_id = ? ORDER BY sort_order ASC, created_at DESC").bind(f.id).all();return{...f,works:u,work_count:u.length}}));return new Response(JSON.stringify({ok:!0,is_own:!1,user:c,reviews:s,wishlist:r,wishlist_hidden:!c.wishlist_public,posts:a,life_works:l,pick_lists:p,stats:{review_count:s.length,wishlist_count:c.wishlist_public?r.length:null,likes_received:c.total_likes_received||0,post_count:a.length,life_work_count:l.length,pick_list_count:p.length}}),{headers:e})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:e})}if(d==="/mypage/reviews"&&i.method==="GET")try{let c=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!c)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:e});let s=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(c).first();if(!s)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:e});let r=await t.DB.prepare("SELECT nickname FROM users WHERE id = ?").bind(s.user_id).first(),{results:a}=await t.DB.prepare(`
        ...
        WHERE r.user_id = ?
        ORDER BY r.created_at DESC
      `).bind(s.user_id,s.user_id).all();return new Response(JSON.stringify({ok:!0,reviews:a,nickname:r?.nickname||"\uB098"}),{headers:e})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:e})}if(d.match(/^\/user\/\d+\/reviews$/)&&i.method==="GET")try{let n=parseInt(d.split("/")[2]),c=await t.DB.prepare("SELECT nickname FROM users WHERE id = ?").bind(n).first();if(!c)return new Response(JSON.stringify({ok:!1,message:"\uC720\uC800\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC5B4\uC694"}),{status:404,headers:e});let r=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i),a=-1;if(r){let _=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(r).first();_&&(a=_.user_id)}let{results:l}=await t.DB.prepare(`
        SELECT r.id, r.tmdb_id, r.score, r.text, r.emotions, r.custom_tags,
          r.likes, r.spoiler, r.created_at,
          COALESCE(rk.title_ko,  wk.title_ko)    as title_ko,
          COALESCE(rk.poster_path, wk.poster_path) as poster_path,
      ...
      `).bind(parseInt(a),s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('queue_resolve', ?, ?, ?, ?)").bind(p.platform,p.category_slot,String(a),JSON.stringify({tmdb_id:a,title_ko:u,title_en:m})).run(),new Response(JSON.stringify({ok:!0,poster_path:f,title_ko:u,title_en:m}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/rank-override"&&i.method==="POST"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),{platform:r,category_slot:a,date:l,tmdb_id:_,original_rank:p,override_rank:f,reason:u}=s;return!r||!a||!l||!_||!f?new Response(JSON.stringify({ok:!1,message:"\uD544\uC218 \uD30C\uB77C\uBBF8\uD130 \uB204\uB77D"}),{status:400,headers:e}):(await t.DB.prepare(`
        ...
        ON CONFLICT(platform, category_slot, date, tmdb_id) DO UPDATE SET
          ...
          updated_at    = datetime('now')
      `).bind(r,a,l,parseInt(_),p||0,parseInt(f),u||null).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, before_value, after_value) VALUES ('rank_override', ?, ?, ?, ?, ?)").bind(r,a,String(_),JSON.stringify({rank:p}),JSON.stringify({rank:f,reason:u})).run(),new Response(JSON.stringify({ok:!0}),{headers:e}))}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/rank-override"&&i.method==="DELETE"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),{platform:r,category_slot:a,date:l,tmdb_id:_}=s;return await t.DB.prepare("DELETE FROM rank_overrides WHERE platform = ? AND category_slot = ? AND date = ? AND tmdb_id = ?").bind(r,a,l,parseInt(_)).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d.match(/^\/admin\/works\/\d+$/)&&i.method==="GET"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(d.split("/")[3]),r=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(s).first();return r?new Response(JSON.stringify({ok:!0,data:r}),{headers:e}):new Response(JSON.stringify({ok:!1,message:"Not found"}),{status:404,headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/works"&&i.method==="GET"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=g.searchParams.get("q")||"",r=g.searchParams.get("filter")||"",a=g.searchParams.get("date")||"",l=g.searchParams.get("sort")||"recent",_=parseInt(g.searchParams.get("page")||"1"),p=50,f=(_-1)*p,u=l==="updated"?"updated_at DESC, id DESC":"COALESCE(created_at, updated_at) DESC, id DESC",m,k;r==="new_match"&&a?(m=`SELECT * FROM works WHERE first_matched_date = ? AND match_source IN ('auto_claude', 'auto_tmdb') ORDER BY ${u} LIMIT ? OFFSET ?`,k=[a,p,f]):s?(m=`SELECT * FROM works WHERE title_ko LIKE ? OR title_en LIKE ? ORDER BY ${u} LIMIT ? OFFSET ?`,k=[`%${s}%`,`%${s}%`,p,f]):(m=`SELECT * FROM works ORDER BY ${u} LIMIT ? OFFSET ?`,k=[p,f]);let{results:E}=await t.DB.prepare(m).bind(...k).all();return new Response(JSON.stringify({ok:!0,data:E}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d.match(/^\/admin\/works\/\d+$/)&&i.method==="PATCH"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(d.split("/")[3]),r=await i.json(),{title_ko:a,title_en:l,poster_path:_,delete_duplicates:p,media_type:f,mbti_tags:u}=r,m=f==="tv"||f==="movie"?f:null,k=u!==void 0,E=k?u||null:void 0,w=await t.DB.prepare("SELECT title_ko, title_en, poster_path, media_type FROM works WHERE tmdb_id = ?").bind(s).first();if(p&&(l||w?.title_en)){let y=l||w?.title_en;await t.DB.prepare("DELETE FROM works WHERE title_en = ? AND tmdb_id != ?").bind(y,s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value, memo) VALUES ('works_delete', ?, ?, ?)").bind(String(s),JSON.stringify({title_en:y}),`\uC911\uBCF5 \uC0AD\uC81C: title_en="${y}" tmdb_id!=${s}`).run()}return await t.DB.prepare(`
        UPDATE works SET
          title_ko         = COALESCE(?, title_ko),
          title_en         = COALESCE(?, title_en),
          poster_path      = COALESCE(?, poster_path),
...
${l}`,f=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":t.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:3e3,system:_,messages:[{role:"user",content:p}]})});if(!f.ok){let R=await f.text().catch(()=>"");return new Response(JSON.stringify({ok:!1,message:`Claude API \uC624\uB958 (status ${f.status})`,detail:R.slice(0,300)}),{status:502,headers:e})}let m=((await f.json()).content||[]).filter(R=>R.type==="text").map(R=>R.text).join(""),k;try{let R=m.replace(/```json|```/g,"").trim();k=JSON.parse(R)}catch{return new Response(JSON.stringify({ok:!1,message:"Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694",raw:m.slice(0,300)}),{status:502,headers:e})}Array.isArray(k)||(k=[]);let E=new Set(a.map(R=>R.keyword_en)),w=new Map;for(let R of k){let D=(R.keyword_en||"").trim().toLowerCase(),S=(R.keyword_ko||"").trim();!D||!S||!E.has(D)||w.set(D,S)}let y=[],O=0;for(let R of a){if(!w.has(R.keyword_en)){y.push(t.DB.prepare("UPDATE keyword_translation SET translate_attempts = COALESCE(translate_attempts, 0) + 1 WHERE keyword_en = ? AND source IS NULL").bind(R.keyword_en));continue}y.push(t.DB.prepare("UPDATE keyword_translation SET keyword_ko = ?, source = 'auto' WHERE keyword_en = ? AND source IS NULL").bind(w.get(R.keyword_en),R.keyword_en)),O++}y.length&&await t.DB.batch(y);let N=await t.DB.prepare(`
      ...
      `).bind(r,r).all();return new Response(JSON.stringify({ok:!0,items:a}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/keywords/update"&&i.method==="POST"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),r=(s.keyword_en||"").trim(),a=(s.keyword_ko||"").trim();if(!r||!a)return new Response(JSON.stringify({ok:!1,message:"keyword_en, keyword_ko \uBAA8\uB450 \uD544\uC694\uD574\uC694"}),{status:400,headers:e});let l=await t.DB.prepare("UPDATE keyword_translation SET keyword_ko = ?, source = 'admin' WHERE keyword_en = ?").bind(a,r).run();return!l.meta||l.meta.changes===0?new Response(JSON.stringify({ok:!1,message:"\uD574\uB2F9 keyword_en\uC744 \uCC3E\uC9C0 \uBABB\uD588\uC5B4\uC694"}),{status:404,headers:e}):new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/works/discover-collect"&&i.method==="POST"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),r=s.media_type,a=Math.max(parseInt(s.page)||1,1);if(!["movie","tv"].includes(r))return new Response(JSON.stringify({ok:!1,message:"media_type\uC740 'movie' \uB610\uB294 'tv'\uB9CC \uD5C8\uC6A9"}),{status:400,headers:e});let l=r==="movie"?`https://api.themoviedb.org/3/discover/movie?api_key=${t.TMDB_API_KEY}&language=ko-KR&region=KR&with_original_language=ko&sort_by=popularity.desc&page=${a}`:`https://api.themoviedb.org/3/discover/tv?api_key=${t.TMDB_API_KEY}&language=ko-KR&with_origin_country=KR&sort_by=popularity.desc&page=${a}`,_=await fetch(l);if(!_.ok)return new Response(JSON.stringify({ok:!1,message:`TMDB discover \uC870\uD68C \uC2E4\uD328 (status ${_.status})`}),{status:502,headers:e});let p=await _.json(),f=p.results||[],u=p.total_pages||1;if(!f.length)return new Response(JSON.stringify({ok:!0,attempted:0,inserted:0,skipped:0,hasNextPage:!1,nextPage:a+1,totalPages:u}),{headers:e});let m=f.map(R=>R.id),k=m.map(()=>"?").join(","),{results:E}=await t.DB.prepare(`SELECT tmdb_id FROM works WHERE tmdb_id IN (${k})`).bind(...m).all(),w=new Set((E||[]).map(R=>R.tmdb_id)),y=f.filter(R=>!w.has(R.id)),O=[],N=0;for(let R of y){let D=null,S=null,T=null,L=null,C=null,I=null,B="";try{let J=await fetch(`https://api.themoviedb.org/3/${r}/${R.id}?language=ko-KR&api_key=${t.TMDB_API_KEY}`);if(J.ok){let M=await J.json();D=M.name||M.title||R.name||R.title||null,T=M.poster_path||R.poster_path||null,L=(M.genres||[]).map(W=>W.name).join(", ")||null,C=M.vote_average?parseFloat(M.vote_average.toFixed(1)):null,I=parseInt((M.first_air_date||M.release_date||"").slice(0,4))||null,B=M.overview||R.overview||""}}catch{}if(D){try{let J=await fetch(`https://api.themoviedb.org/3/${r}/${R.id}?language=en-US&api_key=${t.TMDB_API_KEY}`);if(J.ok){let M=await J.json(),W=M.original_title||M.original_name||"",K=M.title||M.name||"";S=/[\uAC00-\uD7A3]/.test(W)?K:W||K}}catch{}O.push(t.DB.prepare(`
        ...
        LIMIT ?
      `).bind(r).all();if(!_.length)return new Response(JSON.stringify({ok:!0,attempted:0,classified:0,remaining:0,message:"\uBD84\uB958\uD560 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let p=_.map(S=>`- tmdb_id:${S.tmdb_id} / \uC81C\uBAA9:"${S.title_ko||""}" / \uC904\uAC70\uB9AC:"${(S.overview||"").slice(0,200)}"`).join(`
...
${p}`,m=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":t.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:2e3,system:f,messages:[{role:"user",content:u}]})});if(!m.ok){let S=await m.text().catch(()=>"");return new Response(JSON.stringify({ok:!1,message:`Claude API \uC624\uB958 (status ${m.status})`,detail:S.slice(0,300)}),{status:502,headers:e})}let E=((await m.json()).content||[]).filter(S=>S.type==="text").map(S=>S.text).join(""),w;try{let S=E.replace(/```json|```/g,"").trim();w=JSON.parse(S)}catch{return new Response(JSON.stringify({ok:!1,message:"Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694",raw:E.slice(0,300)}),{status:502,headers:e})}Array.isArray(w)||(w=[]);let y=new Set(l),O=new Map;for(let S of w){let T=parseInt(S.tmdb_id);if(!T)continue;let L=Array.isArray(S.tags)?S.tags.filter(C=>y.has(C)).slice(0,2):[];O.set(T,L)}let N=[],R=0;for(let S of _){if(!O.has(S.tmdb_id))continue;let T=O.get(S.tmdb_id);N.push(t.DB.prepare("UPDATE works SET variety_genre = ?, variety_genre_source = 'auto' WHERE tmdb_id = ?").bind(T.length?T.join(","):null,S.tmdb_id)),R++}N.length&&await t.DB.batch(N);let D=await t.DB.prepare(`
      ...
      `).bind(r,a,a,r).run(),new Response(JSON.stringify({ok:!0}),{headers:e}))}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/persons/collect"&&i.method==="POST"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),r=Math.min(parseInt(s.limit)||20,50),{results:a}=await t.DB.prepare(`
        ...
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,worksScanned:0,personsFound:0,remaining:0,message:"\uC2A4\uCE94\uD560 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let l=new Map,_=[];for(let u of a){_.push(u.tmdb_id);let m=u.media_type==="tv"?"tv":"movie",k=m==="tv"?"aggregate_credits":"credits";try{let E=await fetch(`https://api.themoviedb.org/3/${m}/${u.tmdb_id}/${k}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json();for(let y of(w.cast||[]).slice(0,15))y.id&&y.name&&!l.has(y.id)&&l.set(y.id,{name:y.name,job:"act"});for(let y of w.crew||[])(y.job==="Director"||y.job==="Creator"||y.department==="Directing"||(y.jobs||[]).some(N=>N.job==="Director"||N.job==="Creator"))&&y.id&&y.name&&l.set(y.id,{name:y.name,job:"direct"})}catch{}}let p=[];for(let[u,m]of l)p.push(t.DB.prepare(`INSERT INTO persons (tmdb_id, name, job) VALUES (?, ?, ?)
             ON CONFLICT(tmdb_id) DO NOTHING`).bind(u,m.name,m.job));for(let u of _)p.push(t.DB.prepare("UPDATE works SET credits_scanned = 1 WHERE tmdb_id = ?").bind(u));p.length&&await t.DB.batch(p);let f=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE credits_scanned IS NULL OR credits_scanned = 0").first();return new Response(JSON.stringify({ok:!0,worksScanned:a.length,personsFound:l.size,remaining:f?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/works/backfill-language"&&i.method==="POST"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),r=Math.min(parseInt(s.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE original_language IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let l=[],_=0;for(let f of a){let u=f.media_type?[f.media_type]:["tv","movie"],m=null;for(let k of u)try{let E=await fetch(`https://api.themoviedb.org/3/${k}/${f.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json();if(w.original_language){m=w.original_language;break}}catch{}m?(l.push(t.DB.prepare("UPDATE works SET original_language = ? WHERE tmdb_id = ?").bind(m,f.tmdb_id)),_++):l.push(t.DB.prepare("UPDATE works SET original_language = 'unknown' WHERE tmdb_id = ?").bind(f.tmdb_id))}l.length&&await t.DB.batch(l);let p=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE original_language IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:_,remaining:p?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/works/backfill-release-year"&&i.method==="POST"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),r=Math.min(parseInt(s.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE release_year IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let l=[],_=0;for(let f of a){let u=f.media_type?[f.media_type]:["tv","movie"],m=null;for(let k of u)try{let E=await fetch(`https://api.themoviedb.org/3/${k}/${f.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json(),y=w.release_date||w.first_air_date||"",O=parseInt(y.slice(0,4));if(O){m=O;break}}catch{}m?(l.push(t.DB.prepare("UPDATE works SET release_year = ? WHERE tmdb_id = ?").bind(m,f.tmdb_id)),_++):l.push(t.DB.prepare("UPDATE works SET release_year = 0 WHERE tmdb_id = ?").bind(f.tmdb_id))}l.length&&await t.DB.batch(l);let p=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE release_year IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:_,remaining:p?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/works/backfill-rating"&&i.method==="POST"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),r=Math.min(parseInt(s.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE tmdb_rating IS NULL AND rating_updated_at IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let l=[],_=0,p=new Date().toISOString();for(let u of a){let m=u.media_type?[u.media_type]:["tv","movie"],k=null,E=null,w=!1;for(let y of m)try{let O=await fetch(`https://api.themoviedb.org/3/${y}/${u.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!O.ok)continue;let N=await O.json();w=!0,k=N.vote_average??null,E=N.release_date||N.first_air_date||null;break}catch{}w?(l.push(t.DB.prepare("UPDATE works SET tmdb_rating = ?, release_date = ?, rating_updated_at = ? WHERE tmdb_id = ?").bind(k,E,p,u.tmdb_id)),k!==null&&_++):l.push(t.DB.prepare("UPDATE works SET rating_updated_at = ? WHERE tmdb_id = ?").bind(p,u.tmdb_id))}l.length&&await t.DB.batch(l);let f=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE tmdb_rating IS NULL AND rating_updated_at IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:_,remaining:f?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(d==="/admin/works/batch-imdb-search"&&i.method==="POST"){if(!b(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=30;try{let m=await i.json();m?.limit&&Number.isInteger(m.limit)&&m.limit>0&&(s=m.limit)}catch{}let r=t.OMDB_API_KEY;if(!r)return new Response(JSON.stringify({ok:!1,message:"OMDB key not configured"}),{status:500,headers:e});let l=(await t.DB.prepare("SELECT MAX(date) AS latest_date FROM rankings WHERE date != 'manual'").first())?.latest_date||null,{results:_}=await t.DB.prepare(`
  ...
  `).bind(d,`https://www.youtube.com/watch?v=${t}`,t,g||"").run(),console.log(`[CONTENTS_LINK] \u2705 tmdb_id=${d} youtube_id=${t} title_videos \uBCF5\uC0AC \uC644\uB8CC`)}async function Nt(d,i,t){let g=d.searchParams.get("platform"),e=d.searchParams.get("type"),o=Math.min(parseInt(d.searchParams.get("limit")||"20"),50),n=["is_hidden = 0"],c=[];g&&(n.push("platform = ?"),c.push(g)),e&&(n.push("type = ?"),c.push(e));let s=n.join(" AND ");c.push(o);let{results:r}=await i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
     ...
     LIMIT 5`).all();return A({ok:!0,items:t??[]},200,i)}async function Dt(d,i,t){let g=d.searchParams.get("platform"),e=d.searchParams.get("type"),o=Math.max(parseInt(d.searchParams.get("page")||"1"),1),n=30,c=(o-1)*n,s=["is_hidden = 0"],r=[];g&&(s.push("platform = ?"),r.push(g)),e&&(s.push("type = ?"),r.push(e));let a=s.join(" AND "),l=[...r],_=[...r,n,c],[p,f]=await i.DB.batch([i.DB.prepare(`SELECT COUNT(*) as total FROM ott_contents WHERE ${a}`).bind(...l),i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
              tmdb_id, tmdb_type, thumbnail, published_at, view_count
       FROM ott_contents
       WHERE ${a}
       ORDER BY published_at DESC
       LIMIT ? OFFSET ?`).bind(..._)]),u=p.results?.[0]?.total??0,m=f.results??[];return A({ok:!0,items:m,pagination:{page:o,pageSize:n,total:u,totalPages:Math.ceil(u/n)}},200,t)}async function ht(d,i,t){let g=await i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
            tmdb_id, tmdb_type, thumbnail, published_at, view_count,
...
\uB9C8\uD06C\uB2E4\uC6B4 \uAE30\uD638 \uC5C6\uC774 \uC77C\uBC18 \uD14D\uC2A4\uD2B8\uB85C, \uB2E8\uB77D \uAD6C\uBD84\uC740 \uBE48 \uC904\uB85C\uB9CC \uD574\uC8FC\uC138\uC694.`,L=await Ut(T,o,{useWebSearch:R,maxTokens:R?5e3:4096});if(!L)throw new Error("AI \uC751\uB2F5\uC774 \uBE44\uC5B4\uC788\uC2B5\uB2C8\uB2E4. \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694.");return new Response(JSON.stringify({ok:!0,post:L,rankingData:E,meta:{platform:c,platformName:O,weekInfo:y,categorySlot:k||"all",categoryLabel:k&&E.length===1?E[0].display_name:"\uC804\uCCB4",generatedAt:new Date().toISOString()}}),{headers:e})}catch(E){return new Response(JSON.stringify({ok:!1,error:E.message}),{status:500,headers:e})}}return null}var Pt=["ad","bug"],$t=["pending","answered","resolved"],xt=5,_t=30;async function mt(d,i,t,g,e,o){if(d==="/inquiry"&&i.method==="POST")try{let s=await i.json(),{type:r,name:a,email:l,phone:_,title:p,content:f,page_url:u,website:m}=s;if(m)return new Response(JSON.stringify({ok:!0}),{headers:o});if(!Pt.includes(r))return new Response(JSON.stringify({ok:!1,message:"type\uC740 ad \uB610\uB294 bug\uC5EC\uC57C \uD569\uB2C8\uB2E4"}),{status:400,headers:o});if(!p||!p.trim()||!f||!f.trim())return new Response(JSON.stringify({ok:!1,message:"\uC81C\uBAA9\uACFC \uB0B4\uC6A9\uC740 \uD544\uC218\uC785\uB2C8\uB2E4"}),{status:400,headers:o});if(r==="ad"){if(!a||!a.trim())return new Response(JSON.stringify({ok:!1,message:"\uB2F4\uB2F9\uC790\uBA85 \uB610\uB294 \uC5C5\uCCB4\uBA85\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});if(!l||!l.trim())return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o})}if(l&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(l))return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C \uD615\uC2DD\uC774 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4"}),{status:400,headers:o});let k=r,E=String(p).slice(0,200),w=String(f).slice(0,5e3),y=a?String(a).slice(0,100):null,O=l?String(l).slice(0,200):null,N=_?String(_).slice(0,30):null,R=u?String(u).slice(0,500):null,D=i.headers.get("User-Agent")||null,S=i.headers.get("CF-Connecting-IP")||null,T=null;try{let L=i.headers.get("Authorization")||"",I=(L.startsWith("Bearer ")?L.slice(7).trim():null)||h(i);if(I){let B=await t.DB.prepare("SELECT user_id AS id FROM sessions WHERE id = ? LIMIT 1").bind(I).first();B&&(T=B.id)}}catch{}return S&&((await t.DB.prepare(`SELECT COUNT(*) as cnt FROM inquiries
    ...
    `,{results:c}=await i.DB.prepare(n).bind(o).all();return!c||c.length===0?new Response(JSON.stringify({ok:!0,data:[]}),{status:200,headers:t}):new Response(JSON.stringify({ok:!0,data:c.map((s,r)=>({hot_rank:r+1,...s}))}),{status:200,headers:t})}catch(g){return console.error("getHot100 \uC624\uB958:",g),new Response(JSON.stringify({ok:!1,error:"HOT100 \uC870\uD68C \uC911 \uC624\uB958\uAC00 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4.",detail:g.message}),{status:500,headers:t})}}async function yt(d,i,t){if(!await b(d,i))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:t});try{let{results:e}=await i.DB.prepare(`SELECT platform, category_slot, top_n, display_order, is_active
       ...
       FROM hot100_frontend_tabs
       WHERE is_active = 1
       ORDER BY display_order ASC`).all();if(!e||e.length===0)return new Response(JSON.stringify({ok:!0,tabs:[]}),{status:200,headers:t});let o=[];for(let n of e){let c=n.top_n||10;if(n.platform==="all"){let{results:p}=await i.DB.prepare(`SELECT h.tmdb_id, h.best_platform, w.title_ko, w.title_en,
                  w.poster_path, w.hero_backdrop_path, w.media_type, w.tmdb_rating
           FROM hot100_scores h
           LEFT JOIN works w ON w.tmdb_id = h.tmdb_id
           ORDER BY h.total_score DESC
           LIMIT ?`).bind(c).all();o.push({platform:"all",label:g.all,items:(p||[]).map((f,u)=>({rank:u+1,tmdb_id:f.tmdb_id,best_platform:f.best_platform,title_ko:f.title_ko,title_en:f.title_en,poster_path:f.poster_path,hero_backdrop_path:f.hero_backdrop_path,media_type:f.media_type,tmdb_rating:f.tmdb_rating}))});continue}if(!n.category_slot)continue;let r=(await i.DB.prepare("SELECT MAX(date) AS latest FROM rankings WHERE platform = ? AND category_slot = ? AND date != 'manual'").bind(n.platform,n.category_slot).first())?.latest||null,{results:a}=r?await i.DB.prepare(`SELECT r.rank, r.tmdb_id, r.title_ko, r.title_en, r.poster_path,
