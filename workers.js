    ...
    `).bind(e.platform,e.category_slot,e.platform,e.category_slot));return(await r.DB.batch(t)).flatMap(e=>e.results||[])}async function Q(r,i,t,f,e){if(r==="/rankings"&&i.method==="GET"){let o=f.searchParams.get("platform"),n=f.searchParams.get("category"),_=f.searchParams.get("date"),c="SELECT * FROM rankings WHERE 1=1",d=[];o&&(c+=" AND platform = ?",d.push(o)),n&&(c+=" AND category = ?",d.push(n)),_?(c+=" AND date = ?",d.push(_)):c+=" AND date = (SELECT value FROM app_settings WHERE key = 'latest_ranking_date')",c+=" ORDER BY platform, category, rank";let{results:s}=await t.DB.prepare(c).bind(...d).all();return new Response(JSON.stringify({ok:!0,data:s}),{headers:e})}if(r==="/rankings/main"&&i.method==="GET")try{let o=f.searchParams.get("date")||null,{results:n}=await t.DB.prepare(`
      ...
      `).bind(o).all(),{results:d}=await t.DB.prepare(`
        ...
        WHERE oc.main_section IS NOT NULL
          ...
          AND r.date = 'manual'
        ORDER BY oc.main_section, oc.main_order, r.rank
      `).all(),s={},a={};for(let y of c){let k=`${y.platform}__${y.category_slot}`;s[k]||(s[k]=[]),s[k].push(y)}for(let y of d){let k=`${y.platform}__${y.category_slot}`;a[k]||(a[k]=[]),a[k].push(y)}if(!o){let y=n.filter(k=>!s[`${k.platform}__${k.category_slot}`]);if(y.length){let k=await X(t,y);for(let S of k){let R=`${S.platform}__${S.category_slot}`;s[R]||(s[R]=[]),s[R].push(S)}}}let l={},p={},u={},g=new Set([...Object.keys(s),...Object.keys(a)]);for(let y of g){let k=_[y];if(!k)continue;let S=k.main_limit||10,R=j((s[y]||[]).sort((b,N)=>b.rank-N.rank),(a[y]||[]).sort((b,N)=>b.rank-N.rank),S);for(let b of R){let N={rank:b.rank,title_ko:b.title_ko,title_en:b.title_en,tmdb_id:b.tmdb_id,poster_path:b.poster_path,genre:b.genre,tmdb_rating:b.tmdb_rating,release_year:b.release_year,memo:b.memo||null,display_name:k.display_name,platform:k.platform,category_slot:k.category_slot,main_order:k.main_order};k.main_section==="tv"?(l[y]||(l[y]={platform:k.platform,category_slot:k.category_slot,display_name:k.display_name,main_order:k.main_order,memo_label:k.memo_label||null,items:[]}),l[y].items.push(N)):k.main_section==="movie"?(p[y]||(p[y]={platform:k.platform,category_slot:k.category_slot,display_name:k.display_name,main_order:k.main_order,memo_label:k.memo_label||null,items:[]}),p[y].items.push(N)):k.main_section==="featured"&&k.platform==="netflix"&&(u[y]||(u[y]={platform:k.platform,category_slot:k.category_slot,display_name:k.display_name,main_order:k.main_order,memo_label:k.memo_label||null,items:[]}),u[y].items.push(N))}}let m=Object.values(l).sort((y,k)=>y.main_order-k.main_order),E=Object.values(p).sort((y,k)=>y.main_order-k.main_order),w=Object.values(u).sort((y,k)=>y.main_order-k.main_order).slice(0,2);return new Response(JSON.stringify({ok:!0,tv:m,movie:E,featured:w}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}if(r==="/rankings/platform"&&i.method==="GET")try{let o=f.searchParams.get("platform"),n=f.searchParams.get("date")||null;if(!o)return new Response(JSON.stringify({ok:!1,message:"platform required"}),{status:400,headers:e});let{results:_}=await t.DB.prepare(`
...
`+s.join(`
`)+`
</urlset>`;try{t.SITEMAP_CACHE&&await t.SITEMAP_CACHE.put("sitemap_xml",a,{expirationTtl:3600})}catch(l){console.log("sitemap cache write failed (non-fatal):",l.message)}return new Response(a,{headers:{...e,"Content-Type":"application/xml; charset=utf-8","X-Sitemap-Cache":"MISS"}})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}}return null}function O(r,i){return(r.headers.get("Authorization")||"").replace("Bearer ","")===i.ADMIN_SECRET}function L(r){let t=(r.headers.get("Cookie")||"").match(/session=([^;]+)/);return t?t[1]:null}async function F(r,i,t,f){try{return await f.DB.prepare("INSERT INTO user_point_logs (user_id, points, reason) VALUES (?, ?, ?)").bind(r,i,t).run(),await f.DB.prepare("UPDATE users SET ott_points = MAX(0, COALESCE(ott_points, 0) + ?) WHERE id = ?").bind(i,r).run(),await U(r,f),!0}catch(e){return console.error("[_addOttPoints] \uC624\uB958:",e.message),!1}}async function U(r,i){try{let t=await i.DB.prepare("SELECT grade, ott_points FROM users WHERE id = ?").bind(r).first();if(!t||(await i.DB.prepare("SELECT is_special FROM grade_settings WHERE grade_key = ?").bind(t.grade||"rookie").first())?.is_special)return;let{results:e}=await i.DB.prepare(`SELECT grade_key FROM grade_settings
       WHERE is_special = 0 AND min_ott_points <= ?
       ORDER BY min_ott_points DESC LIMIT 1`).bind(t.ott_points||0).all(),o=e[0]?.grade_key||null;o&&o!==t.grade&&await i.DB.prepare("UPDATE users SET grade = ? WHERE id = ?").bind(o,r).run()}catch(t){console.error("[GRADE]",t.message)}}async function Z(r,i){try{let S=function(N){if(!N||!k.length)return!0;let T=N.toLowerCase(),D=k.filter(C=>T.includes(C.toLowerCase())).length,h=k.length<=2?1:k.length===3?2:3;return D>=h},t=await i.DB.prepare("SELECT title_ko, title_en FROM works WHERE tmdb_id = ?").bind(r).first();if(!t?.title_ko)return console.log(`[YT_CRAWL] tmdb_id=${r} works \uC5C6\uC74C \u2014 \uC2A4\uD0B5`),0;let f=t.title_ko,e=t.title_en||"",o=await i.DB.prepare("SELECT platform, category_slot FROM rankings WHERE tmdb_id = ? ORDER BY date DESC LIMIT 1").bind(r).first(),n=new Set(["category07","category08"]),c=o?.platform==="netflix"&&n.has(o?.category_slot),d=c?"en":"ko",s=c&&e||f;console.log(`[YT_CRAWL] tmdb_id=${r} "${f}" \u2192 ${c?"\uC601\uC5B4":"\uD55C\uAD6D\uC5B4"} \uAC80\uC0C9 \uBAA8\uB4DC (slot=${o?.category_slot||"none"})`);let p=c?{netflix:"Netflix",tving:"Tving",disney:"Disney+",wavve:"Wavve",coupang:"Coupang Play",boxoffice:"Movie"}:{netflix:"\uB137\uD50C\uB9AD\uC2A4",tving:"\uD2F0\uBE59",disney:"\uB514\uC988\uB2C8\uD50C\uB7EC\uC2A4",wavve:"\uC6E8\uC774\uBE0C",coupang:"\uCFE0\uD321\uD50C\uB808\uC774",boxoffice:"\uC601\uD654"},u=o?.platform&&p[o.platform]||"",g=u?`${u} ${s}`:s,{results:m}=await i.DB.prepare("SELECT youtube_id, is_main FROM title_videos WHERE tmdb_id = ?").bind(r).all(),E=new Set(m.map(N=>N.youtube_id)),w=new Set(m.filter(N=>N.is_main===1).map(N=>N.youtube_id));w.size>0&&console.log(`[YT_CRAWL] tmdb_id=${r} \uBA54\uC778 \uC601\uC0C1 ${w.size}\uAC1C \uBCF4\uD638 \uC911`);let y=c?[`${g} official trailer`,`${g} trailer`]:[`${g} \uACF5\uC2DD \uC608\uACE0\uD3B8`,`${g} \uC608\uACE0\uD3B8`],k=s.replace(/[:\-·|]/g," ").replace(/[^\uAC00-\uD7A3a-zA-Z0-9\s]/g,"").split(/\s+/).filter(N=>N.length>=2),R=2,b=[];for(let N of y){if(b.length>=R)break;let T=`https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&order=relevance&maxResults=8&relevanceLanguage=${d}&q=${encodeURIComponent(N)}&key=${i.YOUTUBE_API_KEY}`,D=await fetch(T),h=await D.json();if(!(!D.ok||!h.items?.length))for(let C of h.items){if(b.length>=R)break;let I=C.id?.videoId,B=C.snippet?.title||"";!I||E.has(I)||w.has(I)||S(B)&&(b.push({youtube_id:I,title:B||s,youtube_url:`https://www.youtube.com/watch?v=${I}`}),E.add(I))}}if(!b.length)return console.log(`[YT_CRAWL] tmdb_id=${r} "${g}" \uACB0\uACFC \uC5C6\uC74C (\uAD00\uB828\uC131 \uD544\uD130 \uD1B5\uACFC \uC601\uC0C1 \uC5C6\uC74C)`),0;for(let N of b)await i.DB.prepare(`
        INSERT OR IGNORE INTO title_videos (tmdb_id, youtube_url, youtube_id, title, is_main)
        VALUES (?, ?, ?, ?, 0)
      `).bind(r,N.youtube_url,N.youtube_id,N.title).run();return console.log(`[YT_CRAWL] \u2705 tmdb_id=${r} "${g}" ${b.length}\uAC1C \uC800\uC7A5`),b.length}catch(t){return console.error(`[YT_CRAWL] tmdb_id=${r} \uC624\uB958:`,t.message),0}}async function v(r,i){return Z(r,i)}async function q(r,i){let t=await Z(r,i);try{await i.DB.prepare("UPDATE works SET yt_crawl_attempted_at = datetime('now') WHERE tmdb_id = ?").bind(r).run()}catch(f){console.error(`[YT_CRAWL_BATCH] tmdb_id=${r} \uC2DC\uB3C4 \uC2DC\uAC01 \uAE30\uB85D \uC2E4\uD328:`,f.message)}return t}async function tt(r,i){try{let f=(await i.DB.prepare("SELECT media_type FROM works WHERE tmdb_id = ?").bind(r).first())?.media_type||"tv",e=[];try{e=(await(await fetch(`https://api.themoviedb.org/3/${f}/${r}/videos?language=ko-KR&api_key=${i.TMDB_API_KEY}`)).json()).results||[]}catch{}if(!e.length)try{e=(await(await fetch(`https://api.themoviedb.org/3/${f}/${r}/videos?language=en-US&api_key=${i.TMDB_API_KEY}`)).json()).results||[]}catch{}let o=e.filter(_=>_.site==="YouTube"),n=[...o.filter(_=>_.type==="Trailer"||_.type==="Teaser"),...o.filter(_=>_.type!=="Trailer"&&_.type!=="Teaser")];if(!n.length)return console.log(`[TMDB_SAVE] tmdb_id=${r} TMDB \uC601\uC0C1 \uC5C6\uC74C`),0;for(let _=0;_<n.length;_++){let c=n[_],d=_===0?1:0;await i.DB.prepare(`
        INSERT OR IGNORE INTO title_videos (tmdb_id, youtube_url, youtube_id, title, is_main)
        VALUES (?, ?, ?, ?, ?)
...
`),p=(await(await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":f.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:4e3,messages:[{role:"user",content:s}]})})).json()).content?.[0]?.text||"[]",u=[];try{let g=p.split("```json").join("").split("```").join("").trim(),m=JSON.parse(g);u=Array.isArray(m)?m:[]}catch{console.error("[REACTION] Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328:",p.slice(0,300)),u=[]}await f.DB.prepare("DELETE FROM reaction_comments WHERE reaction_id = ?").bind(r).run();for(let g=0;g<c.length;g++){let m=c[g],w=(u.find(y=>y.idx===g)||u.find(y=>y.idx===g+1)||u[g]||{}).translated||"";await f.DB.prepare(`
    ...
    `).bind(...d).all()).results}let a=[],l=new Set;s.forEach(m=>{l.has(m.tmdb_id)||t.has(m.tmdb_id)||(l.add(m.tmdb_id),a.push(m))});let p=_.flat(),u=[...new Set(p.map(m=>m.tmdb_id).filter(Boolean))],g=new Set;if(u.length){let m=u.map(()=>"?").join(","),{results:E}=await r.DB.prepare(`SELECT tmdb_id FROM works WHERE tmdb_id IN (${m})`).bind(...u).all();g=new Set(E.map(w=>w.tmdb_id))}return p.forEach(m=>{let E=m.tmdb_id;!E||l.has(E)||t.has(E)||g.has(E)||!m.poster_path||(l.add(E),a.push({tmdb_id:E,title_ko:m.name||m.title||"",title_en:m.original_name||m.original_title||"",poster_path:m.poster_path,media_type:m._type,release_year:parseInt((m.first_air_date||m.release_date||"").slice(0,4))||null,tmdb_rating:m.vote_average||null,original_language:m.original_language||null}))}),a.sort((m,E)=>{let w=m.original_language==="ko"?0:1,y=E.original_language==="ko"?0:1;return w!==y?w-y:(E.tmdb_rating||0)-(m.tmdb_rating||0)}),a.slice(0,f)}async function at(r,i,t,f,e){if(r==="/works/search"&&i.method==="GET"){let o=f.searchParams.get("q")||"",n=Math.min(parseInt(f.searchParams.get("limit")||"15"),30),_=Math.max(parseInt(f.searchParams.get("offset")||"0"),0),c=100,d=15,s=24;if(!o.trim())return new Response(JSON.stringify({ok:!1,message:"q required"}),{status:400,headers:e});try{let[a,l,p]=await t.DB.batch(it(t,o,c)),u=new Map;a.results.forEach(R=>u.set(R.tmdb_id,0)),l.results.forEach(R=>{u.has(R.tmdb_id)||u.set(R.tmdb_id,1)}),p.results.forEach(R=>{u.has(R.tmdb_id)||u.set(R.tmdb_id,1)});let g=u.size>c,m=[...u.keys()].slice(0,c),E=[];if(m.length){let R=m.map(()=>"?").join(",");E=(await t.DB.prepare(`
          ...
          WHERE tmdb_id IN (${R})
            AND (adult_flag IS NULL OR adult_flag != 1)
            AND poster_path IS NOT NULL AND poster_path != ''
        `).bind(...m).all()).results}if(E.length<d){let R=[...new Set(o.split(/\s+/).filter(b=>b.length>=2))];R.length>=2&&(await Ht(t,R,new Set(u.keys()),s)).forEach(N=>{u.set(N.tmdb_id,3),E.push(N)})}let w=E.length;E.sort((R,b)=>{let N=u.get(R.tmdb_id)??1,T=u.get(b.tmdb_id)??1;if(N!==T)return N-T;let D=R.original_language==="ko"?0:1,h=b.original_language==="ko"?0:1;return D!==h?D-h:(b.tmdb_rating||0)-(R.tmdb_rating||0)});let y=E.slice(_,_+n),k=E.length>_+n,S=[];if(y.length){let R=y.map(D=>D.tmdb_id),b=R.map(()=>"?").join(","),{results:N}=await t.DB.prepare(`
          SELECT tmdb_id, platform, rank
          FROM rankings
          WHERE tmdb_id IN (${b})
            AND date = (SELECT value FROM app_settings WHERE key = 'latest_ranking_date')
        `).bind(...R).all(),T={};N.forEach(D=>{T[D.tmdb_id]||(T[D.tmdb_id]={}),T[D.tmdb_id][D.platform]=D.rank}),S=y.map(D=>({...D,ott_ranks:T[D.tmdb_id]||{}}))}return new Response(JSON.stringify({ok:!0,data:S,has_more:k,limit:n,offset:_,total:w,capped:g}),{headers:e})}catch(a){return new Response(JSON.stringify({ok:!1,message:a.message}),{status:500,headers:e})}}if(r==="/works/exists"&&i.method==="GET"){let n=(f.searchParams.get("ids")||"").split(",").map(_=>parseInt(_.trim())).filter(_=>Number.isInteger(_)).slice(0,100);if(!n.length)return new Response(JSON.stringify({ok:!0,existing_ids:[]}),{headers:e});try{let _=n.map(()=>"?").join(","),{results:c}=await t.DB.prepare(`
      ...
      `).bind(o,p||0,s.id,s.nickname,u,l?1:0).run();return new Response(JSON.stringify({ok:!0,id:g.meta?.last_row_id,nickname:s.nickname}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}if(r.match(/^\/reactions\/posts\/\d+$/)&&i.method==="DELETE")try{let o=parseInt(r.split("/")[3]),n=i.headers.get("Authorization")||"",_=n.startsWith("Bearer ")?n.slice(7).trim():null,c=L(i),d=_||c;if(!d)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:e});let s=await t.DB.prepare("SELECT s.user_id AS id FROM sessions s WHERE s.id = ? LIMIT 1").bind(d).first();if(!s)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:e});let a=await t.DB.prepare("SELECT id, user_id FROM reaction_posts WHERE id = ?").bind(o).first();return a?a.user_id!==s.id?new Response(JSON.stringify({ok:!1,message:"\uBCF8\uC778 \uB313\uAE00\uB9CC \uC0AD\uC81C\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4."}),{status:403,headers:e}):(await t.DB.prepare("DELETE FROM reaction_posts WHERE id = ?").bind(o).run(),new Response(JSON.stringify({ok:!0}),{headers:e})):new Response(JSON.stringify({ok:!1,message:"\uB313\uAE00\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4."}),{status:404,headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}if(r.match(/^\/reactions\/posts\/\d+\/like$/)&&i.method==="POST")try{let o=parseInt(r.split("/")[3]);return await t.DB.prepare("UPDATE reaction_posts SET like_count = like_count + 1 WHERE id = ?").bind(o).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}if(r==="/admin/reactions"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let o=await i.json(),{tmdb_id:n,title_ko:_,poster_path:c,video_id:d,video_title:s,channel_name:a,thumbnail:l,view_count:p,like_count:u,published_at:g,custom_title:m}=o;if(!n||!d)return new Response(JSON.stringify({ok:!1,message:"tmdb_id and video_id required"}),{status:400,headers:e});await t.DB.prepare(`
        ...
        VALUES (?, ?, ?, 'youtube', ?, ?, ?, ?, ?, ?, ?, ?, 1)
      `).bind(parseInt(n),_||"",c||"",d,s||"",m||s||"",a||"",l||"",p||0,u||0,g||new Date().toISOString()).run();let w=(await t.DB.prepare("SELECT id FROM reactions WHERE video_id = ? LIMIT 1").bind(d).first())?.id;return w&&t.YOUTUBE_API_KEY&&t.ANTHROPIC_API_KEY&&f.waitUntil(z(w,d,parseInt(n),t)),new Response(JSON.stringify({ok:!0,reaction_id:w,collecting:!!(w&&t.YOUTUBE_API_KEY),message:t.YOUTUBE_API_KEY?"\uB4F1\uB85D \uC644\uB8CC! \uB313\uAE00 \uC218\uC9D1\xB7\uBC88\uC5ED \uC911 (\uC57D 30\uCD08 \uD6C4 \uD45C\uC2DC)":"\uB4F1\uB85D \uC644\uB8CC"}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/reactions\/\d+\/collect$/)&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let o=parseInt(r.split("/")[3]),n=await t.DB.prepare("SELECT id, video_id, tmdb_id FROM reactions WHERE id = ? LIMIT 1").bind(o).first();return n?t.YOUTUBE_API_KEY?(f.waitUntil(z(n.id,n.video_id,n.tmdb_id,t)),new Response(JSON.stringify({ok:!0,message:"\uB313\uAE00 \uC218\uC9D1\xB7\uBC88\uC5ED \uC2DC\uC791! \uC57D 30\uCD08 \uD6C4 \uD655\uC778\uD558\uC138\uC694"}),{headers:e})):new Response(JSON.stringify({ok:!1,message:"YOUTUBE_API_KEY not set"}),{status:500,headers:e}):new Response(JSON.stringify({ok:!1,message:"reaction not found"}),{status:404,headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/reactions\/\d+$/)&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let o=parseInt(r.split("/")[3]),n=await i.json(),{custom_title:_,is_featured_off:c}=n;return c?await t.DB.prepare("UPDATE reactions SET is_featured = 0 WHERE id = ?").bind(o).run():await t.DB.prepare("UPDATE reactions SET custom_title = ? WHERE id = ?").bind(_||"",o).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/reactions\/\d+\/featured$/)&&i.method==="PUT"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let o=parseInt(r.split("/")[3]);return await t.DB.prepare("UPDATE reactions SET is_featured = 0").run(),await t.DB.prepare("UPDATE reactions SET is_featured = 1 WHERE id = ?").bind(o).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/reactions\/\d+$/)&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let o=parseInt(r.split("/")[3]);return await t.DB.prepare("DELETE FROM reactions WHERE id = ?").bind(o).run(),await t.DB.prepare("DELETE FROM reaction_comments WHERE reaction_id = ?").bind(o).run(),await t.DB.prepare("DELETE FROM reaction_posts WHERE reaction_id = ?").bind(o).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}}return null}var Y=["\uADC0\uC5EC\uC6B4","\uC6A9\uAC10\uD55C","\uC2E0\uBE44\uB85C\uC6B4","\uC5C9\uB6B1\uD55C","\uC870\uC6A9\uD55C","\uD65C\uBC1C\uD55C","\uB290\uAE0B\uD55C","\uC5F4\uC815\uC801\uC778","\uB0AD\uB9CC\uC801\uC778","\uC9C4\uC9C0\uD55C","\uC720\uCF8C\uD55C","\uB2F9\uB2F9\uD55C","\uC218\uC90D\uC740","\uB3C5\uD2B9\uD55C","\uBE60\uB978","\uB530\uB73B\uD55C","\uCC28\uAC00\uC6B4","\uBC30\uACE0\uD508","\uC878\uB9B0","\uBA4B\uC9C4","\uD669\uB2F9\uD55C","\uC9C4\uC9C0\uD55C","\uB290\uB9B0","\uC601\uB9AC\uD55C","\uAC15\uD55C"];async function nt(r,i,t,f){let e=new URL(i.url);if(r==="/auth/google"&&i.method==="GET"){let o=e.searchParams.get("redirect")||"",n="https://accounts.google.com/o/oauth2/v2/auth?client_id="+t.GOOGLE_CLIENT_ID+"&redirect_uri="+encodeURIComponent("https://ottrank-api.tdidream.workers.dev/auth/google/callback")+"&response_type=code&scope="+encodeURIComponent("openid email profile")+"&access_type=offline"+(o?"&state="+encodeURIComponent(o):"");return Response.redirect(n,302)}if(r==="/auth/google/callback"&&i.method==="GET"){let o=e.searchParams.get("code");if(!o)return Response.redirect("https://ottrank.kr?login=fail",302);try{let _=await(await fetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"authorization_code",client_id:t.GOOGLE_CLIENT_ID,client_secret:t.GOOGLE_CLIENT_SECRET,redirect_uri:"https://ottrank-api.tdidream.workers.dev/auth/google/callback",code:o})})).json();if(!_.access_token)return Response.redirect("https://ottrank.kr?login=fail",302);let d=await(await fetch("https://www.googleapis.com/oauth2/v2/userinfo",{headers:{Authorization:"Bearer "+_.access_token}})).json(),s=String(d.id),a=d.email||"",l=d.picture||"",p=await t.DB.prepare("SELECT id, nickname FROM users WHERE provider = 'google' AND provider_id = ?").bind(s).first();await t.DB.prepare(`
      ...
      `).bind(n,d.user_id,a,JSON.stringify(l||[]),JSON.stringify(p||[]),(u||"").slice(0,500),g?1:0).run(),E&&f.waitUntil(F(d.user_id,10,"review",t)),f.waitUntil(U(d.user_id,t)),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:e})}if(r.match(/^\/reviews\/\d+\/like\/\d+$/)&&i.method==="POST")try{let n=parseInt(r.split("/")[4]),c=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||L(i);if(!c)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:e});let d=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(c).first();if(!d)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:e});let s=await t.DB.prepare("SELECT user_id FROM reviews WHERE id = ?").bind(n).first();if(!s)return new Response(JSON.stringify({ok:!1,message:"\uB9AC\uBDF0\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC5B4\uC694"}),{status:404,headers:e});let a=await t.DB.prepare("SELECT id, is_active FROM review_likes WHERE review_id = ? AND user_id = ?").bind(n,d.user_id).first(),l;a?a.is_active?(await t.DB.prepare("UPDATE review_likes SET is_active = 0 WHERE id = ?").bind(a.id).run(),await t.DB.prepare("UPDATE reviews SET likes = MAX(0, likes - 1) WHERE id = ?").bind(n).run(),s.user_id&&await t.DB.prepare("UPDATE users SET total_likes_received = MAX(0, total_likes_received - 1) WHERE id = ?").bind(s.user_id).run(),l=!1):(await t.DB.prepare("UPDATE review_likes SET is_active = 1 WHERE id = ?").bind(a.id).run(),await t.DB.prepare("UPDATE reviews SET likes = likes + 1 WHERE id = ?").bind(n).run(),s.user_id&&await t.DB.prepare("UPDATE users SET total_likes_received = total_likes_received + 1 WHERE id = ?").bind(s.user_id).run(),l=!0):(await t.DB.prepare("INSERT INTO review_likes (review_id, user_id, is_active) VALUES (?, ?, 1)").bind(n,d.user_id).run(),await t.DB.prepare("UPDATE reviews SET likes = likes + 1 WHERE id = ?").bind(n).run(),s.user_id&&(await t.DB.prepare("UPDATE users SET total_likes_received = total_likes_received + 1 WHERE id = ?").bind(s.user_id).run(),f.waitUntil(F(s.user_id,1,"like_received",t)),f.waitUntil(U(s.user_id,t))),l=!0);let p=await t.DB.prepare("SELECT likes FROM reviews WHERE id = ?").bind(n).first();return new Response(JSON.stringify({ok:!0,liked:l,likes:p?.likes??0}),{headers:e})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:e})}if(r.match(/^\/reviews\/\d+$/)&&i.method==="DELETE")try{let n=parseInt(r.split("/")[2]),c=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||L(i);if(!c)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:e});let d=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(c).first();return d?(await t.DB.prepare("DELETE FROM reviews WHERE tmdb_id = ? AND user_id = ?").bind(n,d.user_id).run(),f.waitUntil(U(d.user_id,t)),new Response(JSON.stringify({ok:!0}),{headers:e})):new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:e})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:e})}if(r==="/mypage"&&i.method==="GET")try{let _=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||L(i);if(!_)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:e});let c=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(_).first();if(!c)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:e});let d=c.user_id,[s,a,l,p,u,g,m,E]=await t.DB.batch([t.DB.prepare(`
        ...
        `).bind(d),t.DB.prepare("SELECT * FROM pick_lists WHERE user_id = ? ORDER BY created_at DESC").bind(d),t.DB.prepare(`
          ...
          ORDER BY created_at DESC
          LIMIT 20
        `).bind(d),t.DB.prepare("SELECT grade_key, grade_name, min_ott_points, emoji_url, is_special, sort_order FROM grade_settings ORDER BY sort_order ASC")]),w=s.results[0]||null,y=a.results,k=l.results,S=p.results,R=u.results,b=g.results,N=m.results,T=E.results,D=[];if(b.length){let h=await t.DB.batch(b.map(C=>t.DB.prepare("SELECT * FROM pick_list_works WHERE pick_list_id = ? ORDER BY sort_order ASC, created_at DESC").bind(C.id)));D=b.map((C,I)=>{let B=h[I].results;return{...C,works:B,work_count:B.length}})}return new Response(JSON.stringify({ok:!0,is_own:!0,user:w,reviews:y,wishlist:k,posts:S,life_works:R,pick_lists:D,recent_point_logs:N,grade_settings:T,stats:{review_count:y.length,wishlist_count:k.length,likes_received:w?.total_likes_received||0,post_count:S.length,life_work_count:R.length,pick_list_count:D.length}}),{headers:e})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:e})}if(r==="/mypage/summary"&&i.method==="GET")try{let _=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||L(i);if(!_)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:e});let c=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(_).first();if(!c)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:e});let d=await t.DB.prepare(`
      ...
      `).bind(parseInt(u),S||"",R||"",k,D,S||null,R||null,k,D).run();let h=parseInt(g)||null;return h||(h=((await t.DB.prepare("SELECT MAX(rank) as max_rank FROM rankings WHERE platform = ? AND category_slot = ? AND date = ?").bind(a,l,p).first())?.max_rank||0)+1),await t.DB.prepare(`
      ...
      `).bind(a,l,l,p,-h,parseInt(u),S||"",R||"",k,b,N,T,y?1:0,l).run(),await t.DB.prepare("UPDATE rankings SET rank = ? WHERE platform = ? AND category_slot = ? AND date = ? AND rank = ?").bind(h,a,l,p,-h).run(),R&&S&&await t.DB.prepare(`
        ...
        SET tmdb_id     = COALESCE(?, tmdb_id),
            ...
            is_manual   = 1
        WHERE id = ?
      `).bind(l?parseInt(l):null,k,S,y,...b!==void 0?[b]:[],parseInt(a)).run(),l){g&&(S&&await t.DB.prepare("DELETE FROM works WHERE title_en = ? AND tmdb_id != ?").bind(S,parseInt(l)).run(),k&&/[\uAC00-\uD7A3]/.test(k)&&await t.DB.prepare("DELETE FROM works WHERE title_ko = ? AND tmdb_id != ?").bind(k,parseInt(l)).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, memo) VALUES ('works_delete', ?, ?)").bind(String(l),`\uC911\uBCF5 \uC0AD\uC81C: title_en="${S}" title_ko="${k}"`).run());let D=m==="tv"||m==="movie"?m:null,h=w?null:y;await t.DB.prepare(`
          ...
          ON CONFLICT(tmdb_id) DO UPDATE SET
            ...
            media_type  = COALESCE(?, media_type),
            updated_at  = datetime('now')
        `).bind(parseInt(l),k||"",S||"",h,D,k||null,S||null,h,D).run()}let N=S||k||"",T=k||S||"";return N&&T&&l&&await t.DB.prepare(`
          INSERT INTO title_map (title_en, title_ko, tmdb_id)
          VALUES (?, ?, ?)
          ON CONFLICT(title_en) DO UPDATE SET
            title_ko = excluded.title_ko,
            tmdb_id  = COALESCE(excluded.tmdb_id, tmdb_id)
        `).bind(N.trim(),T.trim(),parseInt(l)).run(),new Response(JSON.stringify({ok:!0,poster_path:y,title_ko:k,title_en:S}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/unfix"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});let s=await i.json(),{id:a}=s;return await t.DB.prepare("UPDATE rankings SET is_manual = 0 WHERE id = ?").bind(a).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}let _=r.match(/^\/admin\/rankings\/(\d+)$/);if(_&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(_[1]),{is_manual:a}=await i.json();if(a!==0&&a!==2)return new Response(JSON.stringify({ok:!1,message:"is_manual \uAC12\uC740 0(\uD574\uC81C) \uB610\uB294 2(\uD06C\uB864\uB9C1\uACE0\uC815)\uB9CC \uD5C8\uC6A9\uB429\uB2C8\uB2E4."}),{status:400,headers:e});let l=await t.DB.prepare("SELECT id, platform, category_slot, title_ko FROM rankings WHERE id = ?").bind(s).first();return l?(await t.DB.prepare("UPDATE rankings SET is_manual = ? WHERE id = ?").bind(a,s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('crawl_lock', ?, ?, ?, ?)").bind(l.platform,l.category_slot,String(s),JSON.stringify({is_manual:a,title_ko:l.title_ko})).run(),new Response(JSON.stringify({ok:!0,is_manual:a}),{headers:e})):new Response(JSON.stringify({ok:!1,message:"\uD574\uB2F9 \uB7AD\uD0B9\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4."}),{status:404,headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}let c=r.match(/^\/admin\/rankings\/(\d+)$/);if(c&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(c[1]),a=await t.DB.prepare("SELECT id, platform, category_slot, title_ko, rank, is_manual FROM rankings WHERE id = ?").bind(s).first();return a?(await t.DB.prepare("DELETE FROM rankings WHERE id = ?").bind(s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('ranking_delete', ?, ?, ?, ?)").bind(a.platform,a.category_slot,String(s),JSON.stringify({title_ko:a.title_ko,rank:a.rank,is_manual:a.is_manual})).run(),new Response(JSON.stringify({ok:!0}),{headers:e})):new Response(JSON.stringify({ok:!1,message:"\uD574\uB2F9 \uB7AD\uD0B9\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4."}),{status:404,headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/categories"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=f.searchParams.get("platform"),a="SELECT * FROM ott_categories",l=[];s&&(a+=" WHERE platform = ?",l.push(s)),a+=" ORDER BY platform, category_slot";let{results:p}=l.length?await t.DB.prepare(a).bind(...l).all():await t.DB.prepare(a).all();return new Response(JSON.stringify({ok:!0,data:p}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/categories\/\d+$/)&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),a=await i.json(),{display_name:l,crawl_limit:p,main_limit:u,platform_limit:g,is_active:m,main_section:E,main_order:w,platform_section:y,platform_order:k,memo_label:S,hot100_eligible:R,hot100_weight:b,person_section:N,person_order:T,person_limit:D}=a;return await t.DB.prepare(`
      ...
      `).bind(parseInt(l),s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('queue_resolve', ?, ?, ?, ?)").bind(g.platform,g.category_slot,String(l),JSON.stringify({tmdb_id:l,title_ko:E,title_en:w})).run(),new Response(JSON.stringify({ok:!0,poster_path:m,title_ko:E,title_en:w}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/rank-override"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),{platform:a,category_slot:l,date:p,tmdb_id:u,original_rank:g,override_rank:m,reason:E}=s;return!a||!l||!p||!u||!m?new Response(JSON.stringify({ok:!1,message:"\uD544\uC218 \uD30C\uB77C\uBBF8\uD130 \uB204\uB77D"}),{status:400,headers:e}):(await t.DB.prepare(`
        ...
        ON CONFLICT(platform, category_slot, date, tmdb_id) DO UPDATE SET
          ...
          updated_at    = datetime('now')
      `).bind(a,l,p,parseInt(u),g||0,parseInt(m),E||null).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, before_value, after_value) VALUES ('rank_override', ?, ?, ?, ?, ?)").bind(a,l,String(u),JSON.stringify({rank:g}),JSON.stringify({rank:m,reason:E})).run(),new Response(JSON.stringify({ok:!0}),{headers:e}))}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/rank-override"&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),{platform:a,category_slot:l,date:p,tmdb_id:u}=s;return await t.DB.prepare("DELETE FROM rank_overrides WHERE platform = ? AND category_slot = ? AND date = ? AND tmdb_id = ?").bind(a,l,p,parseInt(u)).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/works\/\d+$/)&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),a=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(s).first();return a?new Response(JSON.stringify({ok:!0,data:a}),{headers:e}):new Response(JSON.stringify({ok:!1,message:"Not found"}),{status:404,headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=f.searchParams.get("q")||"",a=f.searchParams.get("filter")||"",l=f.searchParams.get("date")||"",p=f.searchParams.get("sort")||"recent",u=parseInt(f.searchParams.get("page")||"1"),g=50,m=(u-1)*g,E=p==="updated"?"updated_at DESC, id DESC":"COALESCE(created_at, updated_at) DESC, id DESC",w,y;a==="new_match"&&l?(w=`SELECT * FROM works WHERE first_matched_date = ? AND match_source IN ('auto_claude', 'auto_tmdb') ORDER BY ${E} LIMIT ? OFFSET ?`,y=[l,g,m]):a==="adult_confirmed"&&s?(w=`SELECT * FROM works WHERE adult_flag = 1 AND (title_ko LIKE ? OR title_en LIKE ?) ORDER BY ${E} LIMIT ? OFFSET ?`,y=[`%${s}%`,`%${s}%`,g,m]):a==="adult_confirmed"?(w=`SELECT * FROM works WHERE adult_flag = 1 ORDER BY ${E} LIMIT ? OFFSET ?`,y=[g,m]):s?(w=`SELECT * FROM works WHERE title_ko LIKE ? OR title_en LIKE ? ORDER BY ${E} LIMIT ? OFFSET ?`,y=[`%${s}%`,`%${s}%`,g,m]):(w=`SELECT * FROM works ORDER BY ${E} LIMIT ? OFFSET ?`,y=[g,m]);let{results:k}=await t.DB.prepare(w).bind(...y).all();return new Response(JSON.stringify({ok:!0,data:k}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/works\/\d+$/)&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),a=await i.json(),{title_ko:l,title_en:p,poster_path:u,delete_duplicates:g,media_type:m,mbti_tags:E}=a,w=m==="tv"||m==="movie"?m:null,y=E!==void 0,k=y?E||null:void 0,S=await t.DB.prepare("SELECT title_ko, title_en, poster_path, media_type FROM works WHERE tmdb_id = ?").bind(s).first();if(g&&(p||S?.title_en)){let R=p||S?.title_en;await t.DB.prepare("DELETE FROM works WHERE title_en = ? AND tmdb_id != ?").bind(R,s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value, memo) VALUES ('works_delete', ?, ?, ?)").bind(String(s),JSON.stringify({title_en:R}),`\uC911\uBCF5 \uC0AD\uC81C: title_en="${R}" tmdb_id!=${s}`).run()}return await t.DB.prepare(`
        UPDATE works SET
          title_ko         = COALESCE(?, title_ko),
          title_en         = COALESCE(?, title_en),
          poster_path      = COALESCE(?, poster_path),
          ...
          updated_at       = datetime('now')
        WHERE tmdb_id = ?
      `).bind(l||null,p||null,u||null,w,...y?[k]:[],s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value, after_value) VALUES ('works_update', ?, ?, ?)").bind(String(s),JSON.stringify(S),JSON.stringify(a)).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/works\/\d+\/hero-backdrop$/)&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),a=await i.json(),{backdrop_path:l,hero_title_baked_in:p}=a,u=p===void 0?null:p?1:0;return await t.DB.prepare("UPDATE works SET hero_backdrop_path = ?, hero_title_baked_in = COALESCE(?, hero_title_baked_in) WHERE tmdb_id = ?").bind(l||null,u,s).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/works\/\d+\/hero-upload$/)&&i.method==="PUT"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),a=i.headers.get("Content-Type")||"image/jpeg",p={"image/jpeg":"jpg","image/png":"png","image/webp":"webp"}[a]||"jpg",u=`hero/${s}-${Date.now()}.${p}`;await t.IMAGES.put(u,i.body,{httpMetadata:{contentType:a}});let g=`https://img.ottrank.kr/${u}`,E=new URL(i.url).searchParams.get("baked_in")!=="0";return await t.DB.prepare("UPDATE works SET hero_custom_image_url = ?, hero_title_baked_in = ? WHERE tmdb_id = ?").bind(g,E?1:0,s).run(),new Response(JSON.stringify({ok:!0,url:g}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/works\/\d+\/hero-upload$/)&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),a=await t.DB.prepare("SELECT hero_custom_image_url FROM works WHERE tmdb_id = ?").bind(s).first();if(a?.hero_custom_image_url){let l=a.hero_custom_image_url.replace("https://img.ottrank.kr/","");try{await t.IMAGES.delete(l)}catch{}}return await t.DB.prepare("UPDATE works SET hero_custom_image_url = NULL WHERE tmdb_id = ?").bind(s).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/works\/\d+\/adult-flag$/)&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),l=(await i.json()).adult_flag===1?1:null;return await t.DB.prepare("UPDATE works SET adult_flag = ? WHERE tmdb_id = ?").bind(l,s).run(),new Response(JSON.stringify({ok:!0,adult_flag:l}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/works\/\d+$/)&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),a=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(s).first();return await t.DB.prepare("DELETE FROM works WHERE tmdb_id = ?").bind(s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value) VALUES ('works_delete', ?, ?)").bind(String(s),JSON.stringify(a)).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/new-match-count"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=f.searchParams.get("date")||new Date().toISOString().slice(0,10),a=await t.DB.prepare("SELECT COUNT(*) as count FROM works WHERE first_matched_date = ? AND match_source IN ('auto_claude','auto_tmdb')").bind(s).first();return new Response(JSON.stringify({ok:!0,count:a?.count||0,date:s}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/manual-rankings"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=f.searchParams.get("platform"),a=f.searchParams.get("category_slot");if(!s||!a)return new Response(JSON.stringify({ok:!1,message:"platform, category_slot required"}),{status:400,headers:e});let{results:l}=await t.DB.prepare(`
        ...
        ORDER BY rank ASC
      `).bind(s,a).all();return new Response(JSON.stringify({ok:!0,data:l}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/manual-rankings"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),{platform:a,category_slot:l,source_name:p,tmdb_id:u,rank:g,memo:m}=s,E=s.season!==void 0?s.season:null;if(!a||!l||!u||!g)return new Response(JSON.stringify({ok:!1,message:"platform, category_slot, tmdb_id, rank required"}),{status:400,headers:e});let w=s.title_ko||"",y=s.title_en||"",k=s.poster_path||null,S=s.genre||null,R=s.overview||null,b=s.release_year||null,N=s.tmdb_rating??null,T=s.media_type==="tv"||s.media_type==="movie"?s.media_type:null;if(!w||!k||!y){let h=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(parseInt(u)).first();h&&(w=w||h.title_ko||"",y=y||h.title_en||"",k=k||h.poster_path||null,S=S||h.genre||null,R=R||h.overview||null,b=b||h.release_year||null,N=N??h.tmdb_rating??null)}if(!y)try{let h=T?[T]:["tv","movie"];for(let C of h){let I=await fetch(`https://api.themoviedb.org/3/${C}/${u}?language=en-US&api_key=${t.TMDB_API_KEY}`);if(!I.ok)continue;let B=await I.json();if(!B.name&&!B.title)continue;let J=B.original_title||B.original_name||"",W=B.title||B.name||"";y=/[\uAC00-\uD7A3]/.test(J)?W:J||W;break}}catch{}await t.DB.prepare(`
        INSERT INTO rankings
      ...
      `).bind(a,l,l,p||"",parseInt(g),w,y,parseInt(u),k,S,R,b,N,m||null,E!==null?parseInt(E):null).run();let D=new Date().toISOString();return await t.DB.prepare(`
        ...
        ON CONFLICT(tmdb_id) DO UPDATE SET
          ...
          rating_updated_at = excluded.rating_updated_at,
          updated_at        = datetime('now')
      `).bind(parseInt(u),w||"",y||"",k,N,D).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('manual_ranking_add', ?, ?, ?, ?)").bind(a,l,String(u),JSON.stringify({rank:g,title_ko:w,title_en:y,memo:m})).run(),new Response(JSON.stringify({ok:!0,data:{title_ko:w,title_en:y,poster_path:k,genre:S,release_year:b,tmdb_rating:N}}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/manual-rankings/reorder"&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),{platform:a,category_slot:l,items:p}=s;if(!a||!l||!Array.isArray(p))return new Response(JSON.stringify({ok:!1,message:"platform, category_slot, items required"}),{status:400,headers:e});let u=p.map(m=>t.DB.prepare("UPDATE rankings SET rank = ? WHERE id = ? AND date = 'manual'").bind(-parseInt(m.rank),parseInt(m.id)));await t.DB.batch(u);let g=p.map(m=>t.DB.prepare("UPDATE rankings SET rank = ?, memo = ?, season = ? WHERE id = ? AND date = 'manual'").bind(parseInt(m.rank),m.memo??null,m.season!==void 0&&m.season!==null?parseInt(m.season):null,parseInt(m.id)));return await t.DB.batch(g),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, after_value) VALUES ('manual_ranking_reorder', ?, ?, ?)").bind(a,l,JSON.stringify(p)).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/manual-rankings\/\d+$/)&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]),a=await t.DB.prepare("SELECT * FROM rankings WHERE id = ? AND date = 'manual'").bind(s).first();return a?(await t.DB.prepare("DELETE FROM rankings WHERE id = ? AND date = 'manual'").bind(s).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, before_value) VALUES ('manual_ranking_delete', ?, ?, ?, ?)").bind(a.platform,a.category_slot,String(a.tmdb_id),JSON.stringify({rank:a.rank,title_ko:a.title_ko,memo:a.memo})).run(),new Response(JSON.stringify({ok:!0}),{headers:e})):new Response(JSON.stringify({ok:!1,message:"Not found or not a manual ranking"}),{status:404,headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/rankings/reorder"&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),{date:a,platform:l,category_slot:p,items:u}=s;if(!a||!l||!p||!Array.isArray(u))return new Response(JSON.stringify({ok:!1,message:"date, platform, category_slot, items required"}),{status:400,headers:e});let g=u.map(E=>t.DB.prepare("UPDATE rankings SET rank = ? WHERE id = ? AND date = ? AND platform = ? AND category_slot = ?").bind(-parseInt(E.rank),parseInt(E.id),a,l,p));await t.DB.batch(g);let m=u.map(E=>t.DB.prepare("UPDATE rankings SET rank = ? WHERE id = ? AND date = ? AND platform = ? AND category_slot = ?").bind(parseInt(E.rank),parseInt(E.id),a,l,p));return await t.DB.batch(m),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, after_value) VALUES ('ranking_reorder', ?, ?, ?)").bind(l,p,JSON.stringify(u)).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/sync-ratings"&&i.method==="PATCH"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let{results:s}=await t.DB.prepare(`
...
${p}`,m=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":t.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:3e3,system:u,messages:[{role:"user",content:g}]})});if(!m.ok){let T=await m.text().catch(()=>"");return new Response(JSON.stringify({ok:!1,message:`Claude API \uC624\uB958 (status ${m.status})`,detail:T.slice(0,300)}),{status:502,headers:e})}let w=((await m.json()).content||[]).filter(T=>T.type==="text").map(T=>T.text).join(""),y;try{let T=w.replace(/```json|```/g,"").trim();y=JSON.parse(T)}catch{return new Response(JSON.stringify({ok:!1,message:"Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694",raw:w.slice(0,300)}),{status:502,headers:e})}Array.isArray(y)||(y=[]);let k=new Set(l.map(T=>T.keyword_en)),S=new Map;for(let T of y){let D=(T.keyword_en||"").trim().toLowerCase(),h=(T.keyword_ko||"").trim();!D||!h||!k.has(D)||S.set(D,h)}let R=[],b=0;for(let T of l){if(!S.has(T.keyword_en)){R.push(t.DB.prepare("UPDATE keyword_translation SET translate_attempts = COALESCE(translate_attempts, 0) + 1 WHERE keyword_en = ? AND source IS NULL").bind(T.keyword_en));continue}R.push(t.DB.prepare("UPDATE keyword_translation SET keyword_ko = ?, source = 'auto' WHERE keyword_en = ? AND source IS NULL").bind(S.get(T.keyword_en),T.keyword_en)),b++}R.length&&await t.DB.batch(R);let N=await t.DB.prepare(`
        SELECT
      ...
      `).first();return new Response(JSON.stringify({ok:!0,attempted:l.length,translated:b,remaining:N?.remaining||0,stuck:N?.stuck||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/keywords/review"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=Math.min(parseInt(f.searchParams.get("limit"))||30,60),{results:a}=await t.DB.prepare(`
        SELECT id, keyword_en, keyword_ko
        FROM keyword_translation
        WHERE source = 'auto'
        ORDER BY id ASC
        LIMIT ?
      `).bind(s).all(),l=await t.DB.prepare("SELECT COUNT(*) as cnt FROM keyword_translation WHERE source = 'auto'").first();return new Response(JSON.stringify({ok:!0,items:a,remaining:l?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/keywords/review"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),l=(Array.isArray(s.items)?s.items:[]).filter(g=>g&&g.id&&typeof g.keyword_ko=="string"&&g.keyword_ko.trim());if(!l.length)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD55C \uD56D\uBAA9\uC774 \uC5C6\uC5B4\uC694"}),{status:400,headers:e});let p=l.map(g=>t.DB.prepare("UPDATE keyword_translation SET keyword_ko = ?, source = 'admin' WHERE id = ?").bind(g.keyword_ko.trim(),parseInt(g.id)));await t.DB.batch(p);let u=await t.DB.prepare("SELECT COUNT(*) as cnt FROM keyword_translation WHERE source = 'auto'").first();return new Response(JSON.stringify({ok:!0,updated:l.length,remaining:u?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/keywords/search"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=(f.searchParams.get("q")||"").trim();if(!s)return new Response(JSON.stringify({ok:!1,message:"\uAC80\uC0C9\uC5B4(q)\uAC00 \uD544\uC694\uD574\uC694"}),{status:400,headers:e});let a=`%${s}%`,{results:l}=await t.DB.prepare(`
        SELECT id, keyword_en, keyword_ko, source
        FROM keyword_translation
        WHERE keyword_en LIKE ? OR keyword_ko LIKE ?
        ORDER BY keyword_en ASC
        LIMIT 50
      `).bind(a,a).all();return new Response(JSON.stringify({ok:!0,items:l}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/keywords/update"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=(s.keyword_en||"").trim(),l=(s.keyword_ko||"").trim();if(!a||!l)return new Response(JSON.stringify({ok:!1,message:"keyword_en, keyword_ko \uBAA8\uB450 \uD544\uC694\uD574\uC694"}),{status:400,headers:e});let p=await t.DB.prepare("UPDATE keyword_translation SET keyword_ko = ?, source = 'admin' WHERE keyword_en = ?").bind(l,a).run();return!p.meta||p.meta.changes===0?new Response(JSON.stringify({ok:!1,message:"\uD574\uB2F9 keyword_en\uC744 \uCC3E\uC9C0 \uBABB\uD588\uC5B4\uC694"}),{status:404,headers:e}):new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/keywords"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=(f.searchParams.get("q")||"").trim();if(!s)return new Response(JSON.stringify({ok:!1,message:"\uAC80\uC0C9\uC5B4(q)\uAC00 \uD544\uC694\uD574\uC694"}),{status:400,headers:e});let a;if(/^\d+$/.test(s)){let g=await t.DB.prepare("SELECT tmdb_id, title_ko, title_en FROM works WHERE tmdb_id = ?").bind(parseInt(s)).first();a=g?[g]:[]}else{let{results:g}=await t.DB.prepare(`
...
${g}`,w=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":t.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:2e3,system:m,messages:[{role:"user",content:E}]})});if(!w.ok){let h=await w.text().catch(()=>"");return new Response(JSON.stringify({ok:!1,message:`Claude API \uC624\uB958 (status ${w.status})`,detail:h.slice(0,300)}),{status:502,headers:e})}let k=((await w.json()).content||[]).filter(h=>h.type==="text").map(h=>h.text).join(""),S;try{let h=k.replace(/```json|```/g,"").trim();S=JSON.parse(h)}catch{return new Response(JSON.stringify({ok:!1,message:"Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694",raw:k.slice(0,300)}),{status:502,headers:e})}Array.isArray(S)||(S=[]);let R=new Set(p),b=new Map;for(let h of S){let C=parseInt(h.tmdb_id);if(!C)continue;let I=Array.isArray(h.tags)?h.tags.filter(B=>R.has(B)).slice(0,2):[];b.set(C,I)}let N=[],T=0;for(let h of u){if(!b.has(h.tmdb_id))continue;let C=b.get(h.tmdb_id);N.push(t.DB.prepare("UPDATE works SET variety_genre = ?, variety_genre_source = 'auto' WHERE tmdb_id = ?").bind(C.length?C.join(","):null,h.tmdb_id)),T++}N.length&&await t.DB.batch(N);let D=await t.DB.prepare(`
      ...
      `).first();return new Response(JSON.stringify({ok:!0,attempted:u.length,classified:T,remaining:D?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/variety-genre-options"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let{results:s}=await t.DB.prepare("SELECT id, label, sort_order FROM variety_genre_options ORDER BY sort_order ASC").all();return new Response(JSON.stringify({ok:!0,data:s}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/variety-review"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=Math.min(parseInt(f.searchParams.get("limit"))||12,30),{results:a}=await t.DB.prepare(`
      ...
      `).bind(s).all(),l=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE variety_genre_source = 'auto'").first();return new Response(JSON.stringify({ok:!0,items:a,remaining:l?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/variety-review"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),l=(Array.isArray(s.items)?s.items:[]).filter(g=>g&&g.tmdb_id&&Array.isArray(g.tags));if(!l.length)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD55C \uD56D\uBAA9\uC774 \uC5C6\uC5B4\uC694"}),{status:400,headers:e});let p=l.map(g=>{let m=g.tags.filter(Boolean).slice(0,2);return t.DB.prepare("UPDATE works SET variety_genre = ?, variety_genre_source = 'admin' WHERE tmdb_id = ?").bind(m.length?m.join(","):null,parseInt(g.tmdb_id))});await t.DB.batch(p);let u=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE variety_genre_source = 'auto'").first();return new Response(JSON.stringify({ok:!0,updated:l.length,remaining:u?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/variety-review/skip"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=Array.isArray(s.tmdb_ids)?s.tmdb_ids.map(u=>parseInt(u)).filter(u=>Number.isInteger(u)):[];if(!a.length)return new Response(JSON.stringify({ok:!1,message:"tmdb_ids required"}),{status:400,headers:e});let l=new Date().toISOString(),p=a.map(u=>t.DB.prepare("UPDATE works SET variety_review_skipped_at = ? WHERE tmdb_id = ?").bind(l,u));return await t.DB.batch(p),new Response(JSON.stringify({ok:!0,skipped:a.length}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/pinned-similar"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=parseInt(s.tmdb_id),l=parseInt(s.related_tmdb_id),p=parseInt(s.pinned_pct);if((!p||p<1||p>99)&&(p=99),!a||!l)return new Response(JSON.stringify({ok:!1,message:"\uB450 \uC791\uD488\uC758 tmdb_id\uAC00 \uBAA8\uB450 \uD544\uC694\uD569\uB2C8\uB2E4"}),{status:400,headers:e});if(a===l)return new Response(JSON.stringify({ok:!1,message:"\uAC19\uC740 \uC791\uD488\uB07C\uB9AC\uB294 \uC5F0\uACB0\uD560 \uC218 \uC5C6\uC5B4\uC694"}),{status:400,headers:e});let{results:u}=await t.DB.prepare("SELECT tmdb_id FROM works WHERE tmdb_id IN (?, ?)").bind(a,l).all();return u.length<2?new Response(JSON.stringify({ok:!1,message:"works \uD14C\uC774\uBE14\uC5D0 \uC5C6\uB294 \uC791\uD488\uC774 \uD3EC\uD568\uB418\uC5B4 \uC788\uC5B4\uC694"}),{status:400,headers:e}):(await t.DB.batch([t.DB.prepare(`
        ...
        `).bind(a,l,p),t.DB.prepare(`
          ...
          VALUES (?, ?, ?)
          ON CONFLICT(tmdb_id, related_tmdb_id) DO UPDATE SET pinned_pct = excluded.pinned_pct
        `).bind(l,a,p)]),new Response(JSON.stringify({ok:!0,pinned_pct:p}),{headers:e}))}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.startsWith("/admin/works/pinned-similar/")&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/admin/works/pinned-similar/")[1]);if(!s)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:e});let{results:a}=await t.DB.prepare(`
        SELECT w.tmdb_id, w.title_ko, w.title_en, w.poster_path, p.pinned_pct
        FROM work_pinned_similar p
        JOIN works w ON w.tmdb_id = p.related_tmdb_id
        WHERE p.tmdb_id = ?
        ORDER BY p.pinned_pct DESC
      `).bind(s).all();return new Response(JSON.stringify({ok:!0,data:a}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/pinned-similar"&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=parseInt(s.tmdb_id),l=parseInt(s.related_tmdb_id);return!a||!l?new Response(JSON.stringify({ok:!1,message:"\uB450 \uC791\uD488\uC758 tmdb_id\uAC00 \uBAA8\uB450 \uD544\uC694\uD569\uB2C8\uB2E4"}),{status:400,headers:e}):(await t.DB.prepare(`
      ...
      `).bind(a,l,l,a).run(),new Response(JSON.stringify({ok:!0}),{headers:e}))}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/persons/collect"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=Math.min(parseInt(s.limit)||20,50),{results:l}=await t.DB.prepare(`
        ...
        LIMIT ?
      `).bind(a).all();if(!l.length)return new Response(JSON.stringify({ok:!0,worksScanned:0,personsFound:0,remaining:0,message:"\uC2A4\uCE94\uD560 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let p=new Map,u=[];for(let E of l){u.push(E.tmdb_id);let w=E.media_type==="tv"?"tv":"movie",y=w==="tv"?"aggregate_credits":"credits";try{let k=await fetch(`https://api.themoviedb.org/3/${w}/${E.tmdb_id}/${y}?api_key=${t.TMDB_API_KEY}`);if(!k.ok)continue;let S=await k.json();for(let R of(S.cast||[]).slice(0,15))R.id&&R.name&&!p.has(R.id)&&p.set(R.id,{name:R.name,job:"act"});for(let R of S.crew||[])(R.job==="Director"||R.job==="Creator"||R.department==="Directing"||(R.jobs||[]).some(N=>N.job==="Director"||N.job==="Creator"))&&R.id&&R.name&&p.set(R.id,{name:R.name,job:"direct"})}catch{}}let g=[];for(let[E,w]of p)g.push(t.DB.prepare(`INSERT INTO persons (tmdb_id, name, job) VALUES (?, ?, ?)
             ON CONFLICT(tmdb_id) DO NOTHING`).bind(E,w.name,w.job));for(let E of u)g.push(t.DB.prepare("UPDATE works SET credits_scanned = 1 WHERE tmdb_id = ?").bind(E));g.length&&await t.DB.batch(g);let m=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE credits_scanned IS NULL OR credits_scanned = 0").first();return new Response(JSON.stringify({ok:!0,worksScanned:l.length,personsFound:p.size,remaining:m?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/persons/search"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=(f.searchParams.get("q")||"").trim();if(!s)return new Response(JSON.stringify({ok:!0,items:[]}),{headers:e});let{results:a}=await t.DB.prepare("SELECT tmdb_id, name, job FROM persons WHERE name LIKE ? ORDER BY name LIMIT 30").bind(`%${s}%`).all();return new Response(JSON.stringify({ok:!0,items:a}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/persons\/\d+$/)&&i.method==="DELETE"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[3]);return await t.DB.prepare("DELETE FROM persons WHERE tmdb_id = ?").bind(s).run(),new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/backfill-language"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=Math.min(parseInt(s.limit)||30,50),{results:l}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
      ...
      `).bind(a).all();if(!l.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let p=[],u=0;for(let m of l){let E=m.media_type?[m.media_type]:["tv","movie"],w=null;for(let y of E)try{let k=await fetch(`https://api.themoviedb.org/3/${y}/${m.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!k.ok)continue;let S=await k.json(),R=S.release_date||S.first_air_date||"",b=parseInt(R.slice(0,4));if(b){w=b;break}}catch{}w?(p.push(t.DB.prepare("UPDATE works SET release_year = ? WHERE tmdb_id = ?").bind(w,m.tmdb_id)),u++):p.push(t.DB.prepare("UPDATE works SET release_year = 0 WHERE tmdb_id = ?").bind(m.tmdb_id))}p.length&&await t.DB.batch(p);let g=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE release_year IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:l.length,filled:u,remaining:g?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/backfill-rating"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=Math.min(parseInt(s.limit)||30,50),{results:l}=await t.DB.prepare(`
        ...
        LIMIT ?
      `).bind(a).all();if(!l.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let p=[],u=0,g=new Date().toISOString();for(let E of l){let w=E.media_type?[E.media_type]:["tv","movie"],y=null,k=null,S=!1;for(let R of w)try{let b=await fetch(`https://api.themoviedb.org/3/${R}/${E.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!b.ok)continue;let N=await b.json();S=!0,y=N.vote_average??null,k=N.release_date||N.first_air_date||null;break}catch{}S?(p.push(t.DB.prepare("UPDATE works SET tmdb_rating = ?, release_date = ?, rating_updated_at = ? WHERE tmdb_id = ?").bind(y,k,g,E.tmdb_id)),y!==null&&u++):p.push(t.DB.prepare("UPDATE works SET rating_updated_at = ? WHERE tmdb_id = ?").bind(g,E.tmdb_id))}p.length&&await t.DB.batch(p);let m=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE tmdb_rating IS NULL AND rating_updated_at IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:l.length,filled:u,remaining:m?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/batch-imdb-search"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=30;try{let w=await i.json();w?.limit&&Number.isInteger(w.limit)&&w.limit>0&&(s=w.limit)}catch{}let a=t.OMDB_API_KEY;if(!a)return new Response(JSON.stringify({ok:!1,message:"OMDB key not configured"}),{status:500,headers:e});let p=(await t.DB.prepare("SELECT value AS latest_date FROM app_settings WHERE key = 'latest_ranking_date'").first())?.latest_date||null,{results:u}=await t.DB.prepare(`
      ...
      `).first();return console.log(`[IMDB_BATCH_SEARCH] \u2705 \uC644\uB8CC: \uC2DC\uB3C4 ${u.length}\uAC74, \uB9E4\uCE6D ${g}\uAC1C`),new Response(JSON.stringify({ok:!0,attempted:u.length,filled:g,remaining:E?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/imdb-manual"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),a=parseInt(s?.tmdb_id);if(!a)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:e});let l=s?.imdb_rating===""||s?.imdb_rating==null?null:parseFloat(s.imdb_rating);if(l!==null&&(isNaN(l)||l<0||l>10))return new Response(JSON.stringify({ok:!1,message:"imdb_rating\uC740 0~10 \uC0AC\uC774 \uC22B\uC790\uC5EC\uC57C \uD569\uB2C8\uB2E4"}),{status:400,headers:e});let p=(s?.imdb_votes||"").toString().trim()||null,u=await t.DB.prepare("SELECT imdb_id FROM works WHERE tmdb_id = ?").bind(a).first();return u?(await t.DB.prepare("UPDATE works SET imdb_rating = ?, imdb_votes = ?, imdb_updated = datetime('now') WHERE tmdb_id = ?").bind(l,p,a).run(),new Response(JSON.stringify({ok:!0,warning:u.imdb_id?null:"imdb_id\uAC00 \uC5C6\uB294 \uC791\uD488\uC774\uB77C \uD654\uBA74\uC5D0 \uCE74\uB4DC\uAC00 \uC548 \uB730 \uC218 \uC788\uC2B5\uB2C8\uB2E4 (IMDb \uB9E4\uCE6D \uBC30\uCE58 \uC120\uD589 \uD544\uC694)"}),{headers:e})):new Response(JSON.stringify({ok:!1,message:"\uD574\uB2F9 tmdb_id \uC791\uD488\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4"}),{status:404,headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/missing-media-type"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=Math.min(parseInt(f.searchParams.get("limit"))||10,30),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, title_ko, poster_path
        FROM works
        WHERE media_type IS NULL OR media_type = ''
        ORDER BY tmdb_id
        LIMIT ?
      `).bind(s).all(),l=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE media_type IS NULL OR media_type = ''").first();return new Response(JSON.stringify({ok:!0,items:a,remaining:l?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/works/bulk-set-media-type"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),l=(Array.isArray(s.items)?s.items:[]).filter(g=>g&&g.tmdb_id&&(g.media_type==="movie"||g.media_type==="tv"));if(!l.length)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD55C \uD56D\uBAA9\uC774 \uC5C6\uC5B4\uC694 (media_type\uC740 'movie' \uB610\uB294 'tv'\uB9CC \uD5C8\uC6A9)"}),{status:400,headers:e});let p=l.map(g=>t.DB.prepare("UPDATE works SET media_type = ? WHERE tmdb_id = ?").bind(g.media_type,parseInt(g.tmdb_id)));await t.DB.batch(p);let u=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE media_type IS NULL OR media_type = ''").first();return new Response(JSON.stringify({ok:!0,updated:l.length,remaining:u?.cnt||0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/grade-settings"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let{results:s}=await t.DB.prepare("SELECT * FROM grade_settings ORDER BY sort_order ASC").all();return new Response(JSON.stringify({ok:!0,data:s}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/grade-settings"&&i.method==="PUT"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json();if(!Array.isArray(s))return new Response(JSON.stringify({ok:!1,message:"Array required"}),{status:400,headers:e});for(let a of s)await t.DB.prepare(`
        ...
        `).bind(a.grade_key,a.grade_name,a.emoji_url||"",a.min_ott_points||0,a.is_special?1:0,a.sort_order||0).run();return new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/grade-settings/assign"&&i.method==="POST"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let{user_id:s,grade_key:a}=await i.json();return!s||!a?new Response(JSON.stringify({ok:!1,message:"user_id, grade_key required"}),{status:400,headers:e}):(await t.DB.prepare("UPDATE users SET grade = ? WHERE id = ?").bind(a,s).run(),new Response(JSON.stringify({ok:!0}),{headers:e}))}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/users"&&i.method==="GET"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(f.searchParams.get("page")||"1"),a=50,l=(s-1)*a,p=f.searchParams.get("q")||"",u=`
...
2. \uB2E8\uC21C \uC21C\uC704 \uB098\uC5F4\uD615(TOP N) \uC81C\uBAA9\uC740 \uC9C0\uC591\uD558\uACE0 \uD654\uC81C\uC131\uC5D0 \uC9D1\uC911`}};async function G(r,i,t=null){let f=t?`SELECT category_slot, display_name, platform_limit, source_name
       FROM ott_categories
       WHERE platform = ?
         AND is_active = 1
...
`)}async function Et(r,i,t,f,e){if(i.method==="GET"&&r==="/blog-gen/image"){let o=f.searchParams.get("path")||"",n=f.searchParams.get("size")||"w780";if(!o)return new Response(JSON.stringify({ok:!1,error:"path \uD30C\uB77C\uBBF8\uD130 \uD544\uC694"}),{status:400,headers:e});try{let _=`https://image.tmdb.org/t/p/${n}${o}`,c=await fetch(_);if(!c.ok)throw new Error(`\uC774\uBBF8\uC9C0 \uB85C\uB4DC \uC2E4\uD328: ${c.status}`);let d=await c.arrayBuffer(),s=c.headers.get("content-type")||"image/jpeg";return new Response(d,{status:200,headers:{"Content-Type":s,"Access-Control-Allow-Origin":e["Access-Control-Allow-Origin"],"Cache-Control":"public, max-age=86400"}})}catch(_){return new Response(JSON.stringify({ok:!1,error:_.message}),{status:500,headers:e})}}if(i.method==="GET"&&r==="/blog-gen/preview"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,error:"Unauthorized"}),{status:401,headers:e});let o=f.searchParams.get("platform")||"netflix",n=f.searchParams.get("categorySlot")||null;if(!x[o])return new Response(JSON.stringify({ok:!1,error:"\uC9C0\uC6D0\uD558\uC9C0 \uC54A\uB294 \uD50C\uB7AB\uD3FC\uC785\uB2C8\uB2E4."}),{status:400,headers:e});try{let _=await G(o,t,n);return new Response(JSON.stringify({ok:!0,data:_}),{headers:e})}catch(_){return new Response(JSON.stringify({ok:!1,error:_.message}),{status:500,headers:e})}}if(i.method==="POST"&&r==="/blog-gen/suggest"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,error:"Unauthorized"}),{status:401,headers:e});let o=t.ANTHROPIC_API_KEY;if(!o)return new Response(JSON.stringify({ok:!1,error:"ANTHROPIC_API_KEY \uD658\uACBD \uBCC0\uC218\uAC00 \uC124\uC815\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4."}),{status:500,headers:e});let n;try{n=await i.json()}catch{return new Response(JSON.stringify({ok:!1,error:"\uC798\uBABB\uB41C \uC694\uCCAD \uD615\uC2DD\uC785\uB2C8\uB2E4."}),{status:400,headers:e})}let{platform:_="netflix",topicType:c="ranking",categorySlot:d="all"}=n;try{let s=[],a=_==="all"?["netflix","tving"]:[_],l=_!=="all"&&d&&d!=="all"?d:null;for(let D of a){if(D!=="all"&&!x[D])continue;let h=await G(D,t,l);s.push(...h)}let p="";s.length>0?p=s.map(D=>`[${D.display_name}]
`+(D.items||[]).slice(0,5).map((h,C)=>{let I=h.title_ko||h.title_en||"\uC81C\uBAA9 \uC5C6\uC74C",B=h.genre?` (${h.genre.split(",")[0]})`:"",J=h.tmdb_rating?` \u2605${parseFloat(h.tmdb_rating).toFixed(1)}`:"";return`  ${C+1}\uC704. ${I}${B}${J}`}).join(`
`)).join(`

`):p="\uD604\uC7AC \uB7AD\uD0B9 \uB370\uC774\uD130 \uC5C6\uC74C. OTT \uC778\uAE30 \uCF58\uD150\uCE20 \uC77C\uBC18 \uD2B8\uB80C\uB4DC \uAE30\uBC18\uC73C\uB85C \uCD94\uCC9C\uD574\uC8FC\uC138\uC694.";let u=_==="all"?"\uB137\uD50C\uB9AD\uC2A4\xB7\uD2F0\uBE59":x[_]||_,g=gt(),m=(()=>{if(s.length===1){let D=s[0].display_name||"";if(D.includes("\uC601\uD654"))return"\uC601\uD654";if(D.includes("\uB4DC\uB77C\uB9C8")||D.includes("TV")||D.includes("\uC2DC\uB9AC\uC988"))return"\uB4DC\uB77C\uB9C8"}return"\uB4DC\uB77C\uB9C8\xB7\uC601\uD654"})(),E=ut[c]||ut.ranking,w=E.examples.map(D=>"- "+D.replace(/{platform}/g,u).replace(/{media}/g,m).replace(/{week}/g,g)).join(`
...
]`,S=await fetch(ft,{method:"POST",headers:{"Content-Type":"application/json","x-api-key":o,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1500,messages:[{role:"user",content:k}],tools:[{type:"web_search_20250305",name:"web_search",max_uses:3}]})});if(!S.ok){let D=await S.json().catch(()=>({}));throw new Error(D.error?.message||`Anthropic API \uC624\uB958: ${S.status}`)}let N=(((await S.json()).content||[]).filter(D=>D.type==="text").map(D=>D.text).join("").trim()||"[]").replace(/^```json\s*/i,"").replace(/^```\s*/,"").replace(/\s*```$/,"").trim(),T;try{T=JSON.parse(N)}catch{let D=N.match(/\[[\s\S]*\]/);if(D)try{T=JSON.parse(D[0])}catch{}}if(!T)throw new Error("AI \uC751\uB2F5\uC744 JSON\uC73C\uB85C \uD30C\uC2F1\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4. \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694.");if(!Array.isArray(T))throw new Error("AI \uC751\uB2F5\uC774 \uBC30\uC5F4 \uD615\uC2DD\uC774 \uC544\uB2D9\uB2C8\uB2E4.");return T=T.filter(D=>D&&typeof D.title=="string"&&D.title.trim()).map(D=>({title:D.title.trim(),topic:D.topic?.trim()||"",contentType:D.contentType?.trim()||"weekly_ranking"})).slice(0,8),new Response(JSON.stringify({ok:!0,suggestions:T,rankingData:s,meta:{platform:u,weekLabel:g,topicType:c,categorySlot:l||"all",categoryLabel:l&&s.length===1?s[0].display_name:"\uC804\uCCB4",generatedAt:new Date().toISOString()}}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,error:s.message}),{status:500,headers:e})}}if(i.method==="POST"&&r==="/blog-gen"){if(!O(i,t))return new Response(JSON.stringify({ok:!1,error:"Unauthorized"}),{status:401,headers:e});let o=t.ANTHROPIC_API_KEY;if(!o)return new Response(JSON.stringify({ok:!1,error:"ANTHROPIC_API_KEY \uD658\uACBD \uBCC0\uC218\uAC00 \uC124\uC815\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4. Cloudflare Workers \u2192 Settings \u2192 Variables and Secrets\uC5D0\uC11C \uB4F1\uB85D\uD574\uC8FC\uC138\uC694."}),{status:500,headers:e});let n;try{n=await i.json()}catch{return new Response(JSON.stringify({ok:!1,error:"\uC798\uBABB\uB41C \uC694\uCCAD \uD615\uC2DD\uC785\uB2C8\uB2E4."}),{status:400,headers:e})}let{platform:_="netflix",contentType:c="weekly_ranking",categorySlot:d="all",tone:s="friendly",useEmoji:a=!0,useRating:l=!0,useLink:p=!0,useSpoiler:u=!1,useHashtag:g=!0,extraRequest:m=""}=n;if(!x[_])return new Response(JSON.stringify({ok:!1,error:"\uC9C0\uC6D0\uD558\uC9C0 \uC54A\uB294 \uD50C\uB7AB\uD3FC\uC785\uB2C8\uB2E4."}),{status:400,headers:e});let E=d&&d!=="all"?d:null;try{let w=await G(_,t,E);if(w.length===0)return new Response(JSON.stringify({ok:!1,error:E?"\uC120\uD0DD\uD55C \uCE74\uD14C\uACE0\uB9AC\uC758 \uB7AD\uD0B9 \uB370\uC774\uD130\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4. \uB2E4\uB978 \uCE74\uD14C\uACE0\uB9AC\uB97C \uC120\uD0DD\uD558\uAC70\uB098 '\uC804\uCCB4'\uB85C \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694.":"\uB7AD\uD0B9 \uB370\uC774\uD130\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4. \uD06C\uB864\uB9C1 \uC644\uB8CC \uD6C4 \uB2E4\uC2DC \uC2DC\uB3C4\uD558\uAC70\uB098, \uD398\uC774\uC9C0 \uCE74\uD14C\uACE0\uB9AC \uC124\uC815\uC5D0\uC11C OTT \uD398\uC774\uC9C0 \uB178\uCD9C \uC5EC\uBD80\uB97C \uD655\uC778\uD574\uC8FC\uC138\uC694."}),{status:404,headers:e});let y=Zt(w,_),k=gt(),S=x[_],R=w.length===1&&(w[0].display_name||"").includes("\uC601\uD654")?"\uC601\uD654":"\uB4DC\uB77C\uB9C8",b=!!(m&&m.trim()),N=b?m.trim():`${k} ${S} \u2014 ${mt[c]||mt.weekly_ranking}`,T=[];a||T.push("\uC774\uBAA8\uC9C0\uB97C \uC0AC\uC6A9\uD558\uC9C0 \uB9C8\uC138\uC694."),l&&T.push(`\uC624\uB728\uB791(${_t}) \uD3C9\uC810 \uC815\uBCF4\uB97C \uC790\uC5F0\uC2A4\uB7FD\uAC8C \uC5B8\uAE09\uD574\uC8FC\uC138\uC694.`),p&&T.push(`\uD3EC\uC2A4\uD305 \uC911\uAC04\uC774\uB098 \uB9C8\uC9C0\uB9C9\uC5D0 "${_t}" \uB9C1\uD06C\uB97C "\uC624\uB728\uB791\uC5D0\uC11C \uB354 \uBCF4\uAE30" \uD615\uD0DC\uB85C \uC790\uC5F0\uC2A4\uB7FD\uAC8C \uC0BD\uC785\uD574\uC8FC\uC138\uC694.`),u&&T.push("\uC2A4\uD3EC\uC77C\uB7EC \uC8FC\uC758 \uBB38\uAD6C\uAC00 \uD544\uC694\uD55C \uC791\uD488\uC5D0\uB294 \u26A0\uFE0F \uC2A4\uD3EC\uC8FC\uC758 \uB77C\uBCA8\uC744 \uB2EC\uC544\uC8FC\uC138\uC694."),g&&T.push(`\uD3EC\uC2A4\uD305 \uB9C8\uC9C0\uB9C9\uC5D0 \uB124\uC774\uBC84 \uBE14\uB85C\uADF8\uC6A9 \uD574\uC2DC\uD0DC\uADF8\uB97C 15\uAC1C \uC774\uC0C1 \uCD94\uAC00\uD574\uC8FC\uC138\uC694. (\uC608: #${S}${R}\uCD94\uCC9C #OTT\uCD94\uCC9C #${S}\uC21C\uC704 \uB4F1)`);let D=`\uB2F9\uC2E0\uC740 \uB124\uC774\uBC84 \uBE14\uB85C\uADF8\uC5D0 OTT \uCF58\uD150\uCE20 \uAE00\uC744 \uB9E4\uC77C \uC4F0\uB294 30\uB300 \uC9C1\uC7A5\uC778\uC785\uB2C8\uB2E4.
\uB4DC\uB77C\uB9C8\uB97C \uC9C4\uC9DC \uC88B\uC544\uD574\uC11C \uD1F4\uADFC \uD6C4\uC5D0 \uBCF4\uACE0, \uC8FC\uB9D0\uC5D0 \uBAB0\uC544\uBCF4\uACE0, \uB290\uB080 \uB300\uB85C \uC194\uC9C1\uD558\uAC8C \uC501\uB2C8\uB2E4.
${b?`\uC624\uB298 \uC4F8 \uAE00\uC758 \uC8FC\uC81C\uB294 \uC815\uD655\uD788 \uC774\uAC81\uB2C8\uB2E4: "${N}"
...
${b?`\uC8FC\uC81C("${N}")\uAC00 \uC704 \uB370\uC774\uD130\uB9CC\uC73C\uB85C\uB294 \uBD80\uC871\uD560 \uAC00\uB2A5\uC131\uC774 \uB192\uC2B5\uB2C8\uB2E4 \u2014 \uADF8\uB7F0 \uACBD\uC6B0
...
\uB9C8\uD06C\uB2E4\uC6B4 \uAE30\uD638 \uC5C6\uC774 \uC77C\uBC18 \uD14D\uC2A4\uD2B8\uB85C, \uB2E8\uB77D \uAD6C\uBD84\uC740 \uBE48 \uC904\uB85C\uB9CC \uD574\uC8FC\uC138\uC694.`,h=await vt(D,o,{useWebSearch:b,maxTokens:b?5e3:4096});if(!h)throw new Error("AI \uC751\uB2F5\uC774 \uBE44\uC5B4\uC788\uC2B5\uB2C8\uB2E4. \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694.");return new Response(JSON.stringify({ok:!0,post:h,rankingData:w,meta:{platform:_,platformName:S,weekInfo:k,categorySlot:E||"all",categoryLabel:E&&w.length===1?w[0].display_name:"\uC804\uCCB4",generatedAt:new Date().toISOString()}}),{headers:e})}catch(w){return new Response(JSON.stringify({ok:!1,error:w.message}),{status:500,headers:e})}}return null}var qt=["ad","bug"],te=["pending","answered","resolved"],ee=5,wt=30;async function kt(r,i,t,f,e,o){if(r==="/inquiry"&&i.method==="POST")try{let c=await i.json(),{type:d,name:s,email:a,phone:l,title:p,content:u,page_url:g,website:m}=c;if(m)return new Response(JSON.stringify({ok:!0}),{headers:o});if(!qt.includes(d))return new Response(JSON.stringify({ok:!1,message:"type\uC740 ad \uB610\uB294 bug\uC5EC\uC57C \uD569\uB2C8\uB2E4"}),{status:400,headers:o});if(!p||!p.trim()||!u||!u.trim())return new Response(JSON.stringify({ok:!1,message:"\uC81C\uBAA9\uACFC \uB0B4\uC6A9\uC740 \uD544\uC218\uC785\uB2C8\uB2E4"}),{status:400,headers:o});if(d==="ad"){if(!s||!s.trim())return new Response(JSON.stringify({ok:!1,message:"\uB2F4\uB2F9\uC790\uBA85 \uB610\uB294 \uC5C5\uCCB4\uBA85\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});if(!a||!a.trim())return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o})}if(a&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a))return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C \uD615\uC2DD\uC774 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4"}),{status:400,headers:o});let E=d,w=String(p).slice(0,200),y=String(u).slice(0,5e3),k=s?String(s).slice(0,100):null,S=a?String(a).slice(0,200):null,R=l?String(l).slice(0,30):null,b=g?String(g).slice(0,500):null,N=i.headers.get("User-Agent")||null,T=i.headers.get("CF-Connecting-IP")||null,D=null;try{let h=i.headers.get("Authorization")||"",I=(h.startsWith("Bearer ")?h.slice(7).trim():null)||L(i);if(I){let B=await t.DB.prepare("SELECT user_id AS id FROM sessions WHERE id = ? LIMIT 1").bind(I).first();B&&(D=B.id)}}catch{}return T&&((await t.DB.prepare(`SELECT COUNT(*) as cnt FROM inquiries
    ...
    `,{results:_}=await i.DB.prepare(n).bind(o).all(),{results:c}=await i.DB.prepare("SELECT tmdb_id, boost_value, is_pinned, pinned_score, pinned_platform FROM admin_boosts").all(),d=new Map((c||[]).map(E=>[E.tmdb_id,E]));if((!_||_.length===0)&&d.size===0)return new Response(JSON.stringify({ok:!1,error:"\uACC4\uC0B0\uD560 \uB7AD\uD0B9 \uB370\uC774\uD130\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4."}),{status:404,headers:t});let s=[],a=new Set;for(let E of _||[]){a.add(E.tmdb_id);let w=d.get(E.tmdb_id);w&&w.is_pinned?s.push({tmdb_id:E.tmdb_id,best_platform:w.pinned_platform||E.best_platform,best_rank:E.best_rank,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:w.pinned_score??0}):s.push(E)}for(let[E,w]of d)a.has(E)||(w.is_pinned?s.push({tmdb_id:E,best_platform:w.pinned_platform||"manual",best_rank:null,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:w.pinned_score??0}):w.boost_value&&s.push({tmdb_id:E,best_platform:w.pinned_platform||"manual",best_rank:null,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:w.boost_value}));s.sort((E,w)=>w.weighted_score+w.admin_boost-(E.weighted_score+E.admin_boost));let l=new Date(Date.now()+540*60*1e3).toISOString().slice(0,19).replace("T"," "),p=[i.DB.prepare("DELETE FROM hot100_scores")];for(let E of s){let w=E.weighted_score+E.admin_boost;p.push(i.DB.prepare(`INSERT INTO hot100_scores
         ...
         FROM works WHERE tmdb_id IN (${w})`).bind(...E).all(),k=new Map((y||[]).map(R=>[R.tmdb_id,R])),S=[i.DB.prepare("DELETE FROM rankings WHERE platform = 'netflix' AND category_slot = 'category10' AND date = ?").bind(o)];u.forEach((R,b)=>{let N=k.get(R.tmdb_id)||{};S.push(i.DB.prepare(`INSERT INTO rankings
              (platform, category_slot, category, date, rank, tmdb_id,
               title_ko, title_en, poster_path, release_year, genre, tmdb_rating,
               is_manual, source_name)
             VALUES ('netflix', 'category10', 'category10', ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'HOT100 \uAE30\uBC18 \uD1B5\uD569\uB7AD\uD0B9')`).bind(o,b+1,R.tmdb_id,N.title_ko||"",N.title_en||"",N.poster_path||null,N.release_year||null,N.genre||null,N.tmdb_rating||null))}),await i.DB.batch(S),g=u.length}let m={processed:0,found:0,failed:0};try{m=await yt(i,20)}catch(E){console.error("calcHot100 \uB85C\uACE0 \uBC31\uD544 \uC624\uB958:",E)}return new Response(JSON.stringify({ok:!0,netflix_overall_saved:g,calc_date:o,total_works:s.length,hero_logo_backfill:m,top10_preview:s.slice(0,10).map(E=>({tmdb_id:E.tmdb_id,best_platform:E.best_platform,best_rank:E.best_rank,total_score:E.weighted_score+E.admin_boost}))}),{status:200,headers:t})}catch(e){return console.error("calcHot100 \uC624\uB958:",e),new Response(JSON.stringify({ok:!1,error:"HOT100 \uACC4\uC0B0 \uC911 \uC624\uB958\uAC00 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4.",detail:e.message}),{status:500,headers:t})}}async function St(r,i,t){if(!await O(r,i))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:t});try{let{results:e}=await i.DB.prepare(`SELECT ab.tmdb_id, ab.boost_value, ab.reason, ab.is_pinned, ab.pinned_score, ab.pinned_platform, ab.updated_at,
              w.title_ko, w.poster_path
       FROM admin_boosts ab
       LEFT JOIN works w ON w.tmdb_id = ab.tmdb_id
       ORDER BY ab.updated_at DESC`).all();return new Response(JSON.stringify({ok:!0,data:e||[]}),{status:200,headers:t})}catch(e){return new Response(JSON.stringify({ok:!1,error:e.message}),{status:500,headers:t})}}async function Ot(r,i,t){if(!await O(r,i))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:t});try{let o=(new URL(r.url).searchParams.get("q")||"").trim();if(!o)return new Response(JSON.stringify({ok:!0,data:[]}),{status:200,headers:t});let{results:n}=await i.DB.prepare(`SELECT w.tmdb_id, w.title_ko, w.title_en, w.poster_path,
       ...
       ON CONFLICT(tmdb_id) DO UPDATE SET
         ...
         updated_at = excluded.updated_at`).bind(o,p,_||null,u,g,m,E).run(),new Response(JSON.stringify({ok:!0}),{status:200,headers:t})}catch(e){return new Response(JSON.stringify({ok:!1,error:e.message}),{status:500,headers:t})}}async function Nt(r,i,t,f){if(!await O(i,t))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:f});try{return await t.DB.prepare("DELETE FROM admin_boosts WHERE tmdb_id = ?").bind(r).run(),new Response(JSON.stringify({ok:!0}),{status:200,headers:f})}catch(o){return new Response(JSON.stringify({ok:!1,error:o.message}),{status:500,headers:f})}}async function Tt(r,i,t){try{let f=new URL(r.url),e=parseInt(f.searchParams.get("limit")||"100",10),o=Number.isNaN(e)?100:Math.min(e,100),n=`
      SELECT
        h.tmdb_id,
        h.best_platform,
        h.total_score,
      ...
      LIMIT ?
    `,{results:_}=await i.DB.prepare(n).bind(o).all();return!_||_.length===0?new Response(JSON.stringify({ok:!0,data:[]}),{status:200,headers:t}):new Response(JSON.stringify({ok:!0,data:_.map((c,d)=>({hot_rank:d+1,...c}))}),{status:200,headers:t})}catch(f){return console.error("getHot100 \uC624\uB958:",f),new Response(JSON.stringify({ok:!1,error:"HOT100 \uC870\uD68C \uC911 \uC624\uB958\uAC00 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4.",detail:f.message}),{status:500,headers:t})}}async function Dt(r,i,t){if(!await O(r,i))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:t});try{let{results:e}=await i.DB.prepare(`SELECT platform, category_slot, top_n, display_order, is_active
       FROM hot100_frontend_tabs
       ORDER BY display_order ASC`).all();return new Response(JSON.stringify({ok:!0,data:e||[]}),{status:200,headers:t})}catch(e){return new Response(JSON.stringify({ok:!1,error:e.message}),{status:500,headers:t})}}async function ht(r,i,t,f){if(!await O(i,t))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:f});try{let o=await i.json(),{category_slot:n,top_n:_,display_order:c,is_active:d}=o;return await t.DB.prepare(`UPDATE hot100_frontend_tabs SET
