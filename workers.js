...
\uB9C8\uD06C\uB2E4\uC6B4 \uAE30\uD638 \uC5C6\uC774 \uC77C\uBC18 \uD14D\uC2A4\uD2B8\uB85C, \uB2E8\uB77D \uAD6C\uBD84\uC740 \uBE48 \uC904\uB85C\uB9CC \uD574\uC8FC\uC138\uC694.`,I=await Ht(T,o,{useWebSearch:R,maxTokens:R?5e3:4096});if(!I)throw new Error("AI \uC751\uB2F5\uC774 \uBE44\uC5B4\uC788\uC2B5\uB2C8\uB2E4. \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694.");return new Response(JSON.stringify({ok:!0,post:I,rankingData:E,meta:{platform:l,platformName:b,weekInfo:y,categorySlot:k||"all",categoryLabel:k&&E.length===1?E[0].display_name:"\uC804\uCCB4",generatedAt:new Date().toISOString()}}),{headers:s})}catch(E){return new Response(JSON.stringify({ok:!1,error:E.message}),{status:500,headers:s})}}return null}var Ft=["ad","bug"],Wt=["pending","answered","resolved"],Ut=5,pt=30;async function _t(d,i,t,f,s,o){if(d==="/inquiry"&&i.method==="POST")try{let e=await i.json(),{type:r,name:a,email:c,phone:p,title:m,content:_,page_url:u,website:g}=e;if(g)return new Response(JSON.stringify({ok:!0}),{headers:o});if(!Ft.includes(r))return new Response(JSON.stringify({ok:!1,message:"type\uC740 ad \uB610\uB294 bug\uC5EC\uC57C \uD569\uB2C8\uB2E4"}),{status:400,headers:o});if(!m||!m.trim()||!_||!_.trim())return new Response(JSON.stringify({ok:!1,message:"\uC81C\uBAA9\uACFC \uB0B4\uC6A9\uC740 \uD544\uC218\uC785\uB2C8\uB2E4"}),{status:400,headers:o});if(r==="ad"){if(!a||!a.trim())return new Response(JSON.stringify({ok:!1,message:"\uB2F4\uB2F9\uC790\uBA85 \uB610\uB294 \uC5C5\uCCB4\uBA85\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});if(!c||!c.trim())return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o})}if(c&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(c))return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C \uD615\uC2DD\uC774 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4"}),{status:400,headers:o});let k=r,E=String(m).slice(0,200),w=String(_).slice(0,5e3),y=a?String(a).slice(0,100):null,b=c?String(c).slice(0,200):null,N=p?String(p).slice(0,30):null,R=u?String(u).slice(0,500):null,D=i.headers.get("User-Agent")||null,S=i.headers.get("CF-Connecting-IP")||null,T=null;try{let I=i.headers.get("Authorization")||"",L=(I.startsWith("Bearer ")?I.slice(7).trim():null)||h(i);if(L){let B=await t.DB.prepare("SELECT user_id AS id FROM sessions WHERE id = ? LIMIT 1").bind(L).first();B&&(T=B.id)}}catch{}return S&&((await t.DB.prepare(`SELECT COUNT(*) as cnt FROM inquiries
    ...
    `,{results:l}=await i.DB.prepare(n).bind(o).all(),{results:e}=await i.DB.prepare("SELECT tmdb_id, pinned_score FROM admin_boosts WHERE is_pinned = 1").all(),r=new Map((e||[]).map(_=>[_.tmdb_id,_.pinned_score??0]));if((!l||l.length===0)&&r.size===0)return new Response(JSON.stringify({ok:!1,error:"\uACC4\uC0B0\uD560 \uB7AD\uD0B9 \uB370\uC774\uD130\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4."}),{status:404,headers:t});let a=[],c=new Set;for(let _ of l||[])c.add(_.tmdb_id),r.has(_.tmdb_id)?a.push({tmdb_id:_.tmdb_id,best_platform:_.best_platform,best_rank:_.best_rank,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:r.get(_.tmdb_id)}):a.push(_);for(let[_,u]of r)c.has(_)||a.push({tmdb_id:_,best_platform:"manual",best_rank:null,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:u});a.sort((_,u)=>u.weighted_score+u.admin_boost-(_.weighted_score+_.admin_boost));let p=new Date(Date.now()+540*60*1e3).toISOString().slice(0,19).replace("T"," "),m=[i.DB.prepare("DELETE FROM hot100_scores")];for(let _ of a){let u=_.weighted_score+_.admin_boost;m.push(i.DB.prepare(`INSERT INTO hot100_scores
       ...
       LIMIT 20`).bind(`%${o}%`,`%${o}%`,parseInt(o,10)||0).all();return new Response(JSON.stringify({ok:!0,data:n||[]}),{status:200,headers:t})}catch(s){return new Response(JSON.stringify({ok:!1,error:s.message}),{status:500,headers:t})}}async function Et(d,i,t){if(!await O(d,i))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:t});try{let s=await d.json(),{tmdb_id:o,boost_value:n,reason:l,is_pinned:e,pinned_score:r}=s;if(!o)return new Response(JSON.stringify({ok:!1,error:"tmdb_id\uAC00 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:400,headers:t});let a=e,c=r;if(e===void 0){let m=await i.DB.prepare("SELECT is_pinned, pinned_score FROM admin_boosts WHERE tmdb_id = ?").bind(o).first();a=m?.is_pinned||0,c=m?.pinned_score??null}let p=new Date(Date.now()+540*60*1e3).toISOString().slice(0,19).replace("T"," ");return await i.DB.prepare(`INSERT INTO admin_boosts (tmdb_id, boost_value, reason, is_pinned, pinned_score, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(tmdb_id) DO UPDATE SET
