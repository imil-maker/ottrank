function Z(r,i,t){if(!i.length)return r.slice(0,t).map((l,s)=>({...l,rank:s+1}));let f=new Set(i.map(l=>l.tmdb_id).filter(Boolean)),e=r.filter(l=>!f.has(l.tmdb_id)),o={};for(let l of i){let s=Math.max(1,parseInt(l.rank)||1);o[s]||(o[s]=[]),o[s].push(l)}let n=[],p=0,_=1;for(;n.length<t;){if(o[_]&&o[_].length){let l=o[_].shift();n.push({...l,rank:n.length+1})}else if(p<e.length)n.push({...e[p],rank:n.length+1}),p++;else{let l=Object.values(o).flat();for(let s of l){if(n.length>=t)break;n.push({...s,rank:n.length+1})}break}_++}return n}async function it(r,i){if(!i||!i.length)return[];let t=i.map(e=>r.DB.prepare(`
...
`),c=(await(await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":f.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:4e3,messages:[{role:"user",content:s}]})})).json()).content?.[0]?.text||"[]",m=[];try{let u=c.split("```json").join("").split("```").join("").trim(),E=JSON.parse(u);m=Array.isArray(E)?E:[]}catch{console.error("[REACTION] Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328:",c.slice(0,300)),m=[]}await f.DB.prepare("DELETE FROM reaction_comments WHERE reaction_id = ?").bind(r).run();for(let u=0;u<_.length;u++){let E=_[u],g=(m.find(y=>y.idx===u)||m.find(y=>y.idx===u+1)||m[u]||{}).translated||"";await f.DB.prepare(`
      ...
      `).bind(r,t,E.text.slice(0,1e3),g.slice(0,1e3),E.author.slice(0,100),E.likes,"neutral").run()}console.log(`[REACTION] \u2705 \uC644\uB8CC: reaction=${r} \uB313\uAE00 ${_.length}\uAC1C \uC800\uC7A5`)}catch(e){console.error("[REACTION] \uC624\uB958:",e.message)}}async function lt(r,i,t,f,e,o){if(r.startsWith("/videos/")&&!r.includes("/admin")&&i.method==="GET"){let n=parseInt(r.split("/videos/")[1]);if(!n)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:o});try{let{results:p}=await t.DB.prepare("SELECT * FROM title_videos WHERE tmdb_id = ? ORDER BY is_main DESC, created_at DESC").bind(n).all();return p.length===0&&f.waitUntil(dt(n,t)),new Response(JSON.stringify({ok:!0,data:p}),{headers:o})}catch(p){return new Response(JSON.stringify({ok:!1,message:p.message}),{status:500,headers:o})}}if(r==="/admin/videos/crawl"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});try{let n=await i.json(),{tmdb_id:p}=n;if(!p)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:o});let _=await nt(parseInt(p),t);return new Response(JSON.stringify({ok:!0,saved:_}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}}if(r==="/admin/videos/batch-crawl"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});try{let n=20;try{let g=await i.json();g?.limit&&Number.isInteger(g.limit)&&g.limit>0&&(n=g.limit)}catch{}let p=30,l=(await t.DB.prepare("SELECT COUNT(*) AS cnt FROM works WHERE yt_crawl_attempted_at >= date('now')").first())?.cnt||0;if(l>=p){let g=await t.DB.prepare(`
        ...
        ORDER BY
          ...
          w.created_at DESC
        LIMIT ?
      `).bind(d,s).all();if(!c.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uB300\uC0C1 \uC791\uD488 \uC5C6\uC74C (\uBAA8\uB450 \uCFE8\uB2E4\uC6B4 \uC911\uC774\uAC70\uB098 \uC601\uC0C1\uC774 \uC774\uBBF8 \uCDA9\uBD84\uD568)"}),{headers:o});let m=[],u=0;for(let g of c)try{let y=await ot(g.tmdb_id,t);u+=y,m.push({tmdb_id:g.tmdb_id,saved:y,ok:!0})}catch(y){console.error(`[BATCH_CRAWL] tmdb_id=${g.tmdb_id} \uC624\uB958:`,y.message),m.push({tmdb_id:g.tmdb_id,saved:0,ok:!1,error:y.message})}let w=(await t.DB.prepare(`
        SELECT COUNT(*) AS cnt
        FROM works w
        WHERE (
          SELECT COUNT(*) FROM title_videos tv WHERE tv.tmdb_id = w.tmdb_id
      ...
      `).first())?.cnt||0;return console.log(`[BATCH_CRAWL] \u2705 \uC644\uB8CC: \uC2DC\uB3C4 ${c.length}\uAC74, \uC800\uC7A5 ${u}\uAC1C, \uB0A8\uC74C ${w}`),new Response(JSON.stringify({ok:!0,attempted:c.length,filled:u,remaining:w,results:m}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}}if(r==="/admin/videos"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});try{let n=await i.json(),{tmdb_id:p,youtube_url:_}=n,{title:l}=n;if(!p||!_)return new Response(JSON.stringify({ok:!1,message:"tmdb_id, youtube_url required"}),{status:400,headers:o});let s=_.match(/(?:v=|youtu\.be\/|embed\/)([a-zA-Z0-9_-]{11})/);if(!s)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD558\uC9C0 \uC54A\uC740 \uC720\uD29C\uBE0C URL"}),{status:400,headers:o});let a=s[1],d=await t.DB.prepare("SELECT id, title FROM title_videos WHERE tmdb_id = ? AND youtube_id = ? LIMIT 1").bind(p,a).first();if(d)return new Response(JSON.stringify({ok:!1,message:`\uC774\uBBF8 \uB4F1\uB85D\uB41C \uC601\uC0C1\uC785\uB2C8\uB2E4. (\uC81C\uBAA9: "${d.title||a}")`}),{status:409,headers:o});if(!l)try{l=(await(await fetch(`https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${a}&format=json`)).json()).title||""}catch{l=""}return await t.DB.prepare("INSERT INTO title_videos (tmdb_id, youtube_url, youtube_id, title, is_main) VALUES (?, ?, ?, ?, 0)").bind(p,_,a,l).run(),new Response(JSON.stringify({ok:!0}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}}if(r.match(/\/admin\/videos\/(\d+)\/main/)&&i.method==="PATCH"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});let n=parseInt(r.match(/\/admin\/videos\/(\d+)\/main/)[1]);try{let{results:p}=await t.DB.prepare("SELECT tmdb_id FROM title_videos WHERE id = ?").bind(n).all();if(!p.length)return new Response(JSON.stringify({ok:!1,message:"\uC5C6\uC74C"}),{status:404,headers:o});let _=p[0].tmdb_id;return await t.DB.batch([t.DB.prepare("UPDATE title_videos SET is_main = 0 WHERE tmdb_id = ?").bind(_),t.DB.prepare("UPDATE title_videos SET is_main = 1 WHERE id = ?").bind(n)]),new Response(JSON.stringify({ok:!0}),{headers:o})}catch(p){return new Response(JSON.stringify({ok:!1,message:p.message}),{status:500,headers:o})}}if(r.match(/\/admin\/videos\/(\d+)$/)&&i.method==="DELETE"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:o});let n=parseInt(r.match(/\/admin\/videos\/(\d+)$/)[1]);try{return await t.DB.prepare("DELETE FROM title_videos WHERE id = ?").bind(n).run(),new Response(JSON.stringify({ok:!0}),{headers:o})}catch(p){return new Response(JSON.stringify({ok:!1,message:p.message}),{status:500,headers:o})}}if(r.startsWith("/imdb/")&&r!=="/imdb/save"&&i.method==="GET"){let n=r.split("/imdb/")[1];if(!n||!/^tt\d+$/.test(n))return new Response(JSON.stringify({ok:!1,message:"invalid imdb_id"}),{status:400,headers:o});try{let p=await t.DB.prepare("SELECT imdb_rating, imdb_votes, imdb_updated FROM works WHERE imdb_id = ? LIMIT 1").bind(n).first();if(p?.imdb_rating){let a=new Date(p.imdb_updated||0);if((Date.now()-a.getTime())/(1e3*60*60*24)<7)return new Response(JSON.stringify({ok:!0,source:"cache",rating:p.imdb_rating.toFixed(1),votes:p.imdb_votes||""}),{headers:o})}let _=t.OMDB_API_KEY;if(!_)return new Response(JSON.stringify({ok:!1,message:"OMDB key not configured"}),{status:500,headers:o});let s=await(await fetch(`https://www.omdbapi.com/?i=${n}&apikey=${_}`)).json();if(s.Response!=="False"){let a=parseFloat(s.imdbRating);if(!isNaN(a)){let d=s.imdbVotes||"",c=new Date().toISOString();return await t.DB.prepare("UPDATE works SET imdb_rating = ?, imdb_votes = ?, imdb_updated = ? WHERE imdb_id = ?").bind(a,d,c,n).run(),new Response(JSON.stringify({ok:!0,source:"omdb",rating:a.toFixed(1),votes:d}),{headers:o})}}return new Response(JSON.stringify({ok:!1,message:"rating not available"}),{status:404,headers:o})}catch(p){return console.error("[IMDB GET]",p),new Response(JSON.stringify({ok:!1,message:p.message}),{status:500,headers:o})}}if(r==="/imdb/save"&&i.method==="POST")try{let n=await i.json(),{tmdb_id:p,imdb_id:_}=n;return!p||!_?new Response(JSON.stringify({ok:!1,message:"tmdb_id and imdb_id required"}),{status:400,headers:o}):/^tt\d+$/.test(_)?(await t.DB.prepare("UPDATE works SET imdb_id = ? WHERE tmdb_id = ?").bind(_,parseInt(p)).run(),new Response(JSON.stringify({ok:!0}),{headers:o})):new Response(JSON.stringify({ok:!1,message:"invalid imdb_id format"}),{status:400,headers:o})}catch(n){return console.error("[IMDB SAVE]",n),new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}if(r==="/youtube/trending"&&i.method==="GET")try{let{results:n}=await t.DB.prepare("SELECT * FROM youtube_trending ORDER BY rank ASC").all();if(n.length>0){let c=new Date(n[0].collected_at);if((Date.now()-c.getTime())/(1e3*60*60)<6)return new Response(JSON.stringify({ok:!0,data:n,cached:!0}),{headers:o})}let p=`https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&chart=mostPopular&regionCode=KR&maxResults=50&key=${t.YOUTUBE_API_KEY}`,_=await fetch(p),l=await _.json();if(!_.ok||!l.items?.length)return n.length>0?new Response(JSON.stringify({ok:!0,data:n,cached:!0}),{headers:o}):new Response(JSON.stringify({ok:!1,message:"YouTube API \uC624\uB958"}),{status:500,headers:o});let s=new Date().toISOString(),a=l.items.map((c,m)=>({rank:m+1,video_id:c.id,title:c.snippet?.title||"",channel:c.snippet?.channelTitle||"",thumbnail:c.snippet?.thumbnails?.medium?.url||c.snippet?.thumbnails?.default?.url||"",view_count:parseInt(c.statistics?.viewCount||0),collected_at:s}));await t.DB.prepare("DELETE FROM youtube_trending").run();let d=a.map(c=>t.DB.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).bind(c.rank,c.video_id,c.title,c.channel,c.thumbnail,c.view_count,c.collected_at));return await t.DB.batch(d),new Response(JSON.stringify({ok:!0,data:a,cached:!1}),{headers:o})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:o})}if(r==="/works/register"&&i.method==="POST")try{let n=await i.json(),{tmdb_id:p,title_ko:_,title_en:l,poster_path:s,media_type:a,genre:d,original_language:c,tmdb_rating:m,release_date:u}=n;if(!p||!_)return new Response(JSON.stringify({ok:!1,message:"tmdb_id, title_ko required"}),{status:400,headers:o});let E=l&&/[\uAC00-\uD7A3]/.test(l),g=l&&/[a-zA-Z]/.test(l)&&!E?l:null,y=m??null,k=u||null,O=new Date().toISOString(),R=await t.DB.prepare("SELECT tmdb_id FROM works WHERE tmdb_id = ?").bind(parseInt(p)).first(),S=null,h=null,b=a||null;if(!R){let N=a?[a]:["movie","tv"],T=!1;for(let W of N)try{let U=await fetch(`https://api.themoviedb.org/3/${W}/${parseInt(p)}/keywords?api_key=${t.TMDB_API_KEY}`);if(!U.ok)continue;T=!0;let j=await U.json(),z=j.keywords||j.results||[];if(z.length){S=z.map(V=>V.name).filter(Boolean).join(",");break}}catch{}let A=S&&S.split(",").includes("softcore"),I=T&&!S,L=/[\u3040-\u309F\u30A0-\u30FF]/.test(_||"")&&_.length>=20,F=/\bNTR\b/.test(_||"")||(_||"").includes("\u4E2D\u51FA\u3057")||(_||"").includes("\u624B\u30B3\u30AD"),M=!!(S&&["creampie","orgy","gang rape","netorare","cuckold","big tits","handjob"].some(W=>S.includes(W)));L||F||M?(h=2,b="movie"):(A||I)&&(h=1,b="movie")}return await t.DB.prepare(`
        INSERT INTO works (
    ...
    `).bind(..._).all()).results}return l.sort((s,a)=>{let d=s.original_language==="ko"?0:1,c=a.original_language==="ko"?0:1;return d!==c?d-c:(a.tmdb_rating||0)-(s.tmdb_rating||0)}),l.slice(0,f)}async function _t(r,i,t,f,e){if(r==="/works/search"&&i.method==="GET"){let o=f.searchParams.get("q")||"",n=Math.min(parseInt(f.searchParams.get("limit")||"15"),30),p=Math.max(parseInt(f.searchParams.get("offset")||"0"),0),_=["netflix","tving","disney","coupang","wavve","watcha"],l=f.searchParams.get("ott")||"",s=_.includes(l)?l:"",a=100,d=15,c=24;if(!o.trim())return new Response(JSON.stringify({ok:!1,message:"q required"}),{status:400,headers:e});try{let[m,u,E]=await t.DB.batch(ct(t,o,a)),w=new Map;if(m.results.forEach(b=>w.set(b.tmdb_id,0)),u.results.forEach(b=>{w.has(b.tmdb_id)||w.set(b.tmdb_id,1)}),E.results.forEach(b=>{w.has(b.tmdb_id)||w.set(b.tmdb_id,2)}),w.size===0){let b=o.replace(/\s+/g,""),{results:N}=await t.DB.prepare(`
        ...
        `).bind(s).all(),N=new Set(b.map(T=>T.tmdb_id));y=y.filter(T=>N.has(T))}let k=[];if(y.length){let b=y.map(()=>"?").join(",");k=(await t.DB.prepare(`
          ...
          WHERE tmdb_id IN (${b})
            AND (adult_flag IS NULL OR adult_flag NOT IN (1, 2))
            AND poster_path IS NOT NULL AND poster_path != ''
        `).bind(...y).all()).results}if(!s&&k.length<d){let b=[...new Set(o.split(/\s+/).filter(N=>N.length>=2))].slice(0,3);b.length>=2&&(await Gt(t,b,new Set(w.keys()),c)).forEach(T=>{w.set(T.tmdb_id,4),k.push(T)})}let O=k.length;k.sort((b,N)=>{let T=w.get(b.tmdb_id)??1,A=w.get(N.tmdb_id)??1;if(T!==A)return T-A;let I=b.original_language==="ko"?0:1,C=N.original_language==="ko"?0:1;return I!==C?I-C:(N.tmdb_rating||0)-(b.tmdb_rating||0)});let R=k.slice(p,p+n),S=k.length>p+n,h=[];if(R.length){let b=R.map(L=>L.tmdb_id),N=b.map(()=>"?").join(","),[{results:T},{results:A}]=await Promise.all([t.DB.prepare(`
      ...
      `).bind(...n).all(),l=[];if(_.length){let s=_.map(E=>E.tmdb_id),a=s.map(()=>"?").join(","),[{results:d},{results:c}]=await Promise.all([t.DB.prepare(`
          ...
          `).bind(...s).all()]),m={};d.forEach(E=>{m[E.tmdb_id]||(m[E.tmdb_id]={}),m[E.tmdb_id][E.platform]=E.rank});let u={};c.forEach(E=>{(u[E.tmdb_id]||=[]).push(E.ott_key)}),l=_.map(E=>({...E,ott_ranks:m[E.tmdb_id]||{},ott_keys:u[E.tmdb_id]||[]}))}return new Response(JSON.stringify({ok:!0,data:l}),{headers:e})}catch(p){return new Response(JSON.stringify({ok:!1,message:p.message}),{status:500,headers:e})}}if(r==="/search-log"&&i.method==="POST")try{let o=await i.json().catch(()=>({})),n=(o.q||"").toString().trim().slice(0,200),p=parseInt(o.total,10)||0,_=o.totalCount,l=_==null?null:String(_).slice(0,20);return n?(await t.DB.prepare("INSERT INTO search_logs (query, result_count, total_count) VALUES (?, ?, ?)").bind(n,p,l).run(),new Response(JSON.stringify({ok:!0}),{headers:e})):new Response(JSON.stringify({ok:!0}),{headers:e})}catch(o){return new Response(JSON.stringify({ok:!1,message:o.message}),{status:500,headers:e})}return null}async function pt(r,i,t,f,e){if(r==="/reactions"&&i.method==="GET"){let o=new URL(i.url),n=o.searchParams.get("tmdb_id"),p=o.searchParams.get("featured"),_=parseInt(o.searchParams.get("page")||"1"),l=20,s=(_-1)*l,a,d;p==="1"?(a="SELECT * FROM reactions WHERE is_featured = 1 ORDER BY created_at DESC LIMIT 1",d=[]):n?(a="SELECT * FROM reactions WHERE tmdb_id = ? ORDER BY is_featured DESC, like_count DESC, created_at DESC",d=[parseInt(n)]):(a="SELECT * FROM reactions ORDER BY is_featured DESC, created_at DESC LIMIT ? OFFSET ?",d=[l,s]);let{results:c}=d.length?await t.DB.prepare(a).bind(...d).all():await t.DB.prepare(a).all();return new Response(JSON.stringify({ok:!0,data:c}),{headers:e})}if(r.match(/^\/reactions\/work\/\d+$/)&&i.method==="GET")try{let o=parseInt(r.split("/")[3]),n=["great","good","meh","bad"],{results:p}=await t.DB.prepare(`
  ...
  `).bind(i).all();o.forEach(l=>e.add(l.platform));let n=[[/netflix/i,"netflix"],[/tving/i,"tving"],[/disney/i,"disney"],[/coupang/i,"coupang"],[/wavve/i,"wavve"],[/watcha/i,"watcha"]];try{if(f==="tv"&&!e.has("coupang")){let s=await fetch(`https://api.themoviedb.org/3/tv/${i}?api_key=${r.TMDB_API_KEY}`);s.ok&&((await s.json()).networks||[]).some(d=>d.id===5169)&&e.add("coupang")}let l=await fetch(`https://api.themoviedb.org/3/${f}/${i}/watch/providers?api_key=${r.TMDB_API_KEY}`);if(l.ok){let s=await l.json(),a=s.results&&s.results.KR||{};[...a.flatrate||[],...a.rent||[],...a.buy||[]].forEach(c=>{let m=n.find(([u])=>u.test(c.provider_name||""));m&&e.add(m[1])})}}catch{}let{results:p}=await r.DB.prepare("SELECT ott_key, action FROM work_ott_overrides WHERE tmdb_id = ?").bind(i).all();p.forEach(l=>{l.action==="add"?e.add(l.ott_key):l.action==="remove"&&e.delete(l.ott_key)});let _=[r.DB.prepare("DELETE FROM work_ott WHERE tmdb_id = ?").bind(i)];e.forEach(l=>{_.push(r.DB.prepare("INSERT INTO work_ott (tmdb_id, ott_key) VALUES (?, ?)").bind(i,l))}),_.push(r.DB.prepare("UPDATE works SET ott_updated_at = datetime('now') WHERE tmdb_id = ?").bind(i)),await r.DB.batch(_)}async function et(r,i,t,f,e){let o=r.match(/^\/work-ott\/(\d+)$/);if(o&&i.method==="GET"){let s=parseInt(o[1]);try{let{results:a}=await t.DB.prepare(`SELECT id, tmdb_id, ott_key, action, created_at
         ...
         DO UPDATE SET action = excluded.action,
                       created_at = datetime('now')`).bind(a,d,c).run();try{await gt(t,a)}catch{}return new Response(JSON.stringify({ok:!0}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,error:s.message}),{status:500,headers:e})}}let n=r.match(/^\/work-ott\/(\d+)$/);if(n&&i.method==="DELETE"){if(!D(i,t))return new Response(JSON.stringify({ok:!1,error:"Unauthorized"}),{status:401,headers:e});let s=parseInt(n[1]);try{let a=await t.DB.prepare("SELECT tmdb_id FROM work_ott_overrides WHERE id = ?").bind(s).first();if(await t.DB.prepare("DELETE FROM work_ott_overrides WHERE id = ?").bind(s).run(),a&&a.tmdb_id)try{await gt(t,a.tmdb_id)}catch{}return new Response(JSON.stringify({ok:!0}),{headers:e})}catch(a){return new Response(JSON.stringify({ok:!1,error:a.message}),{status:500,headers:e})}}if(r==="/admin/title-map"&&i.method==="GET"){if(!D(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(f.searchParams.get("page")||"1"),a=50,d=(s-1)*a,{results:c}=await t.DB.prepare("SELECT * FROM title_map ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(a,d).all();return new Response(JSON.stringify({ok:!0,data:c}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/rankings"&&i.method==="POST"){if(!D(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json(),{platform:a,category_slot:d,date:c,tmdb_id:m,rank:u,title_ko:E,title_en:w,media_type:g,is_manual:y}=s;if(!a||!d||!c||!m||!E)return new Response(JSON.stringify({ok:!1,message:"platform, category_slot, date, tmdb_id, title_ko \uD544\uC218"}),{status:400,headers:e});let k=null,O=E||null,R=w||null,S=null,h=null,b=null,N=g==="tv"||g==="movie"?g:null;try{let A=N?[N]:["tv","movie"];for(let I of A){let C=await fetch(`https://api.themoviedb.org/3/${I}/${m}?language=ko-KR&api_key=${t.TMDB_API_KEY}`);if(!C.ok)continue;let L=await C.json();if(!(!L.poster_path&&!L.name&&!L.title)){if(k=L.poster_path||null,S=parseInt((L.first_air_date||L.release_date||"").slice(0,4))||null,b=L.vote_average?parseFloat(L.vote_average.toFixed(1)):null,h=(L.genres||[]).map(F=>F.name).join(", ")||null,N||(N=I),O||(O=L.name||L.title||null),!R){let F=await fetch(`https://api.themoviedb.org/3/${I}/${m}?language=en-US&api_key=${t.TMDB_API_KEY}`);if(F.ok){let H=await F.json(),M=H.original_title||H.original_name||"",W=H.title||H.name||"";R=/[\uAC00-\uD7A3]/.test(M)?W:M||W}}break}}}catch{}await t.DB.prepare(`
      ...
      `).bind(parseInt(m),O||"",R||"",k,N,O||null,R||null,k,N).run();let T=parseInt(u)||null;return T||(T=((await t.DB.prepare("SELECT MAX(rank) as max_rank FROM rankings WHERE platform = ? AND category_slot = ? AND date = ?").bind(a,d,c).first())?.max_rank||0)+1),await t.DB.prepare(`
      ...
      `).bind(a,d,d,c||"",parseInt(u),g,y,parseInt(m),k,O,R,S,h,E||null,w!==null?parseInt(w):null).run();let N=new Date().toISOString();return await t.DB.prepare(`
      ...
      `).bind(...m).all(),w={};E.forEach(N=>{(w[N.tmdb_id]||=new Set).add(N.platform)});let{results:g}=await t.DB.prepare(`
        ...
        WHERE tmdb_id IN (${u})
      `).bind(...m).all(),y={};g.forEach(N=>{(y[N.tmdb_id]||=[]).push(N)});let k=[[/netflix/i,"netflix"],[/tving/i,"tving"],[/disney/i,"disney"],[/coupang/i,"coupang"],[/wavve/i,"wavve"],[/watcha/i,"watcha"]],O=0,R=0,S=[],h=[];for(let N of c){let T=N.tmdb_id,A=N.media_type==="movie"?"movie":"tv",I=new Set(w[T]||[]),C=!1;try{if(A==="tv"&&!I.has("coupang")){let F=await fetch(`https://api.themoviedb.org/3/tv/${T}?api_key=${t.TMDB_API_KEY}`);F.ok&&(C=!0,((await F.json()).networks||[]).some(M=>M.id===5169)&&I.add("coupang"))}let L=await fetch(`https://api.themoviedb.org/3/${A}/${T}/watch/providers?api_key=${t.TMDB_API_KEY}`);if(L.ok){C=!0;let F=await L.json(),H=F.results&&F.results.KR||{};[...H.flatrate||[],...H.rent||[],...H.buy||[]].forEach(W=>{let U=k.find(([j])=>j.test(W.provider_name||""));U&&I.add(U[1])})}}catch{}if(!C&&I.size===0){R++;continue}(y[T]||[]).forEach(L=>{L.action==="add"?I.add(L.ott_key):L.action==="remove"&&I.delete(L.ott_key)}),S.push(t.DB.prepare("DELETE FROM work_ott WHERE tmdb_id = ?").bind(T)),[...I].forEach(L=>{S.push(t.DB.prepare("INSERT INTO work_ott (tmdb_id, ott_key) VALUES (?, ?)").bind(T,L))}),h.push(T),O++}if(h.length){let N=h.map(()=>"?").join(",");S.push(t.DB.prepare(`UPDATE works SET ott_updated_at = datetime('now') WHERE tmdb_id IN (${N})`).bind(...h))}S.length&&await t.DB.batch(S);let b=await t.DB.prepare(`
...
${u}`,g=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":t.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:2e3,system:E,messages:[{role:"user",content:w}]})});if(!g.ok){let T=await g.text().catch(()=>"");return new Response(JSON.stringify({ok:!1,message:`Claude API \uC624\uB958 (status ${g.status})`,detail:T.slice(0,300)}),{status:502,headers:e})}let k=((await g.json()).content||[]).filter(T=>T.type==="text").map(T=>T.text).join(""),O;try{let T=k.replace(/```json|```/g,"").trim();O=JSON.parse(T)}catch{return new Response(JSON.stringify({ok:!1,message:"Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694",raw:k.slice(0,300)}),{status:502,headers:e})}Array.isArray(O)||(O=[]);let R=new Set(c),S=new Map;for(let T of O){let A=parseInt(T.tmdb_id);if(!A)continue;let I=Array.isArray(T.tags)?T.tags.filter(C=>R.has(C)).slice(0,2):[];S.set(A,I)}let h=[],b=0;for(let T of m){if(!S.has(T.tmdb_id))continue;let A=S.get(T.tmdb_id);h.push(t.DB.prepare("UPDATE works SET variety_genre = ?, variety_genre_source = 'auto' WHERE tmdb_id = ?").bind(A.length?A.join(","):null,T.tmdb_id)),b++}h.length&&await t.DB.batch(h);let N=await t.DB.prepare(`
      ...
      `).bind(a,d,d,a).run(),new Response(JSON.stringify({ok:!0}),{headers:e}))}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/persons/collect"&&i.method==="POST"){if(!D(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=Math.min(parseInt(s.limit)||20,50),{results:d}=await t.DB.prepare(`
        ...
        LIMIT ?
      `).bind(a).all();if(!d.length)return new Response(JSON.stringify({ok:!0,worksScanned:0,personsFound:0,remaining:0,message:"\uC2A4\uCE94\uD560 \uC791\uD488 \uC5C6\uC74C"}),{headers:e});let c=new Map,m=[];for(let w of d){m.push(w.tmdb_id);let g=w.media_type==="tv"?"tv":"movie",y=g==="tv"?"aggregate_credits":"credits";try{let k=await fetch(`https://api.themoviedb.org/3/${g}/${w.tmdb_id}/${y}?api_key=${t.TMDB_API_KEY}`);if(!k.ok)continue;let O=await k.json();for(let R of(O.cast||[]).slice(0,15))R.id&&R.name&&!c.has(R.id)&&c.set(R.id,{name:R.name,job:"act",popularity:R.popularity||null,profile_path:R.profile_path||null});for(let R of O.crew||[])(R.job==="Director"||R.job==="Creator"||R.department==="Directing"||(R.jobs||[]).some(h=>h.job==="Director"||h.job==="Creator"))&&R.id&&R.name&&c.set(R.id,{name:R.name,job:"direct",popularity:R.popularity||null,profile_path:R.profile_path||null})}catch{}}let u=[];for(let[w,g]of c)u.push(t.DB.prepare(`INSERT INTO persons (tmdb_id, name, job, popularity, profile_path) VALUES (?, ?, ?, ?, ?)
      ...
      `).bind(a,d).all(),w=await t.DB.prepare(`
        ...
        LEFT JOIN person_wiki_cache w ON w.tmdb_person_id = p.tmdb_id
        WHERE w.tmdb_person_id IS NULL ${u}
      `).first();return new Response(JSON.stringify({ok:!0,items:E,total:w?.cnt||0,offset:d,limit:a}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/persons/wiki-match-attempt"&&i.method==="POST"){if(!D(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=await i.json().catch(()=>({})),a=Array.isArray(s.tmdb_ids)?s.tmdb_ids.map(w=>parseInt(w)).filter(w=>Number.isInteger(w)).slice(0,50):[];if(!a.length)return new Response(JSON.stringify({ok:!1,message:"tmdb_ids\uAC00 \uD544\uC694\uD574\uC694"}),{status:400,headers:e});let d=a.map(()=>"?").join(","),{results:c}=await t.DB.prepare(`SELECT tmdb_id, name, name_ko, birthday, popularity, profile_path FROM persons WHERE tmdb_id IN (${d})`).bind(...a).all(),m=[],u=[],E={"User-Agent":"OttrankBot/1.0 (https://ottrank.kr; \uC624\uB728\uB791 \uC778\uBB3C \uC704\uD0A4\uB9E4\uCE6D)"};for(let w of c){let g=w.name_ko||w.name,y=(w.birthday||"").slice(0,4),k=null,O=null;try{let R=await fetch(`https://ko.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(g)}&limit=5&namespace=0&format=json`,{headers:E});if(!R.ok)O=`\uAC80\uC0C9 \uC694\uCCAD \uC2E4\uD328 (HTTP ${R.status})`;else{let S=await R.json(),h=S[1]||[],b=S[3]||[];h.length||(O="\uC704\uD0A4\uBC31\uACFC \uAC80\uC0C9 \uACB0\uACFC \uC790\uCCB4\uAC00 \uC5C6\uC74C");let N=`${g} (\uBC30\uC6B0)`;h.includes(N)||(h.unshift(N),b.unshift(`https://ko.wikipedia.org/wiki/${encodeURIComponent(N.replace(/ /g,"_"))}`));for(let T=0;T<h.length;T++){let A=h[T],I=b[T],C=await fetch(`https://ko.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(A)}&prop=extracts&exintro=1&explaintext=1&format=json`,{headers:E});if(!C.ok){O=`\uBCF8\uBB38 \uC870\uD68C \uC2E4\uD328 (HTTP ${C.status}, \uD6C4\uBCF4: ${A})`;continue}let L=await C.json(),F=L.query&&L.query.pages||{},H=Object.values(F)[0],M=H&&H.extract||"",W=M.match(/(\d{4})년/),U=W?W[1]:null,j=y&&U&&y===U,z=!y&&h.length===1;if(j||z){k={wiki_title:A,wiki_birth_year:U||"",wiki_summary:M.slice(0,200),wiki_source_url:I};break}T===h.length-1&&!k&&(O=`\uD6C4\uBCF4 ${h.length}\uAC1C \uD655\uC778\uD568(${h.join(", ")}) \u2014 \uC0DD\uB144\uB3C4 \uC77C\uCE58\uD558\uB294 \uD6C4\uBCF4 \uC5C6\uC74C(TMDB: ${y||"\uC5C6\uC74C"})`)}}}catch(R){O=`\uC694\uCCAD \uC911 \uC624\uB958: ${R.message}`}m.push({tmdb_id:w.tmdb_id,name_ko:w.name_ko||w.name,found:!!k,wiki_title:k?k.wiki_title:null,wiki_birth_year:k?k.wiki_birth_year:null,wiki_summary:k?k.wiki_summary:null,wiki_source_url:k?k.wiki_source_url:null,debug:k?null:O}),u.push(t.DB.prepare(`
...
`)[0].slice(0,500),T=b.match(/==+\s*수상[^=\n]*==+\n*([\s\S]*?)(?=\n==+\s|$)/),A=T?T[1].trim():"",I=A&&!/^==/.test(A)?A.slice(0,2e3):null,C=h&&h.revisions||[],F=(C[0]&&C[0].slots&&C[0].slots.main&&C[0].slots.main["*"]||"").match(/\{\{(?:배우 정보|연예인 정보|영화인 정보)[\s\S]*?\n\}\}/),H=F?F[0]:"",M=null,W=null,U=null;if(H){let Y=H.match(/\|\s*데뷔(?:작|년도)?(?:\([^)]*\))?\s*=\s*([^\|\n]+)/);if(Y){let X=w(Y[1]),Q=X.match(/(\d{4})/);W=Q?Q[1]:null,M=X.replace(/\(?\d{4}\)?[년,\s]*/g,"").trim().slice(0,100)||null}let x=H.match(/\|\s*학력(?:\([^)]*\))?\s*=\s*([^\|\n]+)/);x&&(U=w(x[1]).slice(0,200)||null)}if(!U){let Y=b.match(/==+\s*학력[^=\n]*==+\n*([\s\S]*?)(?=\n==+\s|$)/),x=Y?Y[1].trim():"";x&&!/^==/.test(x)&&(U=w(x.replace(/\n+/g," \xB7 ")).slice(0,300)||null)}let j=g(b).slice(0,8e3),z=h&&h.extlinks||[],V=null,q=null;for(let Y of z){let x=Y["*"]||"",X=x.match(/kmdb\.or\.kr\/db\/per\/(\d+)/);X&&(V=X[1]);let Q=x.match(/imdb\.com\/name\/(nm\d+)/);Q&&(q=Q[1])}E.push(t.DB.prepare(`
      ...
      `).bind(`%${s}%`,`%${s}%`).all();return new Response(JSON.stringify({ok:!0,items:a}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r.match(/^\/admin\/persons\/wiki-detail\/\d+$/)&&i.method==="GET"){if(!D(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=parseInt(r.split("/")[4]),a=await t.DB.prepare("SELECT tmdb_id, name, name_ko, birthday, popularity, profile_path FROM persons WHERE tmdb_id = ?").bind(s).first();if(!a)return new Response(JSON.stringify({ok:!1,message:"\uC778\uBB3C\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC5B4\uC694"}),{status:404,headers:e});let d=await t.DB.prepare(`SELECT tmdb_person_id, wiki_title, bio_summary, career_history, awards_text,
         ...
         FROM person_wiki_cache WHERE tmdb_person_id = ?`).bind(s).first();if(!d)return new Response(JSON.stringify({ok:!0,matched:!1,person:a,wiki:{wiki_title:null,bio_summary:null,career_history:null,awards_text:null,debut_work:null,debut_year:null,education:null,kmdb_id:null,imdb_id:null,source_url:null},hiddenFields:[]}),{headers:e});let c=(d.hidden_fields||"").split(",").map(m=>m.trim()).filter(Boolean);return delete d.hidden_fields,new Response(JSON.stringify({ok:!0,matched:!0,person:a,wiki:d,hiddenFields:c}),{headers:e})}catch(s){return new Response(JSON.stringify({ok:!1,message:s.message}),{status:500,headers:e})}}if(r==="/admin/persons/wiki-manual-save"&&i.method==="POST"){if(!D(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:e});try{let s=["bio_summary","career_history","awards_text","debut_work","education","kmdb_id","imdb_id"],a=await i.json().catch(()=>({})),d=parseInt(a.tmdb_id);if(!Number.isInteger(d))return new Response(JSON.stringify({ok:!1,message:"tmdb_id\uAC00 \uD544\uC694\uD574\uC694"}),{status:400,headers:e});let c=b=>typeof b=="string"&&b.trim()!==""?b.trim():null,m=c(a.wiki_title),u=c(a.bio_summary),E=c(a.career_history),w=c(a.awards_text),g=c(a.debut_work),y=c(a.debut_year),k=c(a.education),O=c(a.kmdb_id),R=c(a.imdb_id),S=c(a.source_url),h=Array.isArray(a.hidden_fields)?a.hidden_fields.filter(b=>s.includes(b)):[];return await t.DB.prepare(`
        INSERT INTO person_wiki_cache
          (tmdb_person_id, wiki_title, bio_summary, career_history, awards_text,
           debut_work, debut_year, education, kmdb_id, imdb_id, source_url, hidden_fields)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  `).bind(r,`https://www.youtube.com/watch?v=${t}`,t,f||"").run(),console.log(`[CONTENTS_LINK] \u2705 tmdb_id=${r} youtube_id=${t} title_videos \uBCF5\uC0AC \uC644\uB8CC`)}async function Xt(r,i,t){let f=r.searchParams.get("platform"),e=r.searchParams.get("type"),o=Math.min(parseInt(r.searchParams.get("limit")||"20"),50),n=["is_hidden = 0"],p=[];f&&(n.push("platform = ?"),p.push(f)),e&&(n.push("type = ?"),p.push(e));let _=n.join(" AND ");p.push(o);let{results:l}=await i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
     ...
     LIMIT ?`).bind(...p).all();return J({ok:!0,items:l??[]},200,t)}async function Qt(r,i){let{results:t}=await r.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
            tmdb_id, tmdb_type, thumbnail, published_at, view_count,
            is_pinned, sort_order
     FROM ott_contents
     WHERE is_pinned = 1 AND is_hidden = 0
     ORDER BY sort_order ASC
     LIMIT 5`).all();return J({ok:!0,items:t??[]},200,i)}async function Zt(r,i,t){let f=r.searchParams.get("platform"),e=r.searchParams.get("type"),o=Math.max(parseInt(r.searchParams.get("page")||"1"),1),n=30,p=(o-1)*n,_=["is_hidden = 0"],l=[];f&&(_.push("platform = ?"),l.push(f)),e&&(_.push("type = ?"),l.push(e));let s=_.join(" AND "),a=[...l],d=[...l,n,p],[c,m]=await i.DB.batch([i.DB.prepare(`SELECT COUNT(*) as total FROM ott_contents WHERE ${s}`).bind(...a),i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
              tmdb_id, tmdb_type, thumbnail, published_at, view_count
       FROM ott_contents
       WHERE ${s}
       ORDER BY published_at DESC
       LIMIT ? OFFSET ?`).bind(...d)]),u=c.results?.[0]?.total??0,E=m.results??[];return J({ok:!0,items:E,pagination:{page:o,pageSize:n,total:u,totalPages:Math.ceil(u/n)}},200,t)}async function vt(r,i,t){let f=await i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
            tmdb_id, tmdb_type, thumbnail, published_at, view_count,
     ...
     VALUES (?, ?, ?)`).bind(l,p.id,s.trim()).run();return J({ok:!0,id:d.meta?.last_row_id},200,t)}async function ee(r,i,t,f){let e=i.headers.get("Authorization")||"",o=e.startsWith("Bearer ")?e.slice(7).trim():null,n=B(i),p=o||n;if(!p)return J({ok:!1,error:"\uB85C\uADF8\uC778\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."},401,f);let _=await t.DB.prepare("SELECT s.user_id AS id FROM sessions s WHERE s.id = ? LIMIT 1").bind(p).first();if(!_)return J({ok:!1,error:"\uB85C\uADF8\uC778\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."},401,f);let l=await t.DB.prepare("SELECT id, user_id FROM ott_content_comments WHERE id = ?").bind(r).first();return l?l.user_id!==_.id?J({ok:!1,error:"\uBCF8\uC778 \uB313\uAE00\uB9CC \uC0AD\uC81C\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4."},403,f):(await t.DB.prepare("DELETE FROM ott_content_comments WHERE id = ?").bind(r).run(),J({ok:!0},200,f)):J({ok:!1,error:"\uB313\uAE00\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4."},404,f)}async function se(r,i,t,f){if(!K(i,t))return J({ok:!1,error:"\uAD8C\uD55C\uC774 \uC5C6\uC2B5\uB2C8\uB2E4."},403,f);let e=r.searchParams.get("platform"),o=r.searchParams.get("type"),n=(r.searchParams.get("q")||"").trim(),p=Math.max(parseInt(r.searchParams.get("page")||"1"),1),_=50,l=(p-1)*_,s=["1=1"],a=[];if(e&&(s.push("platform = ?"),a.push(e)),o&&(s.push("type = ?"),a.push(o)),n){let y=n.replace(/\s+/g,"");s.push("(REPLACE(work_title, ' ', '') LIKE ? OR REPLACE(title, ' ', '') LIKE ?)"),a.push(`%${y}%`,`%${y}%`)}let d=s.join(" AND "),c=[...a],m=[...a,_,l],[u,E]=await t.DB.batch([t.DB.prepare(`SELECT COUNT(*) as total FROM ott_contents WHERE ${d}`).bind(...c),t.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(e,o,n,p,_||null,l||null,s||null,a||null,d).run();if(l&&_)try{await i.DB.prepare(`INSERT OR IGNORE INTO works (tmdb_id, media_type, title_ko, match_source)
           VALUES (?, ?, ?, 'crawler')`).bind(l,s||null,_).run()}catch(E){console.error("[contents] works \uC790\uB3D9\uB4F1\uB85D \uC2E4\uD328(\uBB34\uC2DC):",E.message)}if(l)try{await wt(l,s||null,e,p,i)}catch(E){console.error("[contents] title_videos \uBCF5\uC0AC \uC2E4\uD328(\uBB34\uC2DC):",E.message)}return J({ok:!0,id:u.meta?.last_row_id},200,t)}catch(u){if(u.message?.includes("UNIQUE"))return J({ok:!1,error:"\uC774\uBBF8 \uB4F1\uB85D\uB41C YouTube \uC601\uC0C1\uC785\uB2C8\uB2E4."},409,t);throw u}}async function re(r,i,t,f){if(!K(i,t))return J({ok:!1,error:"\uAD8C\uD55C\uC774 \uC5C6\uC2B5\uB2C8\uB2E4."},403,f);let e;try{e=await i.json()}catch{return J({ok:!1,error:"\uC798\uBABB\uB41C \uC694\uCCAD \uD615\uC2DD\uC785\uB2C8\uB2E4."},400,f)}let o=await t.DB.prepare("SELECT id, youtube_id, title, tmdb_type FROM ott_contents WHERE id = ?").bind(r).first();if(!o)return J({ok:!1,error:"\uC601\uC0C1\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4."},404,f);let n=["work_title","tmdb_id","tmdb_type","type","is_pinned","is_hidden","sort_order"],p=[],_=[];for(let l of n)e[l]!==void 0&&(p.push(`${l} = ?`),_.push(e[l]));if(p.length===0)return J({ok:!1,error:"\uC218\uC815\uD560 \uAC12\uC774 \uC5C6\uC2B5\uB2C8\uB2E4."},400,f);if(_.push(r),await t.DB.prepare(`UPDATE ott_contents SET ${p.join(", ")} WHERE id = ?`).bind(..._).run(),e.tmdb_id!==void 0)try{let l=e.tmdb_type!==void 0?e.tmdb_type:o.tmdb_type;await wt(e.tmdb_id,l,o.youtube_id,o.title,t)}catch(l){console.error("[contents] title_videos \uBCF5\uC0AC \uC2E4\uD328(\uBB34\uC2DC):",l.message)}return J({ok:!0},200,f)}async function ne(r,i,t,f){return K(i,t)?await t.DB.prepare("SELECT id FROM ott_contents WHERE id = ?").bind(r).first()?(await t.DB.prepare("DELETE FROM ott_contents WHERE id = ?").bind(r).run(),J({ok:!0},200,f)):J({ok:!1,error:"\uC601\uC0C1\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4."},404,f):J({ok:!1,error:"\uAD8C\uD55C\uC774 \uC5C6\uC2B5\uB2C8\uB2E4."},403,f)}async function oe(r,i,t){if(!K(r,i))return J({ok:!1,error:"\uAD8C\uD55C\uC774 \uC5C6\uC2B5\uB2C8\uB2E4."},403,t);let f;try{f=await r.json()}catch{return J({ok:!1,error:"\uC798\uBABB\uB41C \uC694\uCCAD \uD615\uC2DD\uC785\uB2C8\uB2E4."},400,t)}let{ordered_ids:e}=f;if(!Array.isArray(e)||e.length===0)return J({ok:!1,error:"ordered_ids \uBC30\uC5F4\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."},400,t);if(e.length>5)return J({ok:!1,error:"\uACE0\uC815 \uC601\uC0C1\uC740 \uCD5C\uB300 5\uAC1C\uC785\uB2C8\uB2E4."},400,t);let o=[i.DB.prepare("UPDATE ott_contents SET is_pinned = 0, sort_order = 0"),...e.map((n,p)=>i.DB.prepare("UPDATE ott_contents SET is_pinned = 1, sort_order = ? WHERE id = ?").bind(p+1,n))];return await i.DB.batch(o),J({ok:!0},200,t)}var Ot="https://api.anthropic.com/v1/messages",kt="https://ottrank.kr",G={netflix:"\uB137\uD50C\uB9AD\uC2A4",tving:"\uD2F0\uBE59",wavve:"\uC6E8\uC774\uBE0C",disney:"\uB514\uC988\uB2C8+",coupang:"\uCFE0\uD321\uD50C\uB808\uC774",boxoffice:"\uBC15\uC2A4\uC624\uD53C\uC2A4"},yt={friendly:`\uB124\uC774\uBC84 \uBE14\uB85C\uADF8 \uAC10\uC131 \uB9D0\uD22C. \uC9E7\uC740 \uC904\uBC14\uAFC8, \uBCF8\uC778 \uC598\uAE30\uB85C \uC2DC\uC791, \uB3C5\uC790\uC5D0\uAC8C \uB9D0 \uAC70\uB294 \uB290\uB08C.
\uC608\uC2DC (\uC774 \uAD6C\uC870 \uADF8\uB300\uB85C \uB530\uB77C\uD560 \uAC83):
...
${b.length>0?`[\uCD94\uAC00 \uC9C0\uC2DC\uC0AC\uD56D]
`+b.map((A,I)=>`${I+1}. ${A}`).join(`
`):""}

\uB9C8\uD06C\uB2E4\uC6B4 \uAE30\uD638 \uC5C6\uC774 \uC77C\uBC18 \uD14D\uC2A4\uD2B8\uB85C, \uB2E8\uB77D \uAD6C\uBD84\uC740 \uBE48 \uC904\uB85C\uB9CC \uD574\uC8FC\uC138\uC694.`,T=await le(N,o,{useWebSearch:S,maxTokens:S?5e3:4096});if(!T)throw new Error("AI \uC751\uB2F5\uC774 \uBE44\uC5B4\uC788\uC2B5\uB2C8\uB2E4. \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694.");return new Response(JSON.stringify({ok:!0,post:T,rankingData:g,meta:{platform:p,platformName:O,weekInfo:k,categorySlot:w||"all",categoryLabel:w&&g.length===1?g[0].display_name:"\uC804\uCCB4",generatedAt:new Date().toISOString()}}),{headers:e})}catch(g){return new Response(JSON.stringify({ok:!1,error:g.message}),{status:500,headers:e})}}return null}var ce=["ad","bug"],_e=["pending","answered","resolved"],pe=5,Tt=30;async function ht(r,i,t,f,e,o){if(r==="/inquiry"&&i.method==="POST")try{let _=await i.json(),{type:l,name:s,email:a,phone:d,title:c,content:m,page_url:u,website:E}=_;if(E)return new Response(JSON.stringify({ok:!0}),{headers:o});if(!ce.includes(l))return new Response(JSON.stringify({ok:!1,message:"type\uC740 ad \uB610\uB294 bug\uC5EC\uC57C \uD569\uB2C8\uB2E4"}),{status:400,headers:o});if(!c||!c.trim()||!m||!m.trim())return new Response(JSON.stringify({ok:!1,message:"\uC81C\uBAA9\uACFC \uB0B4\uC6A9\uC740 \uD544\uC218\uC785\uB2C8\uB2E4"}),{status:400,headers:o});if(l==="ad"){if(!s||!s.trim())return new Response(JSON.stringify({ok:!1,message:"\uB2F4\uB2F9\uC790\uBA85 \uB610\uB294 \uC5C5\uCCB4\uBA85\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o});if(!a||!a.trim())return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:o})}if(a&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a))return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C \uD615\uC2DD\uC774 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4"}),{status:400,headers:o});let w=l,g=String(c).slice(0,200),y=String(m).slice(0,5e3),k=s?String(s).slice(0,100):null,O=a?String(a).slice(0,200):null,R=d?String(d).slice(0,30):null,S=u?String(u).slice(0,500):null,h=i.headers.get("User-Agent")||null,b=i.headers.get("CF-Connecting-IP")||null,N=null;try{let T=i.headers.get("Authorization")||"",I=(T.startsWith("Bearer ")?T.slice(7).trim():null)||B(i);if(I){let C=await t.DB.prepare("SELECT user_id AS id FROM sessions WHERE id = ? LIMIT 1").bind(I).first();C&&(N=C.id)}}catch{}return b&&((await t.DB.prepare(`SELECT COUNT(*) as cnt FROM inquiries
    ...
    `,{results:p}=await i.DB.prepare(n).all(),{results:_}=await i.DB.prepare("SELECT tmdb_id, boost_value, is_pinned, pinned_score, pinned_platform FROM admin_boosts").all(),l=new Map((_||[]).map(w=>[w.tmdb_id,w]));if((!p||p.length===0)&&l.size===0)return new Response(JSON.stringify({ok:!1,error:"\uACC4\uC0B0\uD560 \uB7AD\uD0B9 \uB370\uC774\uD130\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4."}),{status:404,headers:t});let s=[],a=new Set;for(let w of p||[]){a.add(w.tmdb_id);let g=l.get(w.tmdb_id);g&&g.is_pinned?s.push({tmdb_id:w.tmdb_id,best_platform:g.pinned_platform||w.best_platform,best_rank:w.best_rank,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:g.pinned_score??0}):s.push(w)}for(let[w,g]of l)a.has(w)||(g.is_pinned?s.push({tmdb_id:w,best_platform:g.pinned_platform||"manual",best_rank:null,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:g.pinned_score??0}):g.boost_value&&s.push({tmdb_id:w,best_platform:g.pinned_platform||"manual",best_rank:null,rank_score:0,platform_weight:0,weighted_score:0,admin_boost:g.boost_value}));s.sort((w,g)=>g.weighted_score+g.admin_boost-(w.weighted_score+w.admin_boost));let d=new Date(Date.now()+540*60*1e3).toISOString().slice(0,19).replace("T"," "),c=[i.DB.prepare("DELETE FROM hot100_scores")];for(let w of s){let g=w.weighted_score+w.admin_boost;c.push(i.DB.prepare(`INSERT INTO hot100_scores
           ...
           VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?)`).bind(w.tmdb_id,o,w.best_platform,w.platform_weight,w.rank_score,w.weighted_score,w.admin_boost,g,d))}await i.DB.batch(c);let m=s.filter(w=>w.best_platform==="netflix").slice(0,20),u=0;if(m.length>0){let w=m.map(R=>R.tmdb_id),g=w.map(()=>"?").join(","),{results:y}=await i.DB.prepare(`SELECT tmdb_id, title_ko, title_en, poster_path, genre, tmdb_rating, release_year
    ...
    `,{results:p}=await i.DB.prepare(n).bind(o).all();return!p||p.length===0?new Response(JSON.stringify({ok:!0,data:[]}),{status:200,headers:t}):new Response(JSON.stringify({ok:!0,data:p.map((_,l)=>({hot_rank:l+1,..._}))}),{status:200,headers:t})}catch(f){return console.error("getHot100 \uC624\uB958:",f),new Response(JSON.stringify({ok:!1,error:"HOT100 \uC870\uD68C \uC911 \uC624\uB958\uAC00 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4.",detail:f.message}),{status:500,headers:t})}}async function Ft(r,i,t){if(!await D(r,i))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:t});try{let{results:e}=await i.DB.prepare(`SELECT platform, category_slot, top_n, display_order, is_active
       ...
       ORDER BY display_order ASC`).all();return new Response(JSON.stringify({ok:!0,data:e||[]}),{status:200,headers:t})}catch(e){return new Response(JSON.stringify({ok:!1,error:e.message}),{status:500,headers:t})}}async function Wt(r,i,t,f){if(!await D(i,t))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:f});try{let o=await i.json(),{category_slot:n,top_n:p,display_order:_,is_active:l}=o;return await t.DB.prepare(`UPDATE hot100_frontend_tabs SET
         category_slot = COALESCE(?, category_slot),
         top_n         = COALESCE(?, top_n),
         display_order = COALESCE(?, display_order),
         is_active     = COALESCE(?, is_active)
       WHERE platform = ?`).bind(n??null,p??null,_??null,l??null,r).run(),new Response(JSON.stringify({ok:!0}),{status:200,headers:f})}catch(o){return new Response(JSON.stringify({ok:!1,error:o.message}),{status:500,headers:f})}}async function Ut(r,i,t){try{let e=new URL(r.url).searchParams.get("page"),o=`
    ...
    `,n;if(e){let[c,m]=await i.DB.batch([i.DB.prepare("SELECT is_active FROM hot100_page_display WHERE page = ?").bind(e),i.DB.prepare(o)]),u=c.results[0]||null;if(!u||!u.is_active)return new Response(JSON.stringify({ok:!0,active:!1,tabs:[]}),{status:200,headers:t});n=m.results}else{let{results:c}=await i.DB.prepare(o).all();n=c}let p={all:"\uC804\uCCB4 \uC21C\uC704",netflix:"\uB137\uD50C\uB9AD\uC2A4",tving:"\uD2F0\uBE59",disney:"\uB514\uC988\uB2C8+",coupang:"\uCFE0\uD321\uD50C\uB808\uC774",wavve:"\uC6E8\uC774\uBE0C",boxoffice:"\uBC15\uC2A4\uC624\uD53C\uC2A4"},_=n;if(!_||_.length===0)return new Response(JSON.stringify({ok:!0,active:!0,tabs:[]}),{status:200,headers:t});let l=[];for(let c of _){let m=c.top_n||10;if(c.platform==="all"){l.push(i.DB.prepare(`SELECT h.tmdb_id, h.best_platform, w.title_ko, w.title_en,
       ...
       JOIN works w ON w.tmdb_id = h.tmdb_id
       WHERE COALESCE(w.hero_title_baked_in, 0) = 0
         AND w.hero_logo_checked_at IS NULL`).first();return new Response(JSON.stringify({ok:!0,remaining:e?.cnt??0}),{status:200,headers:t})}catch(e){return new Response(JSON.stringify({ok:!1,error:e.message}),{status:500,headers:t})}}async function xt(r,i,t){if(!await D(r,i))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:t});try{let{results:e}=await i.DB.prepare("SELECT page, is_active FROM hot100_page_display ORDER BY page ASC").all();return new Response(JSON.stringify({ok:!0,data:e||[]}),{status:200,headers:t})}catch(e){return new Response(JSON.stringify({ok:!1,error:e.message}),{status:500,headers:t})}}async function jt(r,i,t,f){if(!await D(i,t))return new Response(JSON.stringify({ok:!1,error:"\uAD00\uB9AC\uC790 \uC778\uC99D\uC774 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:401,headers:f});try{let o=await i.json(),{is_active:n}=o;return await t.DB.prepare("UPDATE hot100_page_display SET is_active = ? WHERE page = ?").bind(n?1:0,r).run(),new Response(JSON.stringify({ok:!0}),{status:200,headers:f})}catch(o){return new Response(JSON.stringify({ok:!1,error:o.message}),{status:500,headers:f})}}async function Yt(r,i,t){try{let e=new URL(r.url).searchParams.get("page");if(!e)return new Response(JSON.stringify({ok:!1,error:"page \uD30C\uB77C\uBBF8\uD130\uAC00 \uD544\uC694\uD569\uB2C8\uB2E4."}),{status:400,headers:t});let o=await i.DB.prepare("SELECT is_active FROM hot100_page_display WHERE page = ?").bind(e).first();return new Response(JSON.stringify({ok:!0,is_active:!!(o&&o.is_active)}),{status:200,headers:t})}catch(f){return new Response(JSON.stringify({ok:!1,error:f.message}),{status:500,headers:t})}}var ds={async fetch(r,i,t){let f=new URL(r.url),e=f.pathname,o=r.headers.get("Origin")||"https://ottrank.kr",p=["https://ottrank.kr","http://localhost:8788","http://localhost:3000"].includes(o)?o:"https://ottrank.kr",_={"Content-Type":"application/json","Access-Control-Allow-Origin":p,"Access-Control-Allow-Credentials":"true"};if(r.method==="OPTIONS")return new Response(null,{headers:{"Access-Control-Allow-Origin":p,"Access-Control-Allow-Credentials":"true","Access-Control-Allow-Methods":"GET, POST, PUT, PATCH, DELETE, OPTIONS","Access-Control-Allow-Headers":"Content-Type, Authorization"}});let l=null;if(e.startsWith("/tmdb-img/"))return await Dt(e,r,i,t);(e.startsWith("/contents")||e.startsWith("/admin/contents"))&&(l=await Et(e,r,i,f,_)),!l&&e.startsWith("/auth/")&&(l=await mt(e,r,i,_)),!l&&(e.startsWith("/rankings")||e==="/latest-date"||e==="/platforms"||e==="/sitemap.xml")&&(l=await at(e,r,i,f,_)),!l&&(e==="/works/search"||e==="/works/exists"||e==="/works/ott-map"||e==="/works/details"||e==="/search-log")&&(l=await _t(e,r,i,f,_)),!l&&e.startsWith("/person-wiki")&&(l=await It(e,r,i,f,_)),!l&&(e.startsWith("/videos/")||e.startsWith("/admin/videos")||e.startsWith("/imdb/")||e.startsWith("/youtube/")||e.startsWith("/works/")||e.startsWith("/kmrb/")||e.startsWith("/search/"))&&(l=await lt(e,r,i,t,f,_)),!l&&(e.startsWith("/reactions")||e.startsWith("/admin/reactions"))&&(l=await pt(e,r,i,t,_)),!l&&(e.startsWith("/wishlist")||e.startsWith("/reviews")||e.startsWith("/mypage")||e.startsWith("/user/")||e==="/grade-settings"||e.startsWith("/life-works")||e.startsWith("/pick-lists")||e.startsWith("/admin/reviews"))&&(l=await ut(e,r,i,t,_)),!l&&e.startsWith("/posts")&&(l=await ft(e,r,i,t,f,_)),!l&&e.startsWith("/blog-gen")&&(l=await Nt(e,r,i,f,_)),!l&&e.startsWith("/work-ott")&&(l=await et(e,r,i,f,_)),!l&&(e==="/inquiry"||e.startsWith("/admin/inquiry"))&&(l=await ht(e,r,i,t,f,_)),!l&&e==="/admin/calc-hot100"&&(l=await At(r,i,_)),!l&&e==="/hot100"&&(l=await Ht(r,i,_)),!l&&e==="/hot100/hero-tabs"&&(l=await Ut(r,i,_)),!l&&e==="/admin/hot100/boosts/search"&&r.method==="GET"&&(l=await Bt(r,i,_)),!l&&e==="/admin/hot100/boosts"&&r.method==="GET"&&(l=await Ct(r,i,_)),!l&&e==="/admin/hot100/boosts"&&r.method==="POST"&&(l=await Jt(r,i,_));let s=e.match(/^\/admin\/hot100\/boosts\/(\d+)$/);!l&&s&&r.method==="DELETE"&&(l=await Mt(parseInt(s[1],10),r,i,_)),!l&&e==="/admin/hot100/frontend-tabs"&&r.method==="GET"&&(l=await Ft(r,i,_));let a=e.match(/^\/admin\/hot100\/frontend-tabs\/([a-z]+)$/);!l&&a&&r.method==="PATCH"&&(l=await Wt(a[1],r,i,_)),!l&&e==="/admin/hot100/backfill-logos"&&r.method==="POST"&&(l=await Pt(r,i,_)),!l&&e==="/admin/hot100/backfill-logos/status"&&r.method==="GET"&&(l=await $t(r,i,_)),!l&&e==="/admin/hot100/page-display"&&r.method==="GET"&&(l=await xt(r,i,_));let d=e.match(/^\/admin\/hot100\/page-display\/([a-z]+)$/);return!l&&d&&r.method==="PATCH"&&(l=await jt(d[1],r,i,_)),!l&&e==="/hot100/page-display"&&r.method==="GET"&&(l=await Yt(r,i,_)),!l&&e.startsWith("/admin/")&&(l=await et(e,r,i,f,_)),l||(l=new Response(JSON.stringify({ok:!1,message:"Not found"}),{status:404,headers:_})),l}};export{ds as default};
