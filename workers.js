...
`),m=(await(await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":g.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:4e3,messages:[{role:"user",content:a}]})})).json()).content?.[0]?.text||"[]",_=[];try{let u=m.split("```json").join("").split("```").join("").trim(),f=JSON.parse(u);_=Array.isArray(f)?f:[]}catch{console.error("[REACTION] Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328:",m.slice(0,300)),_=[]}await g.DB.prepare("DELETE FROM reaction_comments WHERE reaction_id = ?").bind(o).run();for(let u=0;u<e.length;u++){let f=e[u],E=(_.find(w=>w.idx===u)||_.find(w=>w.idx===u+1)||_[u]||{}).translated||"";await g.DB.prepare(`
      ...
      `).bind(o,t,f.text.slice(0,1e3),E.slice(0,1e3),f.author.slice(0,100),f.likes,"neutral").run()}console.log(`[REACTION] \u2705 \uC644\uB8CC: reaction=${o} \uB313\uAE00 ${e.length}\uAC1C \uC800\uC7A5`)}catch(s){console.error("[REACTION] \uC624\uB958:",s.message)}}async function v(o,i,t,g,s,l){if(o.startsWith("/videos/")&&!o.includes("/admin")&&i.method==="GET"){let n=parseInt(o.split("/videos/")[1]);if(!n)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:l});try{let{results:d}=await t.DB.prepare("SELECT * FROM title_videos WHERE tmdb_id = ? ORDER BY is_main DESC, created_at DESC").bind(n).all();return d.length===0&&g.waitUntil(Z(n,t)),new Response(JSON.stringify({ok:!0,data:d}),{headers:l})}catch(d){return new Response(JSON.stringify({ok:!1,message:d.message}),{status:500,headers:l})}}if(o==="/admin/videos/crawl"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:l});try{let n=await i.json(),{tmdb_id:d}=n;if(!d)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:l});let e=await X(parseInt(d),t);return new Response(JSON.stringify({ok:!0,saved:e}),{headers:l})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:l})}}if(o==="/admin/videos/batch-crawl"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:l});try{let n=20;try{let E=await i.json();E?.limit&&Number.isInteger(E.limit)&&E.limit>0&&(n=E.limit)}catch{}let d=30,r=(await t.DB.prepare("SELECT COUNT(*) AS cnt FROM works WHERE yt_crawl_attempted_at >= date('now')").first())?.cnt||0;if(r>=d){let E=await t.DB.prepare(`
        ...
        `).first();return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:E?.cnt||0,message:`\uC624\uB298 \uC608\uC0B0(${d}\uAC1C) \uC18C\uC9C4 \u2014 \uB0B4\uC77C \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694`}),{headers:l})}let a=Math.min(n,d-r),p=(await t.DB.prepare("SELECT MAX(date) AS latest_date FROM rankings WHERE date != 'manual'").first())?.latest_date||null,{results:m}=await t.DB.prepare(`
        SELECT w.tmdb_id
        FROM works w
        WHERE (
          SELECT COUNT(*) FROM title_videos tv WHERE tv.tmdb_id = w.tmdb_id
        ...
        ORDER BY
          ...
          w.created_at DESC
        LIMIT ?
      `).bind(p,a).all();if(!m.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uB300\uC0C1 \uC791\uD488 \uC5C6\uC74C (\uBAA8\uB450 \uCFE8\uB2E4\uC6B4 \uC911\uC774\uAC70\uB098 \uC601\uC0C1\uC774 \uC774\uBBF8 \uCDA9\uBD84\uD568)"}),{headers:l});let _=[],u=0;for(let E of m)try{let w=await Q(E.tmdb_id,t);u+=w,_.push({tmdb_id:E.tmdb_id,saved:w,ok:!0})}catch(w){console.error(`[BATCH_CRAWL] tmdb_id=${E.tmdb_id} \uC624\uB958:`,w.message),_.push({tmdb_id:E.tmdb_id,saved:0,ok:!1,error:w.message})}let R=(await t.DB.prepare(`
        SELECT COUNT(*) AS cnt
        FROM works w
        WHERE (
          SELECT COUNT(*) FROM title_videos tv WHERE tv.tmdb_id = w.tmdb_id
      ...
      `).first())?.cnt||0;return console.log(`[BATCH_CRAWL] \u2705 \uC644\uB8CC: \uC2DC\uB3C4 ${m.length}\uAC74, \uC800\uC7A5 ${u}\uAC1C, \uB0A8\uC74C ${R}`),new Response(JSON.stringify({ok:!0,attempted:m.length,filled:u,remaining:R,results:_}),{headers:l})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:l})}}if(o==="/admin/videos"&&i.method==="POST"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:l});try{let n=await i.json(),{tmdb_id:d,youtube_url:e}=n,{title:r}=n;if(!d||!e)return new Response(JSON.stringify({ok:!1,message:"tmdb_id, youtube_url required"}),{status:400,headers:l});let a=e.match(/(?:v=|youtu\.be\/|embed\/)([a-zA-Z0-9_-]{11})/);if(!a)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD558\uC9C0 \uC54A\uC740 \uC720\uD29C\uBE0C URL"}),{status:400,headers:l});let c=a[1],p=await t.DB.prepare("SELECT id, title FROM title_videos WHERE tmdb_id = ? AND youtube_id = ? LIMIT 1").bind(d,c).first();if(p)return new Response(JSON.stringify({ok:!1,message:`\uC774\uBBF8 \uB4F1\uB85D\uB41C \uC601\uC0C1\uC785\uB2C8\uB2E4. (\uC81C\uBAA9: "${p.title||c}")`}),{status:409,headers:l});if(!r)try{r=(await(await fetch(`https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${c}&format=json`)).json()).title||""}catch{r=""}return await t.DB.prepare("INSERT INTO title_videos (tmdb_id, youtube_url, youtube_id, title, is_main) VALUES (?, ?, ?, ?, 0)").bind(d,e,c,r).run(),new Response(JSON.stringify({ok:!0}),{headers:l})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:l})}}if(o.match(/\/admin\/videos\/(\d+)\/main/)&&i.method==="PATCH"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:l});let n=parseInt(o.match(/\/admin\/videos\/(\d+)\/main/)[1]);try{let{results:d}=await t.DB.prepare("SELECT tmdb_id FROM title_videos WHERE id = ?").bind(n).all();if(!d.length)return new Response(JSON.stringify({ok:!1,message:"\uC5C6\uC74C"}),{status:404,headers:l});let e=d[0].tmdb_id;return await t.DB.batch([t.DB.prepare("UPDATE title_videos SET is_main = 0 WHERE tmdb_id = ?").bind(e),t.DB.prepare("UPDATE title_videos SET is_main = 1 WHERE id = ?").bind(n)]),new Response(JSON.stringify({ok:!0}),{headers:l})}catch(d){return new Response(JSON.stringify({ok:!1,message:d.message}),{status:500,headers:l})}}if(o.match(/\/admin\/videos\/(\d+)$/)&&i.method==="DELETE"){if(i.headers.get("Authorization")!==`Bearer ${t.ADMIN_SECRET}`)return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:l});let n=parseInt(o.match(/\/admin\/videos\/(\d+)$/)[1]);try{return await t.DB.prepare("DELETE FROM title_videos WHERE id = ?").bind(n).run(),new Response(JSON.stringify({ok:!0}),{headers:l})}catch(d){return new Response(JSON.stringify({ok:!1,message:d.message}),{status:500,headers:l})}}if(o.startsWith("/imdb/")&&o!=="/imdb/save"&&i.method==="GET"){let n=o.split("/imdb/")[1];if(!n||!/^tt\d+$/.test(n))return new Response(JSON.stringify({ok:!1,message:"invalid imdb_id"}),{status:400,headers:l});try{let d=await t.DB.prepare("SELECT imdb_rating, imdb_votes, imdb_updated FROM works WHERE imdb_id = ? LIMIT 1").bind(n).first();if(d?.imdb_rating){let c=new Date(d.imdb_updated||0);if((Date.now()-c.getTime())/(1e3*60*60*24)<7)return new Response(JSON.stringify({ok:!0,source:"cache",rating:d.imdb_rating.toFixed(1),votes:d.imdb_votes||""}),{headers:l})}let e=t.OMDB_API_KEY;if(!e)return new Response(JSON.stringify({ok:!1,message:"OMDB key not configured"}),{status:500,headers:l});let a=await(await fetch(`https://www.omdbapi.com/?i=${n}&apikey=${e}`)).json();if(a.Response!=="False"){let c=parseFloat(a.imdbRating);if(!isNaN(c)){let p=a.imdbVotes||"",m=new Date().toISOString();return await t.DB.prepare("UPDATE works SET imdb_rating = ?, imdb_votes = ?, imdb_updated = ? WHERE imdb_id = ?").bind(c,p,m,n).run(),new Response(JSON.stringify({ok:!0,source:"omdb",rating:c.toFixed(1),votes:p}),{headers:l})}}return new Response(JSON.stringify({ok:!1,message:"rating not available"}),{status:404,headers:l})}catch(d){return console.error("[IMDB GET]",d),new Response(JSON.stringify({ok:!1,message:d.message}),{status:500,headers:l})}}if(o==="/imdb/save"&&i.method==="POST")try{let n=await i.json(),{tmdb_id:d,imdb_id:e}=n;return!d||!e?new Response(JSON.stringify({ok:!1,message:"tmdb_id and imdb_id required"}),{status:400,headers:l}):/^tt\d+$/.test(e)?(await t.DB.prepare("UPDATE works SET imdb_id = ? WHERE tmdb_id = ?").bind(e,parseInt(d)).run(),new Response(JSON.stringify({ok:!0}),{headers:l})):new Response(JSON.stringify({ok:!1,message:"invalid imdb_id format"}),{status:400,headers:l})}catch(n){return console.error("[IMDB SAVE]",n),new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:l})}if(o==="/youtube/trending"&&i.method==="GET")try{let{results:n}=await t.DB.prepare("SELECT * FROM youtube_trending ORDER BY rank ASC").all();if(n.length>0){let m=new Date(n[0].collected_at);if((Date.now()-m.getTime())/(1e3*60*60)<6)return new Response(JSON.stringify({ok:!0,data:n,cached:!0}),{headers:l})}let d=`https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&chart=mostPopular&regionCode=KR&maxResults=50&key=${t.YOUTUBE_API_KEY}`,e=await fetch(d),r=await e.json();if(!e.ok||!r.items?.length)return n.length>0?new Response(JSON.stringify({ok:!0,data:n,cached:!0}),{headers:l}):new Response(JSON.stringify({ok:!1,message:"YouTube API \uC624\uB958"}),{status:500,headers:l});let a=new Date().toISOString(),c=r.items.map((m,_)=>({rank:_+1,video_id:m.id,title:m.snippet?.title||"",channel:m.snippet?.channelTitle||"",thumbnail:m.snippet?.thumbnails?.medium?.url||m.snippet?.thumbnails?.default?.url||"",view_count:parseInt(m.statistics?.viewCount||0),collected_at:a}));await t.DB.prepare("DELETE FROM youtube_trending").run();let p=c.map(m=>t.DB.prepare(`
        ...
        ORDER BY
          ...
          title_ko ASC
        LIMIT ?
      `).bind(`%${e}%`,`%${e}%`,`${e}%`,d).all();return new Response(JSON.stringify({ok:!0,data:r}),{headers:l})}catch(r){return new Response(JSON.stringify({ok:!1,message:r.message}),{status:500,headers:l})}}if(o==="/works/register"&&i.method==="POST")try{let n=await i.json(),{tmdb_id:d,title_ko:e,title_en:r,poster_path:a,media_type:c,genre:p,original_language:m,tmdb_rating:_,release_date:u}=n;if(!d||!e)return new Response(JSON.stringify({ok:!1,message:"tmdb_id, title_ko required"}),{status:400,headers:l});let f=r&&/[\uAC00-\uD7A3]/.test(r),E=r&&/[a-zA-Z]/.test(r)&&!f?r:null,w=_??null,k=u||null,O=new Date().toISOString();return await t.DB.prepare(`
        INSERT INTO works (
      ...
      `).bind(n).all(),c=new Map;try{let m=await t.DB.prepare("SELECT MAX(date) as d FROM rankings WHERE date != 'manual'").first();if(m?.d){let{results:_}=await t.DB.prepare(`
            ...
            WHERE date = ?
            GROUP BY tmdb_id
          `).bind(m.d).all();for(let u of _)c.set(u.tmdb_id,u.cnt)}}catch{}let p=[];for(let m of a){let _=(m.variety_genre||"").split(",").map(w=>w.trim()).filter(Boolean),u=r.filter(w=>_.includes(w)).length;if(!u)continue;let f=null;if(r.length===2?f=u===2?92:82:r.length===1&&(f=u===1?87:null),!f)continue;let R=c.get(m.tmdb_id)||0,E=Math.min(f+R,99);p.push({tmdb_id:m.tmdb_id,title_ko:m.title_ko,title_en:m.title_en,poster_path:m.poster_path,tmdb_rating:m.tmdb_rating,release_year:m.release_year,match_pct:E,media_type:m.media_type||null})}return p.sort((m,_)=>_.match_pct-m.match_pct||(_.release_year||0)-(m.release_year||0)||(_.tmdb_rating||0)-(m.tmdb_rating||0)),new Response(JSON.stringify({ok:!0,data:p.slice(0,d)}),{headers:l})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:l})}}if(o.startsWith("/works/")&&i.method==="GET"){let n=o.split("/works/")[1];if(!n)return new Response(JSON.stringify({ok:!1,message:"tmdb_id required"}),{status:400,headers:l});let{results:d}=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(parseInt(n)).all();if(!d.length)return new Response(JSON.stringify({ok:!1,message:"Not found"}),{status:404,headers:l});let e={...d[0]};if(!e.mbti_tags&&e.genre){let c=Rt(e.genre);c&&(g.waitUntil(t.DB.prepare("UPDATE works SET mbti_tags = ? WHERE tmdb_id = ?").bind(c,parseInt(n)).run()),e.mbti_tags=c)}let r=720*60*60*1e3;if(!e.keyword_preview_updated_at||Date.now()-new Date(e.keyword_preview_updated_at).getTime()>r){let c={keyword:null,items:[]};if(e.keywords&&e.keywords!=="__NONE__"){let m=e.keywords.split(",").map(_=>_.trim()).filter(Boolean).slice(0,10);if(m.length)try{let _=m.map(f=>t.DB.prepare(`
                SELECT tmdb_id, title_ko, title_en, poster_path
                FROM works
                WHERE (',' || LOWER(keywords) || ',') LIKE ('%,' || ? || ',%')
                  AND tmdb_id != ?
                LIMIT 20
              `).bind(f.toLowerCase(),parseInt(n))),u=await t.DB.batch(_);for(let f=0;f<m.length;f++){let R=u[f]?.results||[];if(R.length>=2){c={keyword:m[f],items:R};break}}}catch{}}let p=new Date().toISOString();e.keyword_preview=JSON.stringify(c),e.keyword_preview_updated_at=p,g.waitUntil(t.DB.prepare("UPDATE works SET keyword_preview = ?, keyword_preview_updated_at = ? WHERE tmdb_id = ?").bind(e.keyword_preview,p,parseInt(n)).run())}try{let{results:c}=await t.DB.prepare(`
      ...
      `).bind(n).all(),{results:c}=await t.DB.prepare(`
        ...
        WHERE lw.user_id = ?
        ORDER BY lw.created_at DESC
      `).bind(n).all(),{results:p}=await t.DB.prepare("SELECT * FROM pick_lists WHERE user_id = ? AND is_public = 1 ORDER BY created_at DESC").bind(n).all(),m=await Promise.all(p.map(async _=>{let{results:u}=await t.DB.prepare("SELECT * FROM pick_list_works WHERE pick_list_id = ? ORDER BY sort_order ASC, created_at DESC").bind(_.id).all();return{..._,works:u,work_count:u.length}}));return new Response(JSON.stringify({ok:!0,is_own:!1,user:d,reviews:e,wishlist:r,wishlist_hidden:!d.wishlist_public,posts:a,life_works:c,pick_lists:m,stats:{review_count:e.length,wishlist_count:d.wishlist_public?r.length:null,likes_received:d.total_likes_received||0,post_count:a.length,life_work_count:c.length,pick_list_count:m.length}}),{headers:s})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:s})}if(o==="/mypage/reviews"&&i.method==="GET")try{let d=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i);if(!d)return new Response(JSON.stringify({ok:!1,message:"\uB85C\uADF8\uC778 \uD544\uC694"}),{status:401,headers:s});let e=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(d).first();if(!e)return new Response(JSON.stringify({ok:!1,message:"\uC138\uC158 \uB9CC\uB8CC"}),{status:401,headers:s});let r=await t.DB.prepare("SELECT nickname FROM users WHERE id = ?").bind(e.user_id).first(),{results:a}=await t.DB.prepare(`
        SELECT r.id, r.tmdb_id, r.score, r.text, r.emotions, r.custom_tags,
          r.likes, r.spoiler, r.created_at,
          COALESCE(rk.title_ko,  wk.title_ko)    as title_ko,
          COALESCE(rk.poster_path, wk.poster_path) as poster_path,
        ...
        WHERE r.user_id = ?
        ORDER BY r.created_at DESC
      `).bind(e.user_id,e.user_id).all();return new Response(JSON.stringify({ok:!0,reviews:a,nickname:r?.nickname||"\uB098"}),{headers:s})}catch(n){return new Response(JSON.stringify({ok:!1,message:n.message}),{status:500,headers:s})}if(o.match(/^\/user\/\d+\/reviews$/)&&i.method==="GET")try{let n=parseInt(o.split("/")[2]),d=await t.DB.prepare("SELECT nickname FROM users WHERE id = ?").bind(n).first();if(!d)return new Response(JSON.stringify({ok:!1,message:"\uC720\uC800\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC5B4\uC694"}),{status:404,headers:s});let r=(i.headers.get("Authorization")||"").replace("Bearer ","").trim()||h(i),a=-1;if(r){let p=await t.DB.prepare("SELECT user_id FROM sessions WHERE id = ? AND expires_at > datetime('now')").bind(r).first();p&&(a=p.user_id)}let{results:c}=await t.DB.prepare(`
        SELECT r.id, r.tmdb_id, r.score, r.text, r.emotions, r.custom_tags,
          r.likes, r.spoiler, r.created_at,
          COALESCE(rk.title_ko,  wk.title_ko)    as title_ko,
          COALESCE(rk.poster_path, wk.poster_path) as poster_path,
      ...
      `).bind(parseInt(p),w||"",k||"",E,D,w||null,k||null,E,D).run();let y=parseInt(m)||null;return y||(y=((await t.DB.prepare("SELECT MAX(rank) as max_rank FROM rankings WHERE platform = ? AND category_slot = ? AND date = ?").bind(r,a,c).first())?.max_rank||0)+1),await t.DB.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(r,a,a,c,-y,parseInt(p),w||"",k||"",E,O,N,T,R?1:0,a).run(),await t.DB.prepare("UPDATE rankings SET rank = ? WHERE platform = ? AND category_slot = ? AND date = ? AND rank = ?").bind(y,r,a,c,-y).run(),k&&w&&await t.DB.prepare(`
          INSERT INTO title_map (title_en, title_ko, tmdb_id)
          VALUES (?, ?, ?)
          ON CONFLICT(title_en) DO UPDATE SET
            title_ko = excluded.title_ko,
            tmdb_id  = COALESCE(excluded.tmdb_id, tmdb_id)
        `).bind(k.trim(),w.trim(),parseInt(p)).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('ranking_add', ?, ?, ?, ?)").bind(r,a,String(p),JSON.stringify({rank:y,title_ko:w,date:c})).run(),new Response(JSON.stringify({ok:!0,rank:y,poster_path:E,title_ko:w,title_en:k}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/rankings"&&i.method==="GET"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});let e=g.searchParams.get("date"),r=g.searchParams.get("manual"),a,c;r==="true"?(a="SELECT * FROM rankings WHERE date = 'manual' ORDER BY platform, category_slot, rank",c=null):e?(a="SELECT * FROM rankings WHERE date = ? ORDER BY platform, category_slot, rank",c=e):(a="SELECT * FROM rankings WHERE date = (SELECT MAX(date) FROM rankings WHERE date != 'manual') ORDER BY platform, category_slot, rank",c=null);let{results:p}=c?await t.DB.prepare(a).bind(c).all():await t.DB.prepare(a).all();return new Response(JSON.stringify({ok:!0,data:p}),{headers:s})}if(o==="/admin/fix"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{id:r,tmdb_id:a,title_ko:c,title_en:p,delete_duplicates:m,media_type:_}=e,u=e.season!==void 0?e.season:void 0,f=e.poster_path||null;if(!r)return new Response(JSON.stringify({ok:!1,message:"id required"}),{status:400,headers:s});let R=null,E=c||null,w=p||null,k=await t.DB.prepare("SELECT title_ko, title_en, poster_path FROM rankings WHERE id = ?").bind(parseInt(r)).first();if(a)try{let D=_==="movie"?["movie"]:_==="tv"?["tv"]:["tv","movie"];for(let y of D){let b=await fetch(`https://api.themoviedb.org/3/${y}/${a}?language=ko-KR&api_key=${t.TMDB_API_KEY}`);if(!b.ok)continue;let I=await b.json();if(!(!I.poster_path&&!I.name&&!I.title)){if(R=I.poster_path||null,E||(E=I.name||I.title||null),!w){let C=await fetch(`https://api.themoviedb.org/3/${y}/${a}?language=en-US&api_key=${t.TMDB_API_KEY}`);if(C.ok){let L=await C.json(),B=L.original_title||L.original_name||"",J=L.title||L.name||"";w=/[\uAC00-\uD7A3]/.test(B)?J:B||J}}break}}}catch{}f&&(R=f);let O=u!==void 0?u!==null?parseInt(u):null:void 0;if(await t.DB.prepare(`
        UPDATE rankings
      ...
      `).bind(a?parseInt(a):null,E,w,R,...O!==void 0?[O]:[],parseInt(r)).run(),a){m&&(w&&await t.DB.prepare("DELETE FROM works WHERE title_en = ? AND tmdb_id != ?").bind(w,parseInt(a)).run(),E&&/[\uAC00-\uD7A3]/.test(E)&&await t.DB.prepare("DELETE FROM works WHERE title_ko = ? AND tmdb_id != ?").bind(E,parseInt(a)).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, memo) VALUES ('works_delete', ?, ?)").bind(String(a),`\uC911\uBCF5 \uC0AD\uC81C: title_en="${w}" title_ko="${E}"`).run());let D=_==="tv"||_==="movie"?_:null,y=f?null:R;await t.DB.prepare(`
        ...
        UPDATE ott_categories SET
          ...
          updated_at       = datetime('now')
        WHERE id = ?
      `).bind(a??null,c??null,p??null,m??null,_??null,u===void 0?"__SKIP__":"__SET__",u===void 0?null:u||null,f===void 0?"__SKIP__":"__SET__",f===void 0?null:f??0,R===void 0?"__SKIP__":"__SET__",R===void 0?null:R||null,E===void 0?"__SKIP__":"__SET__",E===void 0?null:E??0,w===void 0?"__SKIP__":"__SET__",w===void 0?null:w||null,k===void 0?"__SKIP__":"__SET__",k===void 0?null:k??0,O??null,e).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, after_value) VALUES ('category_setting', ?, ?)").bind(String(e),JSON.stringify(r)).run(),new Response(JSON.stringify({ok:!0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/categories"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{platform:r,category_slot:a,source_name:c,display_name:p,crawl_limit:m,main_limit:_,platform_limit:u,is_active:f}=e;if(!r||!a||!c)return new Response(JSON.stringify({ok:!1,message:"platform, category_slot, source_name required"}),{status:400,headers:s});let E=((await t.DB.prepare("SELECT MAX(table_index) as max_idx FROM ott_categories WHERE platform = ?").bind(r).first())?.max_idx??-1)+1;await t.DB.prepare(`
      ...
      `).bind(parseInt(a),u||null,f||null,_,m.platform,m.category_slot,m.rank,m.crawled_date).run(),await t.DB.prepare(`
      ...
      `).bind(parseInt(a),e).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, after_value) VALUES ('queue_resolve', ?, ?, ?, ?)").bind(m.platform,m.category_slot,String(a),JSON.stringify({tmdb_id:a,title_ko:u,title_en:f})).run(),new Response(JSON.stringify({ok:!0,poster_path:_,title_ko:u,title_en:f}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/rank-override"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{platform:r,category_slot:a,date:c,tmdb_id:p,original_rank:m,override_rank:_,reason:u}=e;return!r||!a||!c||!p||!_?new Response(JSON.stringify({ok:!1,message:"\uD544\uC218 \uD30C\uB77C\uBBF8\uD130 \uB204\uB77D"}),{status:400,headers:s}):(await t.DB.prepare(`
        ...
        ON CONFLICT(platform, category_slot, date, tmdb_id) DO UPDATE SET
          ...
          updated_at    = datetime('now')
      `).bind(r,a,c,parseInt(p),m||0,parseInt(_),u||null).run(),await t.DB.prepare("INSERT INTO admin_logs (action, platform, category_slot, target_id, before_value, after_value) VALUES ('rank_override', ?, ?, ?, ?, ?)").bind(r,a,String(p),JSON.stringify({rank:m}),JSON.stringify({rank:_,reason:u})).run(),new Response(JSON.stringify({ok:!0}),{headers:s}))}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/rank-override"&&i.method==="DELETE"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json(),{platform:r,category_slot:a,date:c,tmdb_id:p}=e;return await t.DB.prepare("DELETE FROM rank_overrides WHERE platform = ? AND category_slot = ? AND date = ? AND tmdb_id = ?").bind(r,a,c,parseInt(p)).run(),new Response(JSON.stringify({ok:!0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o.match(/^\/admin\/works\/\d+$/)&&i.method==="GET"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=parseInt(o.split("/")[3]),r=await t.DB.prepare("SELECT * FROM works WHERE tmdb_id = ?").bind(e).first();return r?new Response(JSON.stringify({ok:!0,data:r}),{headers:s}):new Response(JSON.stringify({ok:!1,message:"Not found"}),{status:404,headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/works"&&i.method==="GET"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=g.searchParams.get("q")||"",r=g.searchParams.get("filter")||"",a=g.searchParams.get("date")||"",c=g.searchParams.get("sort")||"recent",p=parseInt(g.searchParams.get("page")||"1"),m=50,_=(p-1)*m,u=c==="updated"?"updated_at DESC, id DESC":"COALESCE(created_at, updated_at) DESC, id DESC",f,R;r==="new_match"&&a?(f=`SELECT * FROM works WHERE first_matched_date = ? AND match_source IN ('auto_claude', 'auto_tmdb') ORDER BY ${u} LIMIT ? OFFSET ?`,R=[a,m,_]):e?(f=`SELECT * FROM works WHERE title_ko LIKE ? OR title_en LIKE ? ORDER BY ${u} LIMIT ? OFFSET ?`,R=[`%${e}%`,`%${e}%`,m,_]):(f=`SELECT * FROM works ORDER BY ${u} LIMIT ? OFFSET ?`,R=[m,_]);let{results:E}=await t.DB.prepare(f).bind(...R).all();return new Response(JSON.stringify({ok:!0,data:E}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o.match(/^\/admin\/works\/\d+$/)&&i.method==="PATCH"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=parseInt(o.split("/")[3]),r=await i.json(),{title_ko:a,title_en:c,poster_path:p,delete_duplicates:m,media_type:_,mbti_tags:u}=r,f=_==="tv"||_==="movie"?_:null,R=u!==void 0,E=R?u||null:void 0,w=await t.DB.prepare("SELECT title_ko, title_en, poster_path, media_type FROM works WHERE tmdb_id = ?").bind(e).first();if(m&&(c||w?.title_en)){let k=c||w?.title_en;await t.DB.prepare("DELETE FROM works WHERE title_en = ? AND tmdb_id != ?").bind(k,e).run(),await t.DB.prepare("INSERT INTO admin_logs (action, target_id, before_value, memo) VALUES ('works_delete', ?, ?, ?)").bind(String(e),JSON.stringify({title_en:k}),`\uC911\uBCF5 \uC0AD\uC81C: title_en="${k}" tmdb_id!=${e}`).run()}return await t.DB.prepare(`
        UPDATE works SET
          title_ko         = COALESCE(?, title_ko),
          title_en         = COALESCE(?, title_en),
          poster_path      = COALESCE(?, poster_path),
      ...
      `).bind(r,a,a,c||"",parseInt(m),f,R,parseInt(p),E,w,k,O,N,_||null,u!==null?parseInt(u):null).run();let D=new Date().toISOString();return await t.DB.prepare(`
...
${m}`,f=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":t.ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:2e3,system:_,messages:[{role:"user",content:u}]})});if(!f.ok){let y=await f.text().catch(()=>"");return new Response(JSON.stringify({ok:!1,message:`Claude API \uC624\uB958 (status ${f.status})`,detail:y.slice(0,300)}),{status:502,headers:s})}let E=((await f.json()).content||[]).filter(y=>y.type==="text").map(y=>y.text).join(""),w;try{let y=E.replace(/```json|```/g,"").trim();w=JSON.parse(y)}catch{return new Response(JSON.stringify({ok:!1,message:"Claude \uC751\uB2F5 \uD30C\uC2F1 \uC2E4\uD328 \u2014 \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694",raw:E.slice(0,300)}),{status:502,headers:s})}Array.isArray(w)||(w=[]);let k=new Set(c),O=new Map;for(let y of w){let b=parseInt(y.tmdb_id);if(!b)continue;let I=Array.isArray(y.tags)?y.tags.filter(C=>k.has(C)).slice(0,2):[];O.set(b,I)}let N=[],T=0;for(let y of p){if(!O.has(y.tmdb_id))continue;let b=O.get(y.tmdb_id);N.push(t.DB.prepare("UPDATE works SET variety_genre = ?, variety_genre_source = 'auto' WHERE tmdb_id = ?").bind(b.length?b.join(","):null,y.tmdb_id)),T++}N.length&&await t.DB.batch(N);let D=await t.DB.prepare(`
      ...
      `).bind(r,a,a,r).run(),new Response(JSON.stringify({ok:!0}),{headers:s}))}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/persons/collect"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||20,50),{results:a}=await t.DB.prepare(`
        ...
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,worksScanned:0,personsFound:0,remaining:0,message:"\uC2A4\uCE94\uD560 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let c=new Map,p=[];for(let u of a){p.push(u.tmdb_id);let f=u.media_type==="tv"?"tv":"movie",R=f==="tv"?"aggregate_credits":"credits";try{let E=await fetch(`https://api.themoviedb.org/3/${f}/${u.tmdb_id}/${R}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json();for(let k of(w.cast||[]).slice(0,15))k.id&&k.name&&!c.has(k.id)&&c.set(k.id,{name:k.name,job:"act"});for(let k of w.crew||[])(k.job==="Director"||k.job==="Creator"||k.department==="Directing"||(k.jobs||[]).some(N=>N.job==="Director"||N.job==="Creator"))&&k.id&&k.name&&c.set(k.id,{name:k.name,job:"direct"})}catch{}}let m=[];for(let[u,f]of c)m.push(t.DB.prepare(`INSERT INTO persons (tmdb_id, name, job) VALUES (?, ?, ?)
             ON CONFLICT(tmdb_id) DO NOTHING`).bind(u,f.name,f.job));for(let u of p)m.push(t.DB.prepare("UPDATE works SET credits_scanned = 1 WHERE tmdb_id = ?").bind(u));m.length&&await t.DB.batch(m);let _=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE credits_scanned IS NULL OR credits_scanned = 0").first();return new Response(JSON.stringify({ok:!0,worksScanned:a.length,personsFound:c.size,remaining:_?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/works/backfill-language"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE original_language IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let c=[],p=0;for(let _ of a){let u=_.media_type?[_.media_type]:["tv","movie"],f=null;for(let R of u)try{let E=await fetch(`https://api.themoviedb.org/3/${R}/${_.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json();if(w.original_language){f=w.original_language;break}}catch{}f?(c.push(t.DB.prepare("UPDATE works SET original_language = ? WHERE tmdb_id = ?").bind(f,_.tmdb_id)),p++):c.push(t.DB.prepare("UPDATE works SET original_language = 'unknown' WHERE tmdb_id = ?").bind(_.tmdb_id))}c.length&&await t.DB.batch(c);let m=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE original_language IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:p,remaining:m?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/works/backfill-release-year"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE release_year IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let c=[],p=0;for(let _ of a){let u=_.media_type?[_.media_type]:["tv","movie"],f=null;for(let R of u)try{let E=await fetch(`https://api.themoviedb.org/3/${R}/${_.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!E.ok)continue;let w=await E.json(),k=w.release_date||w.first_air_date||"",O=parseInt(k.slice(0,4));if(O){f=O;break}}catch{}f?(c.push(t.DB.prepare("UPDATE works SET release_year = ? WHERE tmdb_id = ?").bind(f,_.tmdb_id)),p++):c.push(t.DB.prepare("UPDATE works SET release_year = 0 WHERE tmdb_id = ?").bind(_.tmdb_id))}c.length&&await t.DB.batch(c);let m=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE release_year IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:p,remaining:m?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/works/backfill-rating"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),r=Math.min(parseInt(e.limit)||30,50),{results:a}=await t.DB.prepare(`
        SELECT tmdb_id, media_type FROM works
        WHERE tmdb_rating IS NULL AND rating_updated_at IS NULL
        LIMIT ?
      `).bind(r).all();if(!a.length)return new Response(JSON.stringify({ok:!0,attempted:0,filled:0,remaining:0,message:"\uCC44\uC6B8 \uC791\uD488 \uC5C6\uC74C"}),{headers:s});let c=[],p=0,m=new Date().toISOString();for(let u of a){let f=u.media_type?[u.media_type]:["tv","movie"],R=null,E=null,w=!1;for(let k of f)try{let O=await fetch(`https://api.themoviedb.org/3/${k}/${u.tmdb_id}?api_key=${t.TMDB_API_KEY}`);if(!O.ok)continue;let N=await O.json();w=!0,R=N.vote_average??null,E=N.release_date||N.first_air_date||null;break}catch{}w?(c.push(t.DB.prepare("UPDATE works SET tmdb_rating = ?, release_date = ?, rating_updated_at = ? WHERE tmdb_id = ?").bind(R,E,m,u.tmdb_id)),R!==null&&p++):c.push(t.DB.prepare("UPDATE works SET rating_updated_at = ? WHERE tmdb_id = ?").bind(m,u.tmdb_id))}c.length&&await t.DB.batch(c);let _=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE tmdb_rating IS NULL AND rating_updated_at IS NULL").first();return new Response(JSON.stringify({ok:!0,attempted:a.length,filled:p,remaining:_?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/works/batch-imdb-search"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=30;try{let f=await i.json();f?.limit&&Number.isInteger(f.limit)&&f.limit>0&&(e=f.limit)}catch{}let r=t.OMDB_API_KEY;if(!r)return new Response(JSON.stringify({ok:!1,message:"OMDB key not configured"}),{status:500,headers:s});let c=(await t.DB.prepare("SELECT MAX(date) AS latest_date FROM rankings WHERE date != 'manual'").first())?.latest_date||null,{results:p}=await t.DB.prepare(`
      ...
      `).bind(e).all(),a=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE media_type IS NULL OR media_type = ''").first();return new Response(JSON.stringify({ok:!0,items:r,remaining:a?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/works/bulk-set-media-type"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json().catch(()=>({})),a=(Array.isArray(e.items)?e.items:[]).filter(m=>m&&m.tmdb_id&&(m.media_type==="movie"||m.media_type==="tv"));if(!a.length)return new Response(JSON.stringify({ok:!1,message:"\uC720\uD6A8\uD55C \uD56D\uBAA9\uC774 \uC5C6\uC5B4\uC694 (media_type\uC740 'movie' \uB610\uB294 'tv'\uB9CC \uD5C8\uC6A9)"}),{status:400,headers:s});let c=a.map(m=>t.DB.prepare("UPDATE works SET media_type = ? WHERE tmdb_id = ?").bind(m.media_type,parseInt(m.tmdb_id)));await t.DB.batch(c);let p=await t.DB.prepare("SELECT COUNT(*) as cnt FROM works WHERE media_type IS NULL OR media_type = ''").first();return new Response(JSON.stringify({ok:!0,updated:a.length,remaining:p?.cnt||0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/grade-settings"&&i.method==="GET"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let{results:e}=await t.DB.prepare("SELECT * FROM grade_settings ORDER BY sort_order ASC").all();return new Response(JSON.stringify({ok:!0,data:e}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/grade-settings"&&i.method==="PUT"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let e=await i.json();if(!Array.isArray(e))return new Response(JSON.stringify({ok:!1,message:"Array required"}),{status:400,headers:s});for(let r of e)await t.DB.prepare(`
        ...
        FROM users u
        LEFT JOIN grade_settings gs ON gs.grade_key = u.grade
      `,m=[];c&&(p+=" WHERE u.nickname LIKE ?",m.push(`%${c}%`)),p+=" ORDER BY u.created_at DESC LIMIT ? OFFSET ?",m.push(r,a);let{results:_}=await t.DB.prepare(p).bind(...m).all();return new Response(JSON.stringify({ok:!0,data:_}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}if(o==="/admin/ott-points/adjust"&&i.method==="POST"){if(!S(i,t))return new Response(JSON.stringify({ok:!1,message:"Unauthorized"}),{status:401,headers:s});try{let{user_id:e,points:r,reason:a}=await i.json();if(!e||r===void 0||!a)return new Response(JSON.stringify({ok:!1,message:"user_id, points, reason \uD544\uC218"}),{status:400,headers:s});await t.DB.prepare("INSERT INTO user_point_logs (user_id, points, reason) VALUES (?, ?, ?)").bind(e,r,a).run(),await t.DB.prepare("UPDATE users SET ott_points = MAX(0, COALESCE(ott_points, 0) + ?) WHERE id = ?").bind(r,e).run();let c=await t.DB.prepare("SELECT ott_points FROM users WHERE id = ?").bind(e).first();if(c){let p=await kt(c.ott_points,t);p&&await t.DB.prepare("UPDATE users SET grade = ? WHERE id = ? AND (grade IS NULL OR grade NOT IN (SELECT grade_key FROM grade_settings WHERE is_special = 1))").bind(p,e).run()}return new Response(JSON.stringify({ok:!0}),{headers:s})}catch(e){return new Response(JSON.stringify({ok:!1,message:e.message}),{status:500,headers:s})}}return null}async function kt(o,i){try{let{results:t}=await i.DB.prepare(`SELECT grade_key FROM grade_settings
       WHERE is_special = 0 AND min_ott_points <= ?
     ...
     LIMIT 5`).all();return A({ok:!0,items:t??[]},200,i)}async function Ot(o,i,t){let g=o.searchParams.get("platform"),s=o.searchParams.get("type"),l=Math.max(parseInt(o.searchParams.get("page")||"1"),1),n=30,d=(l-1)*n,e=["is_hidden = 0"],r=[];g&&(e.push("platform = ?"),r.push(g)),s&&(e.push("type = ?"),r.push(s));let a=e.join(" AND "),c=[...r],p=[...r,n,d],[m,_]=await i.DB.batch([i.DB.prepare(`SELECT COUNT(*) as total FROM ott_contents WHERE ${a}`).bind(...c),i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
              tmdb_id, tmdb_type, thumbnail, published_at, view_count
       FROM ott_contents
       WHERE ${a}
       ORDER BY published_at DESC
       LIMIT ? OFFSET ?`).bind(...p)]),u=m.results?.[0]?.total??0,f=_.results??[];return A({ok:!0,items:f,pagination:{page:l,pageSize:n,total:u,totalPages:Math.ceil(u/n)}},200,t)}async function bt(o,i,t){let g=await i.DB.prepare(`SELECT id, youtube_id, platform, type, title, work_title,
            tmdb_id, tmdb_type, thumbnail, published_at, view_count,
...
\uB9C8\uD06C\uB2E4\uC6B4 \uAE30\uD638 \uC5C6\uC774 \uC77C\uBC18 \uD14D\uC2A4\uD2B8\uB85C, \uB2E8\uB77D \uAD6C\uBD84\uC740 \uBE48 \uC904\uB85C\uB9CC \uD574\uC8FC\uC138\uC694.`,I=await Mt(b,l,{useWebSearch:T,maxTokens:T?5e3:4096});if(!I)throw new Error("AI \uC751\uB2F5\uC774 \uBE44\uC5B4\uC788\uC2B5\uB2C8\uB2E4. \uB2E4\uC2DC \uC2DC\uB3C4\uD574\uC8FC\uC138\uC694.");return new Response(JSON.stringify({ok:!0,post:I,rankingData:E,meta:{platform:d,platformName:O,weekInfo:k,categorySlot:R||"all",categoryLabel:R&&E.length===1?E[0].display_name:"\uC804\uCCB4",generatedAt:new Date().toISOString()}}),{headers:s})}catch(E){return new Response(JSON.stringify({ok:!1,error:E.message}),{status:500,headers:s})}}return null}var Ft=["ad","bug"],Ht=["pending","answered","resolved"],Wt=5,mt=30;async function pt(o,i,t,g,s,l){if(o==="/inquiry"&&i.method==="POST")try{let e=await i.json(),{type:r,name:a,email:c,phone:p,title:m,content:_,page_url:u,website:f}=e;if(f)return new Response(JSON.stringify({ok:!0}),{headers:l});if(!Ft.includes(r))return new Response(JSON.stringify({ok:!1,message:"type\uC740 ad \uB610\uB294 bug\uC5EC\uC57C \uD569\uB2C8\uB2E4"}),{status:400,headers:l});if(!m||!m.trim()||!_||!_.trim())return new Response(JSON.stringify({ok:!1,message:"\uC81C\uBAA9\uACFC \uB0B4\uC6A9\uC740 \uD544\uC218\uC785\uB2C8\uB2E4"}),{status:400,headers:l});if(r==="ad"){if(!a||!a.trim())return new Response(JSON.stringify({ok:!1,message:"\uB2F4\uB2F9\uC790\uBA85 \uB610\uB294 \uC5C5\uCCB4\uBA85\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:l});if(!c||!c.trim())return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C\uC744 \uC785\uB825\uD574\uC8FC\uC138\uC694"}),{status:400,headers:l})}if(c&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(c))return new Response(JSON.stringify({ok:!1,message:"\uC774\uBA54\uC77C \uD615\uC2DD\uC774 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4"}),{status:400,headers:l});let R=r,E=String(m).slice(0,200),w=String(_).slice(0,5e3),k=a?String(a).slice(0,100):null,O=c?String(c).slice(0,200):null,N=p?String(p).slice(0,30):null,T=u?String(u).slice(0,500):null,D=i.headers.get("User-Agent")||null,y=i.headers.get("CF-Connecting-IP")||null,b=null;try{let I=i.headers.get("Authorization")||"",L=(I.startsWith("Bearer ")?I.slice(7).trim():null)||h(i);if(L){let B=await t.DB.prepare("SELECT user_id AS id FROM sessions WHERE id = ? LIMIT 1").bind(L).first();B&&(b=B.id)}}catch{}return y&&((await t.DB.prepare(`SELECT COUNT(*) as cnt FROM inquiries
